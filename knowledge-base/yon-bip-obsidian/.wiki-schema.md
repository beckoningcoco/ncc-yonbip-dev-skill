...
```
D:/yon-bip-obsidian/yon-bip-obsidian/
├── raw/                    # 原始素材（AI 只读，不会修改）
│   ├── articles/           # 网页文章
│   ├── tweets/             # X/Twitter 内容
│   ├── wechat/             # 微信公众号文章
...
updated: YYYY-MM-DD
sources: [关联素材列表]
platform_version: "BIP V5"           # 适用平台版本 (必填，如: BIP V5, BIP R6, NCC 2111, NCC 2312)
project: ""                          # 验证项目 (可选，如: 泸州老窖)
last_verified: YYYY-MM-DD            # 最后验证日期 (必填)
status: verified                     # verified | outdated | unverified | superseded
source_type: ""                      # source_code | doc | practice | inference | community
---
...
| 字段 | 必填 | 说明 | 示例值 |
|------|------|------|--------|
| `platform_version` | ✅ 是 | 知识适用的平台及版本 | `BIP V5`, `NCC 2111`, `BIP R6` |
| `project` | 否 | 具体在哪个项目上验证的 | `泸州老窖`, `机械总院` |
| `last_verified` | ✅ 是 | 最后验证日期 | `2026-05-31` |
| `status` | ✅ 是 | verified(已验证) / outdated(已过期) / unverified(未验证) / superseded(已取代) |
| `source_type` | 否 | 知识来源类型 | source_code(源码反编译), doc(官方文档), practice(项目实战), inference(推测), community(社区) |

> **注意**：如果两个 wiki 页面对同一主题给出矛盾的说法，且版本字段不同（如一个 `BIP V5` 一个 `BIP R6`），这不视为冲突——它们是不同版本的不同行为。只有当版本字段相同但说法矛盾时，才需要标记为冲突并触发仲裁。
...
> **原则：每条知识带版本号进来，不让版本歧义进入知识库。**

每次消化素材时，AI 必须执行以下版本信息收集：

1. **自动识别**：从素材内容中自动提取平台版本线索：
   - 文中明确提到 `BIP V5`、`BIP R6`、`NCC 2111`、`NCC 2312` 等版本号
   - 文中描述的功能或 API 可推断出版本归属（如提到"YDS 工具"通常是 BIP V5）
   - 来源 URL 的域名段、文档标题中的版本标注

2. **无法自动识别时主动询问**：
   如果素材中找不到任何版本线索，向用户提问：
   > ⚠️ 未识别到此素材适用的平台版本。请确认：
   > - BIP V5（旗舰版）
   > - BIP R6
   > - NCC 2111
   > - NCC 2312
...
3. **将版本写入 frontmatter**：每篇新生成的 wiki 页面（source、entity、topic）必须包含：
   - `platform_version`：非平台相关的通用知识填 `N/A`
   - `last_verified`：默认为消化日期
   - `status`：默认 `verified`（若素材本身标注为"未验证"或来自社区则填 `unverified`）
   - `source_type`：根据素材来源填写

4. **版本冲突预防**：如果在消化时发现新素材与已有 wiki 页面说法矛盾但 `platform_version` 不同，**不修改已有页面**，而是在新 source 页面的相关观点旁加注释说明版本差异。
...
2. 检查项：
   - 页面间矛盾（同版本下不同页面说法不一致 —— **跨版本的不算矛盾**）
   - 孤立页面（没有其他页面链接到它）
   - 缺失概念页（被 `[[某概念]]` 链接但实际不存在）
   - 缺少交叉引用（相关页面之间没有互相链接）
   - index 一致性（index.md 记录与实际文件是否对应）
   - **版本字段完整性**：检查 frontmatter 是否缺少 `platform_version`、`last_verified`、`status` 必填字段
   - **版本过期检测**：`status: verified` 但 `last_verified` 距今超过 12 个月的页面，标记建议复查
   - **版本冲突检测**：同一 `platform_version` 下，不同页面对同一事实给出矛盾说法，报告为 `[版本冲突]`
