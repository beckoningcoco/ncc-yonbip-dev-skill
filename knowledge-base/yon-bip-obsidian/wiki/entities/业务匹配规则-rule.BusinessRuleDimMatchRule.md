---
tags: ["BIP", "元数据", "数据字典", "ycPriceCenter", "business.rule.BusinessRuleDimMatchRule"]
created: 2026-06-04
updated: 2026-06-04
...
# 业务匹配规则 (`business.rule.BusinessRuleDimMatchRule`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycPriceCenter 模块实体元数据字典。
> 物理表：`cpu_business_rule_dim_match_rule` | 所属应用：`ycPriceCenter`
> 来源：元数据API `queryByUri` 返回的 `business.rule.BusinessRuleDimMatchRule` 实体完整定义。
...
|------|-----|
| 显示名 | 业务匹配规则 |
| 物理表 | `cpu_business_rule_dim_match_rule` |
| domain/服务域 | `cpu-base` |
| schema | `cpu_baseservice` |
| 所属应用 | `ycPriceCenter` |
| 业务对象ID | `d1049e1d-dd14-4249-ae03-edfcaf7d47ea` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 15:29:38.8490` |
...
- **BusinessRuleDimMatchRuleCode** (transformKey): BusinessRuleDimMatchRuleCode — 唯一字段: `['code']`
...
| 7 | `isCharacterSensitive` | 是否特征敏感 | `is_character_sensitive` | Boolean | `switch` |  | ✓ |
...
| 12 | `sensitiveScopes` | 业务匹配规则敏感范围 | `` | 768f5592-eea8-462f-952a-3ed10bf7a602 | `` |  |  |
...
| 2 | `sensitiveScopes` | 业务匹配规则敏感范围 | `business.rule.BusinessRuleDimMatchRuleSensitiveScope` | `` | sensitiveScopes → dimMatchRuleId | 0..n | true | None |  |

### Composition（子表）

- **sensitiveScopes**: `sensitiveScopes` → `business.rule.BusinessRuleDimMatchRuleSensitiveScope` | 0..n
