---
tags: ["BIP", "元数据", "数据字典", "ycPriceCenter", "business.rule.BusinessRuleSceneApplyBillType"]
created: 2026-06-04
updated: 2026-06-04
...
# 业务场景适用单据类型 (`business.rule.BusinessRuleSceneApplyBillType`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycPriceCenter 模块实体元数据字典。
> 物理表：`cpu_business_rule_scene_apply_bill_type` | 所属应用：`ycPriceCenter`
> 来源：元数据API `queryByUri` 返回的 `business.rule.BusinessRuleSceneApplyBillType` 实体完整定义。
...
|------|-----|
| 显示名 | 业务场景适用单据类型 |
| 物理表 | `cpu_business_rule_scene_apply_bill_type` |
| 数据库 schema | `cpu-base` |
| 所属应用 | `ycPriceCenter` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 15:29:42.4750` |
...
| 2 | `sceneId` | 业务场景 | `business.rule.BusinessRuleScene` | `scene_id` | applyBillTypes → sceneId | 1..n | true | None |  |
| 3 | `billTypeId` | 业务规则单据类型 | `business.rule.BusinessRuleBillType` | `bill_type_id` |  →  |  |  | None |  |

### Composition（子表）

- **sceneId**: `applyBillTypes` → `business.rule.BusinessRuleScene` | 1..n
