---
tags: ["BIP", "元数据", "数据字典", "ycPriceCenter", "business.rule.BusinessRuleDimExtMatchRule"]
created: 2026-06-04
updated: 2026-06-04
...
# 业务扩展匹配规则 (`business.rule.BusinessRuleDimExtMatchRule`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycPriceCenter 模块实体元数据字典。
> 物理表：`cpu_business_rule_dim_ext_match_rule` | 所属应用：`ycPriceCenter`
> 来源：元数据API `queryByUri` 返回的 `business.rule.BusinessRuleDimExtMatchRule` 实体完整定义。
...
|------|-----|
| 显示名 | 业务扩展匹配规则 |
| 物理表 | `cpu_business_rule_dim_ext_match_rule` |
| 数据库 schema | `cpu-base` |
| 所属应用 | `ycPriceCenter` |
| 业务对象ID | `cddf6aa0-c47b-464b-a508-035a76e62110` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 15:29:37.1980` |
...
- **BusinessRuleDimExtMatchRulCode** (transformKey): BusinessRuleDimExtMatchRulCode — 唯一字段: `['code']`
...
| 11 | `sensitiveScopes` | 业务扩展匹配规则敏感范围 | `` | fb9ff416-0bb5-4559-b7ab-155b0cc78e68 | `` |  |  |
| 12 | `sysId` | 系统标识 | `sys_id` | Long | `long` |  | ✓ |
...
| 2 | `sensitiveScopes` | 业务扩展匹配规则敏感范围 | `business.rule.BusinessRuleDimExtMatchRuleSensitiveScope` | `` | sensitiveScopes → dimExtMatchRuleId | 1..n | true | None |  |

### Composition（子表）

- **sensitiveScopes**: `sensitiveScopes` → `business.rule.BusinessRuleDimExtMatchRuleSensitiveScope` | 1..n
