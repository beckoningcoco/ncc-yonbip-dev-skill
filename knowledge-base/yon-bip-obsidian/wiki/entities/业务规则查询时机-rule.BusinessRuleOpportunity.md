---
tags: ["BIP", "元数据", "数据字典", "ycPriceCenter", "business.rule.BusinessRuleOpportunity"]
created: 2026-06-04
updated: 2026-06-04
...
# 业务规则查询时机 (`business.rule.BusinessRuleOpportunity`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycPriceCenter 模块实体元数据字典。
> 物理表：`cpu_business_rule_opportunity` | 所属应用：`ycPriceCenter`
> 来源：元数据API `queryByUri` 返回的 `business.rule.BusinessRuleOpportunity` 实体完整定义。
...
|------|-----|
| 显示名 | 业务规则查询时机 |
| 物理表 | `cpu_business_rule_opportunity` |
| domain/服务域 | `cpu-base` |
| schema | `cpu_baseservice` |
| 所属应用 | `ycPriceCenter` |
| 业务对象ID | `b7f30cd3-d3e1-43d1-8ed3-f0eed6a088c5` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 16:19:13.6990` |
...
- **BusinessRuleOpportunityCode** (transformKey): BusinessRuleOpportunityCode — 唯一字段: `['code']`
...
| 7 | `isSys` | 是否系统预置 | `is_sys` | Short | `short` |  | ✓ |
| 8 | `name` | 名称 | `name` | String | `multiLanguage` |  | ✓ |
| 9 | `opportunityApplyBillTypes` | 业务规则查询时机适用单据 | `` | a738d1f3-950a-4ca8-b898-93f341e995c2 | `` |  |  |
| 10 | `pubts` | 时间戳 | `pubts` | DateTime | `timestamp` |  | ✓ |
| 11 | `sysId` | 系统标识 | `sys_id` | Long | `long` |  | ✓ |
...
| 2 | `opportunityApplyBillTypes` | 业务规则查询时机适用单据 | `business.rule.BusinessRuleOpportunityApplyBillType` | `` | opportunityApplyBillTypes → opportunityId | 0..n | true | None |  |

### Composition（子表）

- **opportunityApplyBillTypes**: `opportunityApplyBillTypes` → `business.rule.BusinessRuleOpportunityApplyBillType` | 0..n
