---
tags: [BIP, 元数据, 数据字典, EFA.allocate.FaAllocateOut]
created: 2026-06-03
updated: 2026-06-03
...
# 固定资产调出 (`EFA.allocate.FaAllocateOut`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`fa_bill_allocate_out` | domain：`yonbip-fi-efa` | 应用：`EFA` | 业务对象ID：`9fb75ddf-28be-4d5e-9a69-41d0d73b70f5`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `FaAllocateOutAssetList` | `EFA.allocate.FaAllocateOutAsset` | composition |
| `FaAllocateOutDtlList` | `EFA.allocate.FaAllocateOutDtl` | composition |
...
| `allocate_request_id` | `yonbip-fi-efa.RefTable_a5f0c13498` |
| `free_ch_id` | `` |
| `ytenant_id` | `` |
...
| `allocate_request_id` | `allocate_request_id` | `allocateRequestId` | 调拨申请单id |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `FaAllocateOutAssetList` | 固定资产调出单卡片明细 |
| `` | `` | `FaAllocateOutDtlList` | 固定资产调出单明细 |
