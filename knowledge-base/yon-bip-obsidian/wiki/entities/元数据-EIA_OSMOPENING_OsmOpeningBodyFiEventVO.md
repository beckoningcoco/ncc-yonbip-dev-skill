---
tags: [BIP, 元数据, 数据字典, EIA.OSMOPENING.OsmOpeningBodyFiEventVO]
created: 2026-06-03
updated: 2026-06-03
...
# 委外发料期初综合信息 (`EIA.OSMOPENING.OsmOpeningBodyFiEventVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`ia_ledger_b` | domain：`yonbip-fi-eia` | 应用：`EIA` | 业务对象ID：`c556ed43-38a4-4b65-a216-8c64c74478a8`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `costElements` | `EIA.OSMOPENING.OsmOpeningGrandFiEventVO` | composition |
...
| `decready_acc_v_id` | `fiepub.epub_accsubjectversion_treecard_ref` |
| `free_ch_id` | `` |
...
| `product_acc_v_id` | `fiepub.epub_accsubjectversion_treecard_ref` |
...
| `transfer_carry_down_acc_v_id` | `fiepub.epub_accsubjectversion_treecard_ref` |
...
| `inputtax_acc_v_id` | `fiepub.epub_accsubjectversion_treecard_ref` |
| `inventory_obj_id` | `` |
...
| `busi_bill_id` | `busi_bill_id` | `busiBillId` | 业务id |
...
| `product_acc_id` | `product_acc_id` | `productAccId` | 材料出库科目 |
| `product_acc_v_id` | `product_acc_v_id` | `productAccVId` | 材料出库科目版本ID |
...
| `entrust_acc_id` | `entrust_acc_id` | `entrustAccId` | 委托发出物资科目 |
| `entrust_acc_v_id` | `entrust_acc_v_id` | `entrustAccVId` | 委托加工物资科目版本ID |
...
| `header_id` | `header_id` | `header` | 委外发料期初主表 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `mat_ch_id` | `mat_ch_id` | `matChId` | 物料ID自由项特征组 |
