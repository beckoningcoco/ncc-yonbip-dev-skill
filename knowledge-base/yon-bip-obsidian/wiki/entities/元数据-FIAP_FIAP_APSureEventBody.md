---
tags: [BIP, 元数据, 数据字典, FIAP.FIAP.APSureEventBody]
created: 2026-06-03
updated: 2026-06-03
...
# 确认应付明细信息 (`FIAP.FIAP.APSureEventBody`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`arap_ledger_b` | domain：`yonbip-fi-earap` | 应用：`EAP` | 业务对象ID：`c85ee8fd-6c38-4ed3-a31c-de29142b00a0`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `grandsonItem` | `FIAP.FIAP.APSureEventGrandson` | composition |
| `taxGrandsonItem` | `FIAP.FIAP.taxInvoiceNewTaxCalculationResult` | composition |
| `withHoldingTax` | `FIAP.FIAP.APSureEventWithHoldingTax` | composition |
...
| `event_id` | `event_id` | `event` | 确认应付事务 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `grandsonItem` | 确认应付计划信息 |
| `` | `` | `taxGrandsonItem` | 计税单 |
| `` | `` | `withHoldingTax` | 确认应付代扣税信息 |
