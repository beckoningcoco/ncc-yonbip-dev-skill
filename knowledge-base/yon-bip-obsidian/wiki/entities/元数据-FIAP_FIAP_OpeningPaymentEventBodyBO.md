---
tags: [BIP, 元数据, 数据字典, FIAP.FIAP.OpeningPaymentEventBodyBO]
created: 2026-06-03
updated: 2026-06-03
...
# 期初付款明细信息 (`FIAP.FIAP.OpeningPaymentEventBodyBO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`arap_ledger_b` | domain：`yonbip-fi-earap` | 应用：`EAP` | 业务对象ID：`f9070991-93a9-4e31-bfc0-d5e9b390df06`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `taxGrandsonItem` | `FIAP.FIAP.OpeningPaymentEventBOTaxCalculationResult` | composition |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `event_id` | `event_id` | `event` | 期初付款事务主表 |
