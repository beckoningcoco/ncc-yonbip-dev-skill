---
tags: [BIP, 元数据, 数据字典, FIAP.FIAP.apJournalAccEventBodyBO]
created: 2026-06-03
updated: 2026-06-03
...
# 应付流水账事务明细信息 (`FIAP.FIAP.apJournalAccEventBodyBO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`arap_ledger_b` | domain：`yonbip-fi-earap` | 应用：`EAP` | 业务对象ID：`b5381420-aa2d-498f-9ca0-eb3187bfe577`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `grandsonItem` | `FIAP.FIAP.apJournalAccEventGrandsonBO` | composition |
| `withHoldingTax` | `FIAP.FIAP.apJournalAccEventWithHoldingTaxBO` | composition |
...
| `ori_rebate_deduct_amount` | `ori_rebate_deduct_amount` | `oriRebateDeductAmount` | 返利直接抵现金额(应收返利用)返利直接抵现金额(应收返利用) |
...
| `event_id` | `event_id` | `event` | 应付事务ID |
...
| `tax_in_buff_subject_vid` | `tax_in_buff_subject_vid` | `taxInBuffSubjectVid` | 进项税中转入科目版本 |
...
| `tax_out_buff_subject_vid` | `tax_out_buff_subject_vid` | `taxOutBuffSubjectVid` | 进项税中转科目版本 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `grandsonItem` | 应付流水账事务计划信息 |
| `` | `` | `withHoldingTax` | 应付流水账事务代扣税信息 |
