---
tags: [BIP, 元数据, 数据字典, FIAR.FIAR.AREventBodyBase]
created: 2026-06-03
updated: 2026-06-03
...
# 应收事务明细基类 (`FIAR.FIAR.AREventBodyBase`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`arap_ledger_b` | domain：`yonbip-fi-earap` | 应用：`EAR` | 业务对象ID：`15796d11-c64c-4d48-baae-97ce067258e3`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `grandsonItem` | `FIAR.FIAR.AREventGrandsonBase` | composition |
| `withHoldingTax` | `FIAR.FIAR.AREventWithHoldingTaxBase` | composition |
...
| `event_id` | `event_id` | `event` | 应收事务主表基类 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `grandsonItem` | 应收事务计划基类 |
| `` | `` | `withHoldingTax` | 应收事务代扣税基类 |
