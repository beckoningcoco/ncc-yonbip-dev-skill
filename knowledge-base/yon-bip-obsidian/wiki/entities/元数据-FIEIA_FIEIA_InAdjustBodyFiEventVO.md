---
tags: [BIP, 元数据, 数据字典, FIEIA.FIEIA.InAdjustBodyFiEventVO]
created: 2026-06-03
updated: 2026-06-03
...
# 损益入库调整综合信息 (`FIEIA.FIEIA.InAdjustBodyFiEventVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`ia_ledger_b` | domain：`yonbip-fi-eia` | 应用：`EIA` | 业务对象ID：`a96f1bc5-307b-4cc3-bd0d-473e023785ce`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `costElements` | `FIEIA.FIEIA.InAdjustGrandFiEventVO` | composition |
...
| `batch_no_id` | `ustock.st_batchnoref` |
...
| `batch_no_id` | `batch_no_id` | `batchNoo` | 批次 |
...
| `header_id` | `header_id` | `header` | 损益入库调整 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `free_ch_id` | `free_ch_id` | `freeChId` | 损益入库调整综合信息自定义项特征 |

### other (1个)

| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `costElements` | 损益入库调整分项明细 |
