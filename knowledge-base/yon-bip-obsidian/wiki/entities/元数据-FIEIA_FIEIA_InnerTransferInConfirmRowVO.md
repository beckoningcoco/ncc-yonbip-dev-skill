---
tags: [BIP, 元数据, 数据字典, FIEIA.FIEIA.InnerTransferInConfirmRowVO]
created: 2026-06-03
updated: 2026-06-03
...
# 内部交易入库确认综合信息 (`FIEIA.FIEIA.InnerTransferInConfirmRowVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`ia_ledger_b` | domain：`yonbip-fi-eia` | 应用：`EIA` | 业务对象ID：`06435280-5fe8-441b-a6c1-666922cdbcb6`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `costItems` | `FIEIA.FIEIA.InnerTransferInConfirmItemVO` | composition |
| `costElements` | `FIEIA.FIEIA.InnerTransferInConfirmEleVO` | composition |
...
| `header_id` | `header_id` | `header` | 内部交易入库确认 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `free_ch_id` | `free_ch_id` | `freeChId` | 内部交易入库确认综合信息自定义项特征 |

### other (2个)

| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `costElements` | 内部交易入库确认分项信息 |
| `` | `` | `costItems` | 内部交易入库确认费用项目 |
