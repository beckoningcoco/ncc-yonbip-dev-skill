---
tags: [BIP, 元数据, 数据字典, FIEIA.FIEIA.InventoryLossOutRowVO]
created: 2026-06-03
updated: 2026-06-03
...
# 盘亏出库核算综合信息 (`FIEIA.FIEIA.InventoryLossOutRowVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`ia_ledger_b` | domain：`yonbip-fi-eia` | 应用：`EIA` | 业务对象ID：`4876a9ae-c7da-43c8-bc61-a8cd78016a42`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `costElements` | `FIEIA.FIEIA.InventoryLossOutCostElemVO` | composition |
...
| `header_id` | `header_id` | `header` | 盘亏出库核算 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `free_ch_id` | `free_ch_id` | `freeChId` | 盘亏出库综合信息自定义项特征 |

### other (1个)

| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `costElements` | 盘亏出库核算分项信息 |
