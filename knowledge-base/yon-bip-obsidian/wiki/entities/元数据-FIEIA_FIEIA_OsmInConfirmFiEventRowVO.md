---
tags: [BIP, 元数据, 数据字典, FIEIA.FIEIA.OsmInConfirmFiEventRowVO]
created: 2026-06-03
updated: 2026-06-03
...
# 委外入库确认综合信息 (`FIEIA.FIEIA.OsmInConfirmFiEventRowVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`ia_ledger_b` | domain：`yonbip-fi-eia` | 应用：`EIA` | 业务对象ID：`35398192-b208-4bce-8f51-6581daee09a4`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `costItems` | `FIEIA.FIEIA.OsmInConfirmCostItemVO` | composition |
| `costElements` | `FIEIA.FIEIA.OsmInConfirmCostElementVO` | composition |
...
| `ipv_rate_acc_id` | `fiepub_accsubjectauthref` |
...
| `entrust_acc_v_id` | `entrust_acc_v_id` | `entrustAccVId` | 委托发出物资科目版本ID |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `free_ch_id` | `free_ch_id` | `freeChId` | 委外入库确认综合信息自定义项特征 |

### other (2个)

| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `costElements` | 委外入库确认分项信息 |
| `` | `` | `costItems` | 委外入库确认费用项目 |
