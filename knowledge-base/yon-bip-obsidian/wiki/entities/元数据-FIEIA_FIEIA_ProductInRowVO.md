---
tags: [BIP, 元数据, 数据字典, FIEIA.FIEIA.ProductInRowVO]
created: 2026-06-03
updated: 2026-06-03
...
# 生产入库核算综合信息 (`FIEIA.FIEIA.ProductInRowVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`ia_ledger_b` | domain：`yonbip-fi-eia` | 应用：`EIA` | 业务对象ID：`5c733fd1-dbf9-4579-879f-9adf9e468f36`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `costItems` | `FIEIA.FIEIA.ProductInCostItemVO` | composition |
| `costElements` | `FIEIA.FIEIA.ProductInCostElementVO` | composition |
...
| `bln_alloc` | `bln_alloc` | `blnAlloc` | 是否分配 |
...
| `header_id` | `header_id` | `header` | 产品入库核算 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `free_ch_id` | `free_ch_id` | `freeChId` | 生产入库综合信息自定义项特征 |

### other (2个)

| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `costElements` | 生产入库核算分项信息 |
| `` | `` | `costItems` | 生产入库核算费用项目 |
