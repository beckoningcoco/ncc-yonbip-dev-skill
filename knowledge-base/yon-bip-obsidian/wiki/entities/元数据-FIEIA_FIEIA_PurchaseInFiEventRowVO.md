---
tags: [BIP, 元数据, 数据字典, FIEIA.FIEIA.PurchaseInFiEventRowVO]
created: 2026-06-03
updated: 2026-06-03
...
# 采购入库暂估综合信息 (`FIEIA.FIEIA.PurchaseInFiEventRowVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`ia_ledger_b` | domain：`yonbip-fi-eia` | 应用：`EIA` | 业务对象ID：`3ea30f00-cabd-4e1b-b299-6f71e652cea9`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `costItems` | `FIEIA.FIEIA.PurchaseInCostItemVO` | composition |
| `costElements` | `FIEIA.FIEIA.PurchaseInCostElementVO` | composition |
...
| `header_id` | `header_id` | `header` | 采购入库暂估 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `free_ch_id` | `free_ch_id` | `freeChId` | 自定义项特征 |

### other (2个)

| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `costElements` | 采购入库暂估分项信息 |
| `` | `` | `costItems` | 采购入库暂估费用项目 |
