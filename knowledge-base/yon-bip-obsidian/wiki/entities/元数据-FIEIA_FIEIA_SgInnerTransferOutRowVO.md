---
tags: [BIP, 元数据, 数据字典, FIEIA.FIEIA.SgInnerTransferOutRowVO]
created: 2026-06-03
updated: 2026-06-03
...
# 内部交易发出商品核算综合信息 (`FIEIA.FIEIA.SgInnerTransferOutRowVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`ia_ledger_b` | domain：`yonbip-fi-eia` | 应用：`EIA` | 业务对象ID：`ea0105d3-4722-4995-8610-4695726ec1ef`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `costElements` | `FIEIA.FIEIA.SgInnerTransferOutCostElemVO` | composition |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `header_id` | `header_id` | `header` | 内部交易发出商品核算 |
...
| `carry_over_of_cost_method` | `carry_over_of_cost_method` | `carryOverOfCostMethod` | 结转方式 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `free_ch_id` | `free_ch_id` | `freeChId` | 内部交易发出商品综合信息自定义项特征 |
