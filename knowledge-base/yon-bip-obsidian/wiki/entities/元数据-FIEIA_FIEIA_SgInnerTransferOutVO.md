---
tags: [BIP, 元数据, 数据字典, FIEIA.FIEIA.SgInnerTransferOutVO]
created: 2026-06-03
updated: 2026-06-03
...
# 内部交易发出商品核算 (`FIEIA.FIEIA.SgInnerTransferOutVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`ia_ledger` | domain：`yonbip-fi-eia` | 应用：`EIA` | 业务对象ID：`ea0105d3-4722-4995-8610-4695726ec1ef`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `bodies` | `FIEIA.FIEIA.SgInnerTransferOutRowVO` | composition |
...
| `sterilisation_bill_id` | `sterilisation_bill_id` | `sterilisationBillId` | 待结算单ID |
| `sterilisation_bill_no` | `sterilisation_bill_no` | `sterilisationBillNo` | 待结算单号 |
...
| `carryover_cost_bill_type` | `carryover_cost_bill_type` | `carryoverCostBillType` | 结转成本单据 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `free_ch_id` | `free_ch_id` | `freeChId` | 内部交易发出商品自定义项特征 |

### other (1个)

| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `bodies` | 内部交易发出商品核算综合信息 |
