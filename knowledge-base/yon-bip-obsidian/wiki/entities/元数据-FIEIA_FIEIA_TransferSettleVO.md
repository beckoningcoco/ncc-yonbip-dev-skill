---
tags: [BIP, 元数据, 数据字典, FIEIA.FIEIA.TransferSettleVO]
created: 2026-06-03
updated: 2026-06-03
...
# 内部结算 (`FIEIA.FIEIA.TransferSettleVO`)
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `bodies` | `FIEIA.FIEIA.TransferSettleRowVO` | composition |
...
| `in_currency_id` | `ucfbasedoc.bd_currencytenantref` |
| `out_rate_type` | `ucfbasedoc.bd_exchangeratetyperef` |
| `in_stock_org` | `ucf-org-center.bd_inventoryorg` |
...
| `src_busi_bill_type` | `src_busi_bill_type` | `srcBusiBillType` | 原始业务单据类型 |
...
| `src_busi_bill_code` | `src_busi_bill_code` | `srcBusiBillCode` | 原始业务单据编号 |
...
| `in_stock_org` | `in_stock_org` | `inStockOrg` | 调入库存组织 |
| `out_rate_type` | `out_rate_type` | `inRateType` | 调入汇率类型 |
| `in_currency_id` | `in_currency_id` | `inCurrencyId` | 调入方本币 |
...
| `bln_esti_adjust` | `bln_esti_adjust` | `blnEstiAdjust` | 结算红冲 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `out_tax_rate` | `out_tax_rate` | `inTaxRate` | 调入汇率 |
