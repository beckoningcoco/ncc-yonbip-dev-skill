---
tags: [BIP, 元数据, 数据字典, RCL.RCL.RclSettleInfoBody]
created: 2026-06-03
updated: 2026-06-03
...
# 结算信息子表 (`RCL.RCL.RclSettleInfoBody`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`rcl_settleinfo_b` | domain：`yonbip-fi-ercl` | 应用：`RVN` | 业务对象ID：`66bfd372-2062-4aaa-933e-5e8020b9489c`
...
| `rcl_settleinfo_h_id` | `` |
| `settle_start_period_id` | `finbd.bd_period` |
| `rcl_contract_baseinfo_id` | `` |
...
| `contract_id` | `contract_id` | `contractId` | 履约义务信息ID |
...
| `rcl_contract_code` | `rcl_contract_code` | `rclContractCode` | 合同号 |
...
| `rcl_contract_baseinfo_id` | `rcl_contract_baseinfo_id` | `rclContractBaseInfo` | 履约义务基本信息行 |
| `rcl_settleinfo_h_id` | `rcl_settleinfo_h_id` | `rclSettleInfoHeader_id` | 结算信息主表 |
| `settle_end_period_id` | `settle_end_period_id` | `settleEndPeriod` | 结算截止期间 |
| `settle_start_period_id` | `settle_start_period_id` | `settleStartPeriod` | 结算开始期间 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `is_push_rvn_flag_from_up` | `is_push_rvn_flag_from_up` | `isPushRvnFlagFromUp` | 是否推送收入标志从上游传入 |
...
| `rcl_revenue_contract_version` | `rcl_revenue_contract_version` | `rclRevenueContractVersion` | 履约义版本号 |
| `tax` | `tax` | `tax` | 税额 |
