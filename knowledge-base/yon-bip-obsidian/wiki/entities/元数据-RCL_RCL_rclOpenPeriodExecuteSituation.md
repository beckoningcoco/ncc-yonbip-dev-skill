---
tags: [BIP, 元数据, 数据字典, RCL.RCL.rclOpenPeriodExecuteSituation]
created: 2026-06-03
updated: 2026-06-03
...
# 期初分期执行情况 (`RCL.RCL.rclOpenPeriodExecuteSituation`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`rcl_periodexec` | domain：`yonbip-fi-ercl` | 应用：`RVN` | 业务对象ID：`0dcdcca0-a433-419f-8e06-38e109b5ba50`
...
| `cur_carry_forward_cost_save` | `cur_carry_forward_cost_save` | `curCarryForwardCostSave` | 本次结转成本（保存） |
| `cur_confirm_loss` | `cur_confirm_loss` | `curConfirmLoss` | 本次确认损失 |
| `cur_confirm_loss_save` | `cur_confirm_loss_save` | `curConfirmLossSave` | 本次确认损失（保存） |
