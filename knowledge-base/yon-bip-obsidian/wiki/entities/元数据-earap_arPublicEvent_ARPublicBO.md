---
tags: [BIP, 元数据, 数据字典, earap.arPublicEvent.ARPublicBO]
created: 2026-06-03
updated: 2026-06-03
...
# 日常应收基本信息 (`earap.arPublicEvent.ARPublicBO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`arap_ledger_h` | domain：`yonbip-fi-earap` | 应用：`EAR` | 业务对象ID：`27906f1b-359e-44fb-932f-ce3a9b5444e0`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `bodyItem` | `earap.arPublicEvent.ARPublicBodyBO` | composition |
...
| `write_off_src_data_code` | `write_off_src_data_code` | `writeOffSrcDataCode` | 被冲销会计事务记录号 |
...
| `id` | `id` | `id` | 主键 |
| `opp_bill_id` | `opp_bill_id` | `oppBillId` | 源头单据ID |
...
| `src_app_id` | `src_app_id` | `srcApp` | 来源应用列表 |
...
| `hangup_type` | `hangup_type` | `hangupType` | 挂起类型 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `acc_amount` | `acc_amount` | `accAmount` | 本币含税金额 |
