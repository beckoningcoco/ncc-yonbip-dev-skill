---
tags: [BIP, 元数据, 数据字典, earap.collection.CollectionBody]
created: 2026-06-03
updated: 2026-06-03
...
# 收款单明细信息 (`earap.collection.CollectionBody`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`ar_collection_b` | domain：`yonbip-fi-earapbill` | 应用：`EAR` | 业务对象ID：`032d3a15-07c4-4713-83ec-173cac7a6449`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `taxCalculationResultList` | `earap.collection.TaxCalculationResult` | composition |
...
| `src_order_type_id` | `transtype.bd_billtypetreeref` |
...
| `baddebt_loss_currency_id` | `ucfbasedoc.bd_currencytenantref` |
| `` | `` |
...
| `src_claim_id` | `src_claim_id` | `srcClaimId` | 认领单ID |
| `src_claim_row_id` | `src_claim_row_id` | `srcClaimRowId` | 认领单明细ID |
...
| `baddebt_loss_currency_id` | `baddebt_loss_currency_id` | `baddebtLossCurrency` | 坏账损失币种 |
...
| `header_id` | `header_id` | `headerId` | 收款基本信息ID |
...
| `ori_baddebt_gain_amount` | `ori_baddebt_gain_amount` | `oriBaddebtGainAmount` | 坏账损失收回原币金额 |
