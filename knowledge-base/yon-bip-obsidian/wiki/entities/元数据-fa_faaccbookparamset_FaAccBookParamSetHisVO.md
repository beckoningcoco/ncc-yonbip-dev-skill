---
tags: [BIP, 元数据, 数据字典, fa.faaccbookparamset.FaAccBookParamSetHisVO]
created: 2026-06-03
updated: 2026-06-03
...
# 固定资产业务账簿参数历史 (`fa.faaccbookparamset.FaAccBookParamSetHisVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`fa_accbook_param_version` | domain：`yonbip-fi-efa` | 应用：`EFA` | 业务对象ID：`7d7b01d1-047f-49b9-80f7-94c290198193`
...
| `current_account_period_code` | `current_account_period_code` | `currentAccountPeriod` | 当前会计期间编码 |
...
| `log_type` | `log_type` | `logType` | 日志类型 |
| `accbook_param_id` | `accbook_param_id` | `parId` | 主表id |
...
| `current_account_period_id` | `current_account_period_id` | `currentAccountPeriodId` | 当前会计期间ID |
...
| `bln_first_year_disposal_depr` | `bln_first_year_disposal_depr` | `isFirstyeardisposal` | 使用第一年处置计提折旧 |
...
| `version_no` | `version_no` | `version` | 版本号 |
