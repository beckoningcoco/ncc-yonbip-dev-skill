---
tags: [BIP, 元数据, 数据字典, fa.faadjust.CalcInfoChangeBill]
created: 2026-06-03
updated: 2026-06-03
...
# 核算信息变动 (`fa.faadjust.CalcInfoChangeBill`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`fa_bill_adjust_calc` | domain：`yonbip-fi-efa` | 应用：`EFA` | 业务对象ID：`29c21e74-f625-40f2-8c1a-4d479ecc37ab`
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `subjects` | `fa.faadjust.DeprSubjectChangeBill` | composition |
...
| `asset_code` | `asset_code` | `assetCode` | 来源卡片编码 |
| `direction` | `direction` | `direction` | 方向 |
| `snaphot_id` | `snaphot_id` | `snaphotId` | 核算信息快照id |
| `src_calculate_id` | `src_calculate_id` | `srcCalculateId` | 来源固定资产核算信息id |
| `accounting_classifi` | `accounting_classifi` | `booktype` | 账簿分类 |
| `period_code` | `period_code` | `period` | 账薄会计期间 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `adjust_base_id` | `adjust_base_id` | `adjustbaseId` | 基本信息变动ID |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `acc_accum_depr_amount_difference` | `acc_accum_depr_amount_difference` | `accAccumDeprAmountDifference` | 本币累计折旧发生额 |
| `acc_adjust_depr_amount` | `acc_adjust_depr_amount` | `accAdjustDeprAmount` | 本币追溯调整折旧 |
| `acc_currtype_ori_amount_difference` | `acc_currtype_ori_amount_difference` | `accCurrtypeOriAmountDifference` | 本币原值发生额 |
...
| `year_depr_amount` | `year_depr_amount` | `yearDeprAmount` | 本币本年折旧额 |
...
| `period_depr_amount` | `period_depr_amount` | `monthDeprAmount` | 本币期折旧额 |
