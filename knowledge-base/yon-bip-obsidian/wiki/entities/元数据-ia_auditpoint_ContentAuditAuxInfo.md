---
tags: [BIP, 元数据, 数据字典, ia.auditpoint.ContentAuditAuxInfo]
created: 2026-06-03
updated: 2026-06-03
...
# 非标附件检查辅助信息 (`ia.auditpoint.ContentAuditAuxInfo`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`ssc_ia_content_audit_aux_info` | domain：`yonbip-mid-sscia` | 应用：`ZNSH` | 业务对象ID：``
...
|------|-----|
| 显示名 | 非标附件检查辅助信息 |
| 物理表 | `ssc_ia_content_audit_aux_info` |
...
| `field_display_name` | `field_display_name` | `fieldDisplayName` | 单据字段名称 |
| `field_name` | `field_name` | `fieldName` | 单据字段 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `alias_name` | `alias_name` | `aliasName` | 单据字段别名 |
