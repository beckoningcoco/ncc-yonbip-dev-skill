---
tags: [BIP, 元数据, 数据字典, tlm.derivativesregister.DerivativesRegisterAgreement]
created: 2026-06-03
updated: 2026-06-03
...
# 衍生品合约关联授信子表 (`tlm.derivativesregister.DerivativesRegisterAgreement`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`tlm_derivativesregister_agreement` | domain：`yonbip-fi-ctmtlm` | 应用：`TLM` | 业务对象ID：``
...
|------|-----|
| 显示名 | 衍生品合约关联授信子表 |
| 物理表 | `tlm_derivativesregister_agreement` |
...
| `protocol_currency` | `protocol_currency` | `protocolCurrency` | 业务币种 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `current_amount` | `current_amount` | `currentAmount` | 本次占用额度(业务币种) |
...
| `remain_amount` | `remain_amount` | `remainAmount` | 剩余占用额度(授信币种) |
| `total_amount` | `total_amount` | `totalAmount` | 总占用额度(授信币种) |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `agreement_define_character` | `agreement_define_character` | `fRAgreementCharacterDef` | 衍生品合约关联授信子表特征 |
