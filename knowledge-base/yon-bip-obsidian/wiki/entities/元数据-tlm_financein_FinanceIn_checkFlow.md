---
tags: [BIP, 元数据, 数据字典, tlm.financein.FinanceIn_checkFlow]
created: 2026-06-03
updated: 2026-06-03
...
# 融入核算流 (`tlm.financein.FinanceIn_checkFlow`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`tlm_financein_checkFlow` | domain：`yonbip-fi-ctmtlm` | 应用：`TLM` | 业务对象ID：``
...
| `deal_cc_amount` | `deal_cc_amount` | `dealCcAmount` | 实际收付金额(原币币种) |
...
| `deal_all_paid_oc_amount` | `deal_all_paid_oc_amount` | `dealAllPaidOcAmount` | 实际收付金额（本币币种） |
...
| `declared_unpaid_cc_interest` | `declared_unpaid_cc_interest` | `declaredUnpaidCcInterest` | 已宣告未发放金额(原币币种) |
| `declared_unpaid_oc_interest` | `declared_unpaid_oc_interest` | `declaredUnpaidOcInterest` | 已宣告未发放金额(本币币种) |
