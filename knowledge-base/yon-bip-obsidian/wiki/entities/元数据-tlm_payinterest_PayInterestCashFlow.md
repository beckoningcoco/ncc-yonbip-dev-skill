---
tags: [BIP, 元数据, 数据字典, tlm.payinterest.PayInterestCashFlow]
created: 2026-06-03
updated: 2026-06-03
...
# 付息现金流子表 (`tlm.payinterest.PayInterestCashFlow`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`tlm_payinterest_cashflow` | domain：`yonbip-fi-ctmtlm` | 应用：`TLM` | 业务对象ID：``
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `payInterestCashflowInterest` | `tlm.payinterest.PayInterestCashflowInterest` | composition |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `debt_id` | `debt_id` | `debtId` | 融资付息主表id |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `payInterestCashflowInterest` | 付息现金流利息分段 |
