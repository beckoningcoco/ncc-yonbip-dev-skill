---
tags: [BIP, 元数据, 数据字典, tlm.purchasechange.PurchaseChange_cashflowBackUp]
created: 2026-06-03
updated: 2026-06-03
...
# 申购变更现金流备份表 (`tlm.purchasechange.PurchaseChange_cashflowBackUp`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`tlm_purchasechange_cashflow_backup` | domain：`yonbip-fi-ctmtlm` | 应用：`TLM` | 业务对象ID：``
...
|------|-----|
| 显示名 | 申购变更现金流备份表 |
| 物理表 | `tlm_purchasechange_cashflow_backup` |
...
| 字段名 | URI | 关系 |
|--------|-----|------|
| `purchaseChange_cashflow_interestBackUp` | `tlm.purchasechange.PurchaseChange_cashflow_interestBackUp` | composition |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `accentity_Name` | `accentity_Name` | `accentityName` | 会计主体 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `` | `` | `purchaseChange_cashflow_interestBackUp` | 申购现金流利息分段备份 |
