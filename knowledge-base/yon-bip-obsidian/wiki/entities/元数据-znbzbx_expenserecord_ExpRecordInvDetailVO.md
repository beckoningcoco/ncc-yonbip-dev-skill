---
tags: [BIP, 元数据, 数据字典, znbzbx.expenserecord.ExpRecordInvDetailVO]
created: 2026-06-03
updated: 2026-06-03
...
# 费用记事账单明细 (`znbzbx.expenserecord.ExpRecordInvDetailVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`znbz_exprecordinvdetail` | domain：`znbzbx` | 应用：`RBSM` | 业务对象ID：``
...
| `vcurrency` | `vcurrency` | `vcurrency` | 发票币种 |
...
| `pk_tallydata` | `pk_tallydata` | `pk_tallydata` | 记账数据 |
...
| `nmny` | `nmny` | `nmny` | 价税合计 |
| `ndeductrate` | `ndeductrate` | `ndeductrate` | 税率 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `expRecordInvDetailDcs` | `expRecordInvDetailDcs` | `expRecordInvDetailDcs` | 费用记事发票明细自定义项 |
