---
tags: [BIP, 元数据, 数据字典, znbzbx.paymentapply.DeferPaymentapplyVO]
created: 2026-06-03
updated: 2026-06-03
...
# 挂账付款申请单费用分摊 (`znbzbx.paymentapply.DeferPaymentapplyVO`)

> **平台版本：BIP 旗舰版 V5**
> 物理表：`znbz_deferpaymentapply` | domain：`znbzbx` | 应用：`RBSM` | 业务对象ID：``
...
| `pk_costcenter` | `finbd.bd_costcenterref` |
...
| `pk_paymentapply` | `pk_paymentapply` | `pk_paymentapply` | 主表id |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `napporttaxmny` | `napporttaxmny` | `napporttaxmny` | 税额 |
| `nnatapporttaxmny` | `nnatapporttaxmny` | `nnatapporttaxmny` | 税额-本币 |
...
| 字段名 | 数据库列 | 字段编码 | 显示名 |
|--------|---------|---------|--------|
| `deferPaymentapplyDcs` | `deferPaymentapplyDcs` | `deferPaymentapplyDcs` | 费用分摊自定义项 |
