---
tags: ["BIP", "元数据", "数据字典", "ycBase", "announcement.announcement.AnnouncementDisPlayLocationInst"]
created: 2026-06-04
updated: 2026-06-04
...
# 公告展示位置实例 (`announcement.announcement.AnnouncementDisPlayLocationInst`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_announcement_display_location_inst` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `announcement.announcement.AnnouncementDisPlayLocationInst` 实体完整定义。
...
|------|-----|
| 显示名 | 公告展示位置实例 |
| 物理表 | `cpu_announcement_display_location_inst` |
| 数据库 schema | `yonbip-cpu-bctask` |
| 所属应用 | `ycBase` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 15:15:21.2230` |
...
| 2 | `announcementDisplayLocation` | 公告展示位置 | `announcement_display_location_id` | 400db248-4f86-40f7-a2c7-8ccd4af78738 | `quote` |  | ✓ |
...
| 7 | `displayInsCode` | 公告展示位置编码 | `display_ins_code` | String | `text` |  | ✓ |
| 8 | `displayInsName` | 公告展示位置名称 | `display_ins_name` | String | `text` |  | ✓ |
...
| 4 | `announcementDisplayLocation` | 公告展示位置 | `announcement.announcement.AnnouncementDisPlayLocation` | `announcement_display_location_id` |  →  |  |  | None |  |
| 5 | `announcement` | 公告 | `announcement.announcement.CorporateAnnouncement` | `announcement_id` | disPlayLocationList → announcement | 1..n | true | None |  |

### Composition（子表）

- **announcement**: `disPlayLocationList` → `announcement.announcement.CorporateAnnouncement` | 1..n
