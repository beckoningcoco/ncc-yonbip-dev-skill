---
tags: ["BIP", "元数据", "数据字典", "ycBase", "announcement.announcement.AnnouncementRecord"]
created: 2026-06-04
updated: 2026-06-04
...
# 公告阅读记录 (`announcement.announcement.AnnouncementRecord`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_announcement_record` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `announcement.announcement.AnnouncementRecord` 实体完整定义。
...
| 13 | `readtime` | 首次阅读时间 | `read_time` | DateTime | `timestamp` |  | ✓ |
| 14 | `supply` | 供应商 | `supply_id` | 89bf026e-cc49-4fe7-9a7b-3e8fdcc77c1e | `quote` |  | ✓ |
| 15 | `unbind` | 供应商是否解绑 | `unbind` | Short | `short` |  | ✓ |
| 16 | `vendorTenant` | 供应商租户 | `vendor_tenant_id` | String | `text` |  | ✓ |
...
| 5 | `announcement` | 公告 | `announcement.announcement.CorporateAnnouncement` | `announcement_id` | recordList → announcement | 0..n | true | None |  |

### Composition（子表）

- **announcement**: `recordList` → `announcement.announcement.CorporateAnnouncement` | 0..n
