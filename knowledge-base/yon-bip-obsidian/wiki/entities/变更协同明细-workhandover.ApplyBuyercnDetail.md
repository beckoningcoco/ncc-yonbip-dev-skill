---
tags: ["BIP", "元数据", "数据字典", "ycBase", "workhandover.workhandover.ApplyBuyercnDetail"]
created: 2026-06-04
updated: 2026-06-04
...
# 变更协同明细 (`workhandover.workhandover.ApplyBuyercnDetail`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_workhandover_buyercn_detail` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `workhandover.workhandover.ApplyBuyercnDetail` 实体完整定义。
...
|------|-----|
| 显示名 | 变更协同明细 |
| 物理表 | `cpu_workhandover_buyercn_detail` |
| 数据库 schema | `cpu-bi-service` |
| 所属应用 | `ycBase` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 15:19:03.6660` |
...
|---|--------|--------|---------|------|---------|------|------|
| 1 | `buyercn` | 变更协同 | `buyercn` | d3bb275c-bac7-4dbd-b665-65caf68bdd74 | `quote` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `workHandOverApplyId` | 工作移交申请主键 | `workhandover.workhandover.Apply` | `apply_id` | buyercnDetailList → workHandOverApplyId | 0..n | true | None |  |
...
| 5 | `buyercn` | 变更协同 | `puchangeCollab.buyerCn.CpuBuyerCnVO` | `buyercn` |  →  |  |  | Service |  |

### Composition（子表）

- **workHandOverApplyId**: `buyercnDetailList` → `workhandover.workhandover.Apply` | 0..n
...
| `buyercn` (变更协同) | `puchangeCollab.buyerCn.CpuBuyerCnVO` | `buyercn` |
