---
tags: ["BIP", "元数据", "数据字典", "ycBase", "category.level.ReportDetailDefineCharacter"]
created: 2026-06-04
updated: 2026-06-04
...
# 品类定级评估报告表体特征 (`category.level.ReportDetailDefineCharacter`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_category_level_report_detail_character` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `category.level.ReportDetailDefineCharacter` 实体完整定义。
...
|------|-----|
| 显示名 | 品类定级评估报告表体特征 |
| 物理表 | `cpu_category_level_report_detail_character` |
