---
tags: ["BIP", "元数据", "数据字典", "ycBase", "workhandover.workhandover.ApplyCheckpoolDetail"]
created: 2026-06-04
updated: 2026-06-04
...
# 对账事项明细 (`workhandover.workhandover.ApplyCheckpoolDetail`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_workhandover_checkpool_detail` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `workhandover.workhandover.ApplyCheckpoolDetail` 实体完整定义。
...
|------|-----|
| 显示名 | 对账事项明细 |
| 物理表 | `cpu_workhandover_checkpool_detail` |
...
|---|--------|--------|---------|------|---------|------|------|
| 1 | `checkpool` | 对账事项 | `checkpool` | 3cf2adf0-16c3-4d8e-a9d5-69583d569914 | `quote` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `workHandOverApplyId` | 工作移交申请主键 | `workhandover.workhandover.Apply` | `apply_id` | checkpoolDetailList → workHandOverApplyId | 0..n | true | None |  |
...
| 5 | `checkpool` | 对账事项 | `check.checkpool.GodownEntryVO` | `checkpool` |  →  |  |  | Service |  |

### Composition（子表）

- **workHandOverApplyId**: `checkpoolDetailList` → `workhandover.workhandover.Apply` | 0..n
...
| `checkpool` (对账事项) | `check.checkpool.GodownEntryVO` | `checkpool` |
