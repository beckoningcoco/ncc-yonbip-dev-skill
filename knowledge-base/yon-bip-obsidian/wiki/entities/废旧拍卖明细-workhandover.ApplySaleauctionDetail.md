---
tags: ["BIP", "元数据", "数据字典", "ycBase", "workhandover.workhandover.ApplySaleauctionDetail"]
created: 2026-06-04
updated: 2026-06-04
...
# 废旧拍卖明细 (`workhandover.workhandover.ApplySaleauctionDetail`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_workhandover_saleauction_detail` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `workhandover.workhandover.ApplySaleauctionDetail` 实体完整定义。
...
|------|-----|
| 显示名 | 废旧拍卖明细 |
| 物理表 | `cpu_workhandover_saleauction_detail` |
| 数据库 schema | `cpu-bi-service` |
| 所属应用 | `ycBase` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 15:19:20.3050` |
...
| 13 | `saleauction` | 废旧拍卖 | `saleauction` | 9f8b9356-4cd6-4ae7-8dc0-c84dfecdda07 | `quote` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `workHandOverApplyId` | 工作移交申请主键 | `workhandover.workhandover.Apply` | `apply_id` | saleauctionDetailList → workHandOverApplyId | 0..n | true | None |  |
...
| 5 | `saleauction` | 废旧拍卖 | `saleauction.project.AuctionProject` | `saleauction` |  →  |  |  | Service |  |

### Composition（子表）

- **workHandOverApplyId**: `saleauctionDetailList` → `workhandover.workhandover.Apply` | 0..n
...
| `saleauction` (废旧拍卖) | `saleauction.project.AuctionProject` | `saleauction` |
