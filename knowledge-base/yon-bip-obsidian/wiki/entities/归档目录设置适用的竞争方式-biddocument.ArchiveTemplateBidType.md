---
tags: ["BIP", "元数据", "数据字典", "ycBase", "bctask.biddocument.ArchiveTemplateBidType"]
created: 2026-06-04
updated: 2026-06-04
...
# 归档目录设置适用的竞争方式 (`bctask.biddocument.ArchiveTemplateBidType`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_archive_template_bid_type` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `bctask.biddocument.ArchiveTemplateBidType` 实体完整定义。
...
|------|-----|
| 显示名 | 归档目录设置适用的竞争方式 |
| 物理表 | `cpu_archive_template_bid_type` |
...
| 2 | `bidTypeCode` | 适用竞争方式 | `bid_type_code` | String | `text` |  | ✓ |
...
| 2 | `bidDocumentId` | 模板 | `bctask.biddocument.BidDocument` | `bid_document_id` | bidTypes → bidDocumentId | 0..n | true | None |  |

### Composition（子表）

- **bidDocumentId**: `bidTypes` → `bctask.biddocument.BidDocument` | 0..n
