---
tags: ["BIP", "元数据", "数据字典", "ycBase", "wx.workbeach.WxWorkbeachTodo"]
created: 2026-06-04
updated: 2026-06-04
...
# 微信工作台待办 (`wx.workbeach.WxWorkbeachTodo`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_wx_workbeach_todo` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `wx.workbeach.WxWorkbeachTodo` 实体完整定义。
...
|------|-----|
| 显示名 | 微信工作台待办 |
| 物理表 | `cpu_wx_workbeach_todo` |
| 数据库 schema | `yonbip-cpu-bctask` |
| 所属应用 | `ycBase` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 15:19:26.2790` |
...
| 5 | `todoId` | 待办 | `todo_id` | dae2efc6-7dd2-4407-bd49-5406eef84231 | `quote` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `workbeachId` | 工作台 | `wx.workbeach.WxWorkbeach` | `workbeach_id` | todos → workbeachId | 1..n | true | None |  |
| 2 | `ytenant` | 租户id | `yht.tenant.YhtTenant` | `ytenant_id` |  →  |  |  | Service |  |
| 3 | `todoId` | 待办 | `wx.workbeach.WxTodo` | `todo_id` |  →  |  |  | None |  |

### Composition（子表）

- **workbeachId**: `todos` → `wx.workbeach.WxWorkbeach` | 1..n
