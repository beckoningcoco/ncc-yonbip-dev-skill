---
tags: ["BIP", "元数据", "数据字典", "ycBase", "wx.workbeach.WxTodo"]
created: 2026-06-04
updated: 2026-06-04
...
# 微信工作台待办注册 (`wx.workbeach.WxTodo`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_wx_todo` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `wx.workbeach.WxTodo` 实体完整定义。
...
| 3 | `controlType` | 组件类型 | `control_type` | String | `text` |  | ✓ |
| 4 | `dataRule` | 数据规则 | `data_rule` | String | `text` |  | ✓ |
| 5 | `disablets` | 停用时间 | `disablets` | DateTime | `dateTime` |  | ✓ |
| 6 | `domain` | 微服务标识 | `domain` | String | `text` |  | ✓ |
| 7 | `dr` | 逻辑删除标记 | `dr` | Short | `short` |  | ✓ |
| 8 | `enable` | 启用 | `enable` | sys_intboolean | `singleOption` |  | ✓ |
| 9 | `enablets` | 启用时间 | `enablets` | DateTime | `dateTime` |  | ✓ |
| 10 | `icon` | 图标 | `icon` | String | `text` |  | ✓ |
| 11 | `id` | ID | `id` | Long | `long` |  | ✓ |
| 12 | `isSys` | 是否系统级 | `is_sys` | Short | `short` |  | ✓ |
| 13 | `microServiceCode` | 微服务编码 | `micro_service_code` | String | `text` |  | ✓ |
| 14 | `name` | 名称 | `name` | String | `text` |  | ✓ |
| 15 | `pubts` | 时间戳 | `pubts` | DateTime | `timestamp` |  | ✓ |
| 16 | `serviceCode` | 服务标识 | `service_code` | String | `text` |  | ✓ |
| 17 | `sysId` | 系统级标识 | `sys_id` | Long | `long` |  | ✓ |
| 18 | `worder` | 顺序号 | `worder` | Integer | `int` |  | ✓ |
| 19 | `wurl` | 跳转地址 | `wurl` | String | `text` |  | ✓ |
