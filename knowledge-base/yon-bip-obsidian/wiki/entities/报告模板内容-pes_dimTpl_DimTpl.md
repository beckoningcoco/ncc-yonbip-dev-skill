---
tags: ["BIP", "元数据", "数据字典", "pes.dimTpl.DimTpl"]
created: 2026-06-05
updated: 2026-06-05
sources: [元数据API queryByUri]
platform_version: "BIP V5"
project: ""
last_verified: 2026-06-05
status: verified
source_type: api_response
---

# 报告模板内容 (`pes.dimTpl.DimTpl`)

>  | 物理表：`pes_dim_tpl`

## 基本信息

| 属性 | 值 |
|------|-----|
| 显示名 | 报告模板内容 |
| 物理表 | `pes_dim_tpl` |
| 数据库 schema | `imppes` |
| 所属应用 | `` |
| 构建时间 | `2026-06-05 17:18:34.1800` |

## 主键与编码

| 角色 | 字段名 | 类型 | 说明 |
|------|--------|------|------|
| 主键 | `id` | Long | ID |

---

## 直接属性（20个）

| # | 字段名 | 显示名 | 数据库列 | 类型 |
|---|--------|--------|---------|------|
| 1 | `id` | ID | `id` | Long |
| 2 | `orgId` | 服务组织id | `org_id` | 14302233-1394-4a70-94e1-bed51636f312 |
| 3 | `code` | 编码 | `code` | String |
| 4 | `name` | 名称 | `name` | String |
| 5 | `memo` | 备注 | `memo` | String |
| 6 | `content` | 模板内容 | `content` | String |
| 7 | `createDate` | 创建日期 | `create_date` | Date |
| 8 | `createTime` | 创建时间 | `create_time` | DateTime |
| 9 | `creator` | 创建人名称 | `creator` | String |
| 10 | `creatorId` | 创建人 | `creatorId` | 54800425-15da-4742-ae89-059d05e77c9b |
| 11 | `modifier` | 修改人名称 | `modifier` | String |
| 12 | `modifierId` | 修改人 | `modifierId` | 54800425-15da-4742-ae89-059d05e77c9b |
| 13 | `modifyDate` | 修改日期 | `modify_date` | Date |
| 14 | `modifyTime` | 修改时间 | `modify_time` | DateTime |
| 15 | `picture` | 样例图 | `picture` | String |
| 16 | `pubts` | 时间戳 | `pubts` | DateTime |
| 17 | `stopstatus` | 停用状态 | `stopstatus` | Boolean |
| 18 | `stoptime` | 停用时间 | `stop_time` | DateTime |
| 19 | `tenant` | 租户ID | `tenant_id` | String |
| 20 | `ytenant` | 租户id | `ytenant_id` | e4933a03-9dea-472b-a644-cdd654222f45 |

---

## 关联属性（4个）

| # | 字段名 | 显示名 | 目标实体 | 隔离级 | 组合 |
|---|--------|--------|---------|--------|------|
| 1 | `ytenant` | 租户id | `yht.tenant.YhtTenant` | Service |  |
| 2 | `creatorId` | 创建人 | `base.user.User` | Service |  |
| 3 | `modifierId` | 修改人 | `base.user.User` | Service |  |
| 4 | `orgId` | 服务组织id | `org.func.BaseOrg` | Service |  |
