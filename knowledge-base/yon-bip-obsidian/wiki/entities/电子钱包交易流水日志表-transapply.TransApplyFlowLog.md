---
tags: ["BIP", "元数据", "数据字典", "ycBase", "ewallet.transapply.TransApplyFlowLog"]
created: 2026-06-04
updated: 2026-06-04
...
# 电子钱包交易流水日志表 (`ewallet.transapply.TransApplyFlowLog`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_ew_trans_apply_flow_log` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `ewallet.transapply.TransApplyFlowLog` 实体完整定义。
...
|------|-----|
| 显示名 | 电子钱包交易流水日志表 |
| 物理表 | `cpu_ew_trans_apply_flow_log` |
...
|---|--------|--------|---------|------|---------|------|------|
| 1 | `applyId` | 交易申请单 | `apply_id` | 3dcfb5cf-9b54-4cf0-9058-b8bb153a3c7d | `quote` |  | ✓ |
...
| 8 | `flowId` | 交易流水 | `flow_id` | 15da1204-a7c2-4fd1-8f12-9e1595c8d013 | `quote` |  | ✓ |
...
| 14 | `operateStatus` | 操作状态 | `operate_status` | String | `text` |  | ✓ |
| 15 | `operateType` | 操作类别 | `operate_type` | String | `text` |  | ✓ |
| 16 | `pubts` | 时间戳 | `pubts` | DateTime | `timestamp` |  | ✓ |
| 17 | `remark` | 备注信息 | `remark` | String | `text` |  | ✓ |
| 18 | `requestParam` | 请求参数 | `request_param` | String | `text` |  | ✓ |
| 19 | `requestUrl` | 请求地址 | `request_url` | String | `text` |  | ✓ |
| 20 | `responseResult` | 响应结果 | `response_result` | String | `text` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `applyId` | 交易申请单 | `ewallet.transapply.TransApply` | `apply_id` |  →  |  |  | None |  |
...
| 5 | `flowId` | 交易流水 | `ewallet.transapply.TransApplyFlow` | `flow_id` | flowLogs → flowId | 0..n | true | None |  |

### Composition（子表）

- **flowId**: `flowLogs` → `ewallet.transapply.TransApplyFlow` | 0..n
