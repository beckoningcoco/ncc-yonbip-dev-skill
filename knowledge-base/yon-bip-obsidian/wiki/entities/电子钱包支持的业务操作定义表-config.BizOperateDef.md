---
tags: ["BIP", "元数据", "数据字典", "ewallet.config.BizOperateDef"]
created: 2026-06-04
updated: 2026-06-04
...
# 电子钱包支持的业务操作定义表 (`ewallet.config.BizOperateDef`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP  模块实体元数据字典。
> 物理表：`cpu_ew_biz_operate_def` | 所属应用：``
> 来源：元数据API `queryByUri` 返回的 `ewallet.config.BizOperateDef` 实体完整定义。
...
|------|-----|
| 显示名 | 电子钱包支持的业务操作定义表 |
| 物理表 | `cpu_ew_biz_operate_def` |
| 数据库 schema | `yonbip-cpu-bcsourcing` |
| 所属应用 | `` |
| 业务对象ID | `e2a9e9ea-71b5-4179-847f-69fd5da2534d` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 14:26:12.2040` |
...
| 2 | `code` | 业务操作编码 | `code` | String | `text` |  | ✓ |
| 3 | `dr` | 逻辑删除标记 | `dr` | Short | `short` |  | ✓ |
| 4 | `id` | ID | `id` | Long | `long` |  | ✓ |
| 5 | `name` | 业务操作名称 | `name` | String | `multiLanguage` |  | ✓ |
