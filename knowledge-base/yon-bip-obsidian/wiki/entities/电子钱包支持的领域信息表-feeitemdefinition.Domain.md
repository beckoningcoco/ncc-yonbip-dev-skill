---
tags: ["BIP", "元数据", "数据字典", "ycBase", "ewallet.feeitemdefinition.Domain"]
created: 2026-06-04
updated: 2026-06-04
...
# 电子钱包支持的领域信息表 (`ewallet.feeitemdefinition.Domain`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_ew_domain` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `ewallet.feeitemdefinition.Domain` 实体完整定义。
...
| 所属应用 | `ycBase` |
| 业务对象ID | `8d3c0c04-dc4c-43fb-96b6-fdf495413ee2` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 14:26:09.0680` |
...
- **domainCode** (transformKey): 领域编码 — 唯一字段: `['domainCode']`
...
|---|--------|--------|---------|------|---------|------|------|
| 1 | `apps` | 电子钱包支持的业务模块信息表 | `` | 9570ff1b-f366-41e2-ab57-362405a1da6d | `` |  |  |
...
| 6 | `name` | 领域名称 | `name` | String | `multiLanguage` |  | ✓ |
...
| 2 | `apps` | 电子钱包支持的业务模块信息表 | `ewallet.feeitemdefinition.App` | `` | apps → domainId | 0..n | true | None |  |

### Composition（子表）

- **apps**: `apps` → `ewallet.feeitemdefinition.App` | 0..n
