---
tags: ["BIP", "元数据", "数据字典", "ycBase", "ewallet.wallet.WalletFeeItemCurrency"]
created: 2026-06-04
updated: 2026-06-04
...
# 电子钱包费用项支持的币种 (`ewallet.wallet.WalletFeeItemCurrency`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_ew_wallet_fee_item_currency` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `ewallet.wallet.WalletFeeItemCurrency` 实体完整定义。
...
|------|-----|
| 显示名 | 电子钱包费用项支持的币种 |
| 物理表 | `cpu_ew_wallet_fee_item_currency` |
| 数据库 schema | `yonbip-cpu-bcsourcing` |
| 所属应用 | `ycBase` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 15:30:12.8130` |
...
| 2 | `currencyId` | 币种 | `currency_id` | 02b45339-eb4a-4a31-a8b5-d32f494f4e8e | `quote` |  | ✓ |
| 3 | `dr` | 逻辑删除标记 | `dr` | Short | `short` |  | ✓ |
| 4 | `feeItemId` | 费用项 | `fee_item_id` | 324a6ef0-21df-432b-a753-7abac923e4d1 | `quote` |  | ✓ |
| 5 | `id` | ID | `id` | Long | `long` |  | ✓ |
| 6 | `pubts` | 时间戳 | `pubts` | DateTime | `timestamp` |  | ✓ |
| 7 | `walletId` | 电子钱包 | `wallet_id` | Long | `long` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `feeItemId` | 费用项 | `ewallet.wallet.WalletFeeItem` | `fee_item_id` | currencies → feeItemId | 1..n | true | None |  |
| 2 | `ytenant` | 租户id | `yht.tenant.YhtTenant` | `ytenant_id` |  →  |  |  | Service |  |
| 3 | `currencyId` | 币种 | `bd.currencytenant.CurrencyTenantVO` | `currency_id` |  →  |  |  | Service |  |

### Composition（子表）

- **feeItemId**: `currencies` → `ewallet.wallet.WalletFeeItem` | 1..n
