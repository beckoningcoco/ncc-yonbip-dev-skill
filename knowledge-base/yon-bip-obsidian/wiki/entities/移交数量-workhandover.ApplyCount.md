---
tags: ["BIP", "元数据", "数据字典", "ycBase", "workhandover.workhandover.ApplyCount"]
created: 2026-06-04
updated: 2026-06-04
...
# 移交数量 (`workhandover.workhandover.ApplyCount`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_workhandover_count` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `workhandover.workhandover.ApplyCount` 实体完整定义。
...
|---|--------|--------|---------|------|---------|------|------|
| 1 | `calcCount` | 计算移交总数量 | `calc_count` | Integer | `int` |  | ✓ |
...
| 7 | `handOverBillNo` | 移交业务编码 | `object_code` | String | `text` |  | ✓ |
| 8 | `handOverStatus` | 处理状态 | `handover_status` | String | `text` |  | ✓ |
...
| 14 | `objectInfo` | 移交业务 | `object_info_id` | 5eaadd16-e3ed-4f6a-b472-215c1639c797 | `quote` |  | ✓ |
| 15 | `pubts` | 时间戳 | `pubts` | DateTime | `timestamp` |  | ✓ |
| 16 | `realCount` | 实际移交总数量 | `real_count` | Integer | `int` |  | ✓ |
| 17 | `workHandOverApplyId` | 工作移交申请主键 | `apply_id` | 6b0dec6e-46be-451e-be36-cb6da4650912 | `quote` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `workHandOverApplyId` | 工作移交申请主键 | `workhandover.workhandover.Apply` | `apply_id` | countList → workHandOverApplyId | 0..n | true | None |  |
...
- **workHandOverApplyId**: `countList` → `workhandover.workhandover.Apply` | 0..n
