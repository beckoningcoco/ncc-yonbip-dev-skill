---
tags: ["BIP", "元数据", "数据字典", "ycBase", "workhandover.workhandover.ApplyBuyofferDetail"]
created: 2026-06-04
updated: 2026-06-04
...
# 移交询价明细 (`workhandover.workhandover.ApplyBuyofferDetail`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_workhandover_buyoffer_detail` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `workhandover.workhandover.ApplyBuyofferDetail` 实体完整定义。
...
|------|-----|
| 显示名 | 移交询价明细 |
| 物理表 | `cpu_workhandover_buyoffer_detail` |
...
|---|--------|--------|---------|------|---------|------|------|
| 1 | `buyoffer` | 询价 | `buyoffer` | 5232ab96-f504-4bac-95bc-c1cba8d49f63 | `quote` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `workHandOverApplyId` | 工作移交申请主键 | `workhandover.workhandover.Apply` | `apply_id` | buyofferDetailList → workHandOverApplyId | 0..n | true | None |  |
| 2 | `buyoffer` | 询价 | `sourcing.ibuyoffer.BuyOfferVO` | `buyoffer` |  →  |  |  | Service |  |
...
- **workHandOverApplyId**: `buyofferDetailList` → `workhandover.workhandover.Apply` | 0..n

### Service（外键引用）

| 字段 | 目标实体 | 数据库列 |
|------|---------|---------|
| `buyoffer` (询价) | `sourcing.ibuyoffer.BuyOfferVO` | `buyoffer` |
