---
tags: ["BIP", "元数据", "数据字典", "ycBase", "workhandover.workhandover.ApplyAuctionDetail"]
created: 2026-06-04
updated: 2026-06-04
...
# 竞拍项目明细 (`workhandover.workhandover.ApplyAuctionDetail`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_workhandover_auction_detail` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `workhandover.workhandover.ApplyAuctionDetail` 实体完整定义。
...
|------|-----|
| 显示名 | 竞拍项目明细 |
| 物理表 | `cpu_workhandover_auction_detail` |
...
|---|--------|--------|---------|------|---------|------|------|
| 1 | `auction` | 竞拍项目 | `auction` | 94b777ba-4855-4d81-a975-f6ab6b09f9a8 | `quote` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `workHandOverApplyId` | 工作移交申请主键 | `workhandover.workhandover.Apply` | `apply_id` | auctionDetailList → workHandOverApplyId | 0..n | true | None |  |
...
| 5 | `auction` | 竞拍项目 | `auction.project.AuctionProject` | `auction` |  →  |  |  | Service |  |

### Composition（子表）

- **workHandOverApplyId**: `auctionDetailList` → `workhandover.workhandover.Apply` | 0..n
...
| `auction` (竞拍项目) | `auction.project.AuctionProject` | `auction` |
