---
tags: ["BIP", "元数据", "数据字典", "ycBase", "workhandover.workhandover.ApplyClaimDeductionDetail"]
created: 2026-06-04
updated: 2026-06-04
...
# 索赔扣款明细 (`workhandover.workhandover.ApplyClaimDeductionDetail`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_workhandover_claim_deduction_detail` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `workhandover.workhandover.ApplyClaimDeductionDetail` 实体完整定义。
...
|------|-----|
| 显示名 | 索赔扣款明细 |
| 物理表 | `cpu_workhandover_claim_deduction_detail` |
| 数据库 schema | `cpu-bi-service` |
| 所属应用 | `ycBase` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 15:19:07.8670` |
...
|---|--------|--------|---------|------|---------|------|------|
| 1 | `claimDeduction` | 索赔扣款 | `claim_deduction` | a30f200b-df9f-4f6c-856e-387ba68836ca | `quote` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `workHandOverApplyId` | 工作移交申请主键 | `workhandover.workhandover.Apply` | `apply_id` | claimDeductionDetailList → workHandOverApplyId | 0..n | true | None |  |
| 2 | `ytenant` | 租户id | `yht.tenant.YhtTenant` | `ytenant_id` |  →  |  |  | Service |  |
| 3 | `claimDeduction` | 索赔扣款 | `qmpu.claim.QmpuClaimDeduction` | `claim_deduction` |  →  |  |  | Service |  |
| 4 | `creatorId` | 创建人 | `base.user.User` | `creatorId` |  →  |  |  | Service |  |
| 5 | `modifierId` | 修改人 | `base.user.User` | `modifierId` |  →  |  |  | Service |  |

### Composition（子表）

- **workHandOverApplyId**: `claimDeductionDetailList` → `workhandover.workhandover.Apply` | 0..n
...
| `claimDeduction` (索赔扣款) | `qmpu.claim.QmpuClaimDeduction` | `claim_deduction` |
