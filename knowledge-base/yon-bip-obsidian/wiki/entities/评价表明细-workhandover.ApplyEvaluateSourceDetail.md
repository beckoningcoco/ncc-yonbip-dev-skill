---
tags: ["BIP", "元数据", "数据字典", "ycBase", "workhandover.workhandover.ApplyEvaluateSourceDetail"]
created: 2026-06-04
updated: 2026-06-04
...
# 评价表明细 (`workhandover.workhandover.ApplyEvaluateSourceDetail`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_workhandover_evaluate_source_detail` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `workhandover.workhandover.ApplyEvaluateSourceDetail` 实体完整定义。
...
|------|-----|
| 显示名 | 评价表明细 |
| 物理表 | `cpu_workhandover_evaluate_source_detail` |
...
| 6 | `evaluateSource` | 评价表 | `evaluate_source` | 9a256643-ff1c-42e3-b6d6-fdd475838c09 | `quote` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `workHandOverApplyId` | 工作移交申请主键 | `workhandover.workhandover.Apply` | `apply_id` | evaluateSourceDetailList → workHandOverApplyId | 0..n | true | None |  |
| 2 | `ytenant` | 租户id | `yht.tenant.YhtTenant` | `ytenant_id` |  →  |  |  | Service |  |
| 3 | `evaluateSource` | 评价表 | `cpu-supplymgr.supplyevaluate.EvaluateScore` | `evaluate_source` |  →  |  |  | Service |  |
| 4 | `creatorId` | 创建人 | `base.user.User` | `creatorId` |  →  |  |  | Service |  |
| 5 | `modifierId` | 修改人 | `base.user.User` | `modifierId` |  →  |  |  | Service |  |

### Composition（子表）

- **workHandOverApplyId**: `evaluateSourceDetailList` → `workhandover.workhandover.Apply` | 0..n
...
| `evaluateSource` (评价表) | `cpu-supplymgr.supplyevaluate.EvaluateScore` | `evaluate_source` |
