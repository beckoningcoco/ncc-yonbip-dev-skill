---
tags: ["BIP", "元数据", "数据字典", "ycSupplierManagement", "cpu-supplymgr.applyset.QualifySetCase"]
created: 2026-06-04
updated: 2026-06-04
...
# 资质信息设置方案 (`cpu-supplymgr.applyset.QualifySetCase`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycSupplierManagement 模块实体元数据字典。
> 物理表：`cpu_qualify_set_case` | 所属应用：`ycSupplierManagement`
> 来源：元数据API `queryByUri` 返回的 `cpu-supplymgr.applyset.QualifySetCase` 实体完整定义。
...
|------|-----|
| 显示名 | 资质信息设置方案 |
| 物理表 | `cpu_qualify_set_case` |
| domain/服务域 | `cpu-supplymgr` |
| schema | `cpu_suppliermgr` |
| 所属应用 | `ycSupplierManagement` |
| 业务对象ID | `d1985725-7507-4a35-a2ac-d4cfb50aad85` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 16:35:40.9360` |
...
|---|--------|--------|---------|------|---------|------|------|
| 1 | `classType` | 主供分类类型 | `class_type` | String | `text` |  | ✓ |
| 2 | `code` | 方案编码 | `case_code` | String | `text` |  | ✓ |
| 3 | `configs` | 资质信息设置方案资质配置 | `` | 6fb39a3b-f7b4-40d1-ba5b-dddc9d369c91 | `` |  |  |
...
| 11 | `materialClass` | 资质信息设置方案主供分类 | `` | 01584c73-34be-460c-a738-d878af22c771 | `` |  |  |
| 12 | `modifier` | 修改人 | `modifier` | 98ac0ca3-2fd2-4a38-8a21-5d8243cddc8b | `quote` |  | ✓ |
| 13 | `modifyTime` | 修改时间 | `modify_time` | DateTime | `dateTime` |  | ✓ |
| 14 | `name` | 方案名称 | `case_name` | String | `text` |  | ✓ |
| 15 | `operatorName` | 操作人 | `operator_name` | String | `text` |  | ✓ |
| 16 | `organizationId` | 组织id | `organization_id` | 8aaf4bce-2eb3-443c-bce1-0fb94fec3c0c | `quote` |  | ✓ |
| 17 | `orgs` | 资质信息设置方案适用组织范围 | `` | 5fc5465f-8744-4fca-8057-9a0ae7752785 | `` |  |  |
...
| 2 | `configs` | 资质信息设置方案资质配置 | `cpu-supplymgr.applyset.QualifySetCaseConfig` | `` | configs → caseId | 0..n | true | None |  |
| 3 | `creator` | 创建人 | `base.user.BipUser` | `creator` |  →  |  |  | Service |  |
| 4 | `materialClass` | 资质信息设置方案主供分类 | `cpu-supplymgr.applyset.QualifySetCaseMClass` | `` | materialClass → caseId | 0..n | true | None |  |
| 5 | `ytenant` | 租户id | `yht.tenant.YhtTenant` | `ytenant_id` |  →  |  |  | Service |  |
| 6 | `modifier` | 修改人 | `base.user.BipUser` | `modifier` |  →  |  |  | Service |  |
| 7 | `deptId` | 部门id | `bd.adminOrg.BaseOrgVO` | `dept_id` |  →  |  |  | Service |  |
| 8 | `orgs` | 资质信息设置方案适用组织范围 | `cpu-supplymgr.applyset.QualifySetCaseOrganization` | `` | orgs → caseId | 0..n | true | None |  |

### Composition（子表）

- **configs**: `configs` → `cpu-supplymgr.applyset.QualifySetCaseConfig` | 0..n
- **materialClass**: `materialClass` → `cpu-supplymgr.applyset.QualifySetCaseMClass` | 0..n
- **orgs**: `orgs` → `cpu-supplymgr.applyset.QualifySetCaseOrganization` | 0..n
