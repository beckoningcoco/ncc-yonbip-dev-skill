---
tags: ["BIP", "元数据", "数据字典", "ycYuncaiMall", "mall.operatingprice.CpuOperatingPriceOrg"]
created: 2026-06-04
updated: 2026-06-04
...
# 运营价格协议适用组织范围 (`mall.operatingprice.CpuOperatingPriceOrg`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycYuncaiMall 模块实体元数据字典。
> 物理表：`cpu_operating_price_org` | 所属应用：`ycYuncaiMall`
> 来源：元数据API `queryByUri` 返回的 `mall.operatingprice.CpuOperatingPriceOrg` 实体完整定义。
...
|---|--------|--------|---------|------|---------|------|------|
| 1 | `orgName` | 组织ID | `org_name` | String | `text` |  | ✓ |
| 2 | `dr` | 逻辑删除标识 | `dr` | Boolean | `switch` |  | ✓ |
| 3 | `orgCode` | 组织编码 | `org_code` | String | `text` |  | ✓ |
| 4 | `ytenant` | 租户id | `ytenant_id` | e4933a03-9dea-472b-a644-cdd654222f45 | `quote` | ✓ | ✓ |
| 5 | `id` | ID | `id` | Long | `long` |  | ✓ |
| 6 | `priceId` | 协议主表ID | `price_id` | 54f73169-773d-40df-9fbc-c092f4201577 | `quote` |  | ✓ |
...
| 2 | `priceId` | 协议主表ID | `mall.operatingprice.CpuOperatingPrice` | `price_id` | orgs → priceId | 0..n | true | None |  |
| 3 | `orgId` | 组织ID | `org.func.BaseOrg` | `org_id` |  →  |  |  | Service |  |

### Composition（子表）

- **priceId**: `orgs` → `mall.operatingprice.CpuOperatingPrice` | 0..n
