---
tags: ["BIP", "元数据", "数据字典", "ycBase", "bctask.noticetemplate.NoticeTemplateApplyPurchaseType"]
created: 2026-06-04
updated: 2026-06-04
...
# 适用采购类型 (`bctask.noticetemplate.NoticeTemplateApplyPurchaseType`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_notice_template_apply_purchase_type` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `bctask.noticetemplate.NoticeTemplateApplyPurchaseType` 实体完整定义。
...
|------|-----|
| 显示名 | 适用采购类型 |
| 物理表 | `cpu_notice_template_apply_purchase_type` |
| 数据库 schema | `yonbip-cpu-bctask` |
| 所属应用 | `ycBase` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 15:15:47.8370` |
...
| 4 | `purchaseType` | 采购类型 | `purchase_type` | String | `text` |  | ✓ |
...
| 2 | `templateId` | 公告模板 | `bctask.noticetemplate.NoticeTemplate` | `template_id` | purchaseTypes → templateId | 0..n | true | None |  |

### Composition（子表）

- **templateId**: `purchaseTypes` → `bctask.noticetemplate.NoticeTemplate` | 0..n
