---
tags: ["BIP", "元数据", "数据字典", "ycPriceCenter", "cpubase.quota.QuotaAgreeTempChd"]
created: 2026-06-04
updated: 2026-06-04
...
# 配额协议模版自定义项 (`cpubase.quota.QuotaAgreeTempChd`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycPriceCenter 模块实体元数据字典。
> 物理表：`cpu_qtagree_temp_chd` | 所属应用：`ycPriceCenter`
> 来源：元数据API `queryByUri` 返回的 `cpubase.quota.QuotaAgreeTempChd` 实体完整定义。
...
|------|-----|
| 显示名 | 配额协议模版自定义项 |
| 物理表 | `cpu_qtagree_temp_chd` |
| 数据库 schema | `cpu-base` |
| 所属应用 | `ycPriceCenter` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 16:25:15.4880` |
