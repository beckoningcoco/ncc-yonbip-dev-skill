---
tags: ["BIP", "元数据", "数据字典", "ycBase", "category.level.ApplicationDetailDefineCharacter"]
created: 2026-06-04
updated: 2026-06-04
...
# 采购品类定级申请表体特征 (`category.level.ApplicationDetailDefineCharacter`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_category_level_application_detail_char` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `category.level.ApplicationDetailDefineCharacter` 实体完整定义。
...
|------|-----|
| 显示名 | 采购品类定级申请表体特征 |
| 物理表 | `cpu_category_level_application_detail_char` |
