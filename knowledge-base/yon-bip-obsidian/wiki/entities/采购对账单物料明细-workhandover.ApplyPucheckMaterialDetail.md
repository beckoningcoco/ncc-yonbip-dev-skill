---
tags: ["BIP", "元数据", "数据字典", "ycBase", "workhandover.workhandover.ApplyPucheckMaterialDetail"]
created: 2026-06-04
updated: 2026-06-04
...
# 采购对账单物料明细 (`workhandover.workhandover.ApplyPucheckMaterialDetail`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_workhandover_pucheckbill_material_detail` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `workhandover.workhandover.ApplyPucheckMaterialDetail` 实体完整定义。
...
|------|-----|
| 显示名 | 采购对账单物料明细 |
| 物理表 | `cpu_workhandover_pucheckbill_material_detail` |
| 数据库 schema | `cpu-bi-service` |
| 所属应用 | `ycBase` |
| 元数据类型 | `Class` |
| 元数据层级 | `0` |
| 是否业务对象 | `true` |
| 构建时间 | `2026-06-04 15:19:14.7900` |
...
| 13 | `pucheckMaterial` | 采购对账单物料 | `pucheckMaterial` | 542bed5a-b315-4d74-a415-9174370aed6c | `quote` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `workHandOverApplyId` | 工作移交申请主键 | `workhandover.workhandover.Apply` | `apply_id` | pucheckMaterialDetailList → workHandOverApplyId | 0..n | true | None |  |
| 2 | `pucheckMaterial` | 采购对账单物料 | `check.pucheckbill.PuCheckMaterialVO` | `pucheckMaterial` |  →  |  |  | Service |  |
...
- **workHandOverApplyId**: `pucheckMaterialDetailList` → `workhandover.workhandover.Apply` | 0..n

### Service（外键引用）

| 字段 | 目标实体 | 数据库列 |
|------|---------|---------|
| `pucheckMaterial` (采购对账单物料) | `check.pucheckbill.PuCheckMaterialVO` | `pucheckMaterial` |
