---
tags: ["BIP", "元数据", "数据字典", "ycBase", "workhandover.workhandover.ApplyLawbidDetail"]
created: 2026-06-04
updated: 2026-06-04
...
# 采购项目明细 (`workhandover.workhandover.ApplyLawbidDetail`)

> ⚡ **平台版本：BIP 旗舰版 V5** — 仅适用于用友 BIP 旗舰版（YonBIP），不适用于 NCC / NC Cloud 高级版。

> YonBIP ycBase 模块实体元数据字典。
> 物理表：`cpu_workhandover_lawbid_detail` | 所属应用：`ycBase`
> 来源：元数据API `queryByUri` 返回的 `workhandover.workhandover.ApplyLawbidDetail` 实体完整定义。
...
|------|-----|
| 显示名 | 采购项目明细 |
| 物理表 | `cpu_workhandover_lawbid_detail` |
...
| 8 | `lawbid` | 采购项目 | `lawbid` | 070c3520-bf5d-4b3e-a523-59e73d9f6349 | `quote` |  | ✓ |
...
|---|--------|--------|---------|---------|------|--------|------|--------|--------|
| 1 | `workHandOverApplyId` | 工作移交申请主键 | `workhandover.workhandover.Apply` | `apply_id` | lawbidDetailList → workHandOverApplyId | 0..n | true | None |  |
| 2 | `lawbid` | 采购项目 | `lawbid.project.LawbidProject` | `lawbid` |  →  |  |  | Service |  |
...
- **workHandOverApplyId**: `lawbidDetailList` → `workhandover.workhandover.Apply` | 0..n

### Service（外键引用）

| 字段 | 目标实体 | 数据库列 |
|------|---------|---------|
| `lawbid` (采购项目) | `lawbid.project.LawbidProject` | `lawbid` |
