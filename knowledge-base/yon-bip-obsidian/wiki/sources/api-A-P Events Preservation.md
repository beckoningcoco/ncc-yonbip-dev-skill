---
title: "应付事项保存"
apiId: "76691f297fd14b04979508dd8137355d"
apiPath: "请求方式	POST"
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| data | object | 是 | 是 | 应付事项信息 |
| id | string | 否 | 否 | 应付事项id, 新增时无需填写，修改时必填 |
| pubts | string | 否 | 否 | 时间戳,格式为:yyyy-MM-dd HH:mm:ss |
| code | string | 否 | 否 | 单据编号 |
| busiaccbook | string | 否 | 否 | 会计账簿id |
...
| billdirection | string | 否 | 否 | 单据方向, 1:红字、2:蓝字 |
| payprotocol_code | string | 否 | 否 | 付款协议编码 |
| basebilltype_code | string | 否 | 否 | 事项类型编码 |
| basebilltype | string | 否 | 否 | 事项类型id |
| payprotocol_name | string | 否 | 否 | 付款协议名称 |
| payprotocol | string | 否 | 否 | 付款协议id |
| basebilltype_name | string | 否 | 否 | 事项类型名称 |
| accountrule | string | 否 | 否 | 立账规则id |
...
| supplier_retailInvestors | boolean | 否 | 否 | 供应商是否散户, true:是、false:否 |
...
| exchRate | int | 否 | 是 | 汇率 |
| vendor_name | string | 否 | 否 | 供货供应商名称 |
| vendor | string | 否 | 否 | 供货供应商id |
...
| billtype | int | 否 | 否 | 应收事项类型, 1:销售发票、2:其它应收事项、3:销售发票(红字)、4:其它应收事项(红字)、5:订单日报、6:内部交易结算单、7:收款单、8:其它应付事项、9:客户退款单、10:付款单、11:供应商退款单、12:转账单、13:汇率损益单、14:外币兑换单、 |
| caobject | int | 否 | 否 | 收付款对象类型, 2:供应商 |
| oriSum | double | 否 | 否 | 金额 |
...
| invoiceno | string | 否 | 否 | 发票编号 |
| invoicedate | string | 否 | 否 | 发票日期, 格式为:yyyy-MM-dd |
| orderno | string | 否 | 否 | 订单编号 |
| description | string | 否 | 否 | 备注 |
| provisionalestimateflag | boolean | 否 | 否 | 是否暂估, true:是、false:否 |
...
| headItem | object | 是 | 否 | 表头固定自定项（1-60） |
| oapDetail | object | 是 | 否 | 详细信息 |
| contractno | string | 否 | 否 | 合同编码 |

## 3. 请求示例

Url: /yonbip/fi/oap/save?access_token=访问令牌
Body: {
	"data": [
		{
			"id": "",
...
| verifystate | long | 否 | 审批流状态,0:未审批、2：已审批 |
| supplier_retailInvestors | string | 否 | 供应商是否散户, true:是、false:否 |
...
| caobject | string | 否 | 收付款对象类型, 1:客户、2:供应商、3:人员、4:其他 |
| oriSum | int | 否 | 金额 |
| natSum | int | 否 | 本币金额 |
| balance | int | 否 | 余额 |
| localbalance | int | 否 | 本币余额 |
| invoicetype | string | 否 | 发票类型, 1:增值税电子普通发票、2:增值税普通发票、3:增值税专用发票、4:账单、6:机动车销售统一发票、5:收据、 |
| invoiceno | string | 否 | 发票编号 |
| invoicedate | string | 否 | 发票日期, 格式为:yyyy-MM-dd HH:mm:ss |
...
999	当前会计期间已经结账,单据不能维护	单据只能在结账前新增保存
