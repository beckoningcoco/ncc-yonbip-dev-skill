...
date: 2026-06-07
ingested: 2026-06-07
tags: [YonBIP, OpenAPI, A/P Settlement]
...
> `ContentType	application/json` 请求方式	POST | 分类: A/P Settlement (EFI)
...
请求地址	/yonbip/EFI/apverifyevent/list
...
| bodyItem.grandsonItem.srcGrandson.event.objectType | string | 是 | 否 | 往来对象类型（枚举值 0：供应商；2：员工；3：资金往来对象） 示例：["0", "2", "3"] |
| bodyItem.grandsonItem.srcGrandson.event.supplier | string | 是 | 否 | 供应商ID 示例：["1795155102808932354"] |
| bodyItem.grandsonItem.srcGrandson.event.funder | string | 是 | 否 | 资金业务对象ID 示例：["1878918415468462085"] |
| arapEventType | string | 是 | 否 | 核销事务（枚举值 231：应付核销；226：应付应收核销；221：应收应付核销） 示例：["231", "226", "221"] |
| batchno | string | 否 | 否 | 核销批次号 示例："1960584323790798855" |
| verifyType | string | 是 | 否 | 核销分类（枚举值 21：应付红蓝对冲；22：付款红蓝对冲；23：付款核销应付；24：付款退款核销应付（负）；25：付款退款核销付款；26：付款退款红蓝对冲；31：应付核销应收；32：收款核销付款；33：收款退款核销付款退款） 示例：["21", "22", "23", "24", "25", "31", "26", "32", "33"] |
...
| bodyItem.grandsonItem.srcGrandson.apSubject | string | 是 | 否 | 应付账款科目ID 示例：["1795059144642789385"] |
...
| bodyItem.grandsonItem.srcGrandson.event.supplier.code | string | 是 | 否 | 供应商编码 示例：["00000000000002"] |
| bodyItem.grandsonItem.srcGrandson.apSubject.code | string | 是 | 否 | 应付账款科目编码 示例：["1002"] |
| open_pubts_begin | string | 否 | 否 | 时间戳开始时间 示例：2023-04-13 14:59:24 |
| open_pubts_end | string | 否 | 否 | 时间戳结束时间 示例：2023-04-13 14:59:24 |

## 3. 请求示例

Url: /yonbip/EFI/apverifyevent/list?access_token=访问令牌
Body: {
	"pageSize": 10,
	"pageIndex": 1,
	...
	"bodyItem.grandsonItem.srcGrandson.event.supplier": [
		"1795155102808932354"
	],
	...
	"bodyItem.grandsonItem.srcGrandson.apSubject": [
		"1795059144642789385"
	],
	...
	"simple": {
		...
		"bodyItem.grandsonItem.srcGrandson.event.supplier.code": [
			"00000000000002"
		],
		"bodyItem.grandsonItem.srcGrandson.apSubject.code": [
			"1002"
		],
		...
		"recordList": [
			{
				...
				"bodyItem_grandsonItem_srcGrandson_event_code": "AP08230421000502",
				"creator_userName": "",
				"bodyItem_grandsonItem_srcGrandson_event_srcBillCode": "SREFap230421009407",
