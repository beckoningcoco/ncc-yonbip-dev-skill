---
title: "会计档案-单据数据-新增V1"
apiId: "2362644824526946306"
apiPath: "请求方式	POST"
...
请求地址	/yonbip/FCC/v1/api/collection/account/voucher/bill
...
| categoryPrefix | string | 否 | 是 | 分类标识 示例：PZ 默认值：PZ |
| accountYear | string | 否 | 是 | 年度 示例：2023 |
| accountMonth | string | 否 | 是 | 月份 示例：03 |
| upperBillType | string | 否 | 是 | 上游单据类型 示例：PZ |
| upperBillPk | string | 否 | 是 | 上游单据识别码 示例：B001 |
| billRecordList | object | 是 | 是 | 单据记录对象 |
| title | string | 否 | 是 | 题名 示例：发票 |
...
| billType | string | 否 | 否 | 单据类型 示例：invoice |
| billPk | string | 否 | 否 | 单据识别码 示例：B002 |
...
| receiptsFlag | string | 否 | 否 | 是否是单据 1:单据 2:发票附件 3:银行回单附件 4:普通附件 示例：4 |
...
Url: /yonbip/FCC/v1/api/collection/account/voucher/bill?access_token=访问令牌
Body: {
	"orgCode": "ORG001",
	"categoryPrefix": "PZ",
	"accountYear": "2023",
