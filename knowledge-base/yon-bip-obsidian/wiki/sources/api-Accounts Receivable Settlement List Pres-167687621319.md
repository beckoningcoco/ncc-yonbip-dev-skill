...
请求地址	/yonbip/EFI/receivable/save
请求方式	POST
...
creator	string	否	否	创建人 示例：9dff758d-bfc2-4b90
...
code	string	否	否	单据编号 (设置自动编码时按编码规则取值，接口不用传值；手工编码时需要传值；更新数据时需要传值) 示例：OARar240730001257
extVouchCode	string	否	否	外部系统单号（与外部系统编码联合做幂等性校验，有外部系统单号，没有外部系统编码时，将不进行校验） 示例：AB1234
id	string	否	否	应收发票主表id，新增时不填，修改时必填 示例：2056237265560535041
...
Url: /yonbip/EFI/receivable/save?access_token=访问令牌
Body: {
	"data": {
		"resubmitCheckKey": "",
		...
		"invoiceNo": "20240806",
		"invoiceType": "1",
		"code": "OARar240730001257",
		"extVouchCode": "AB1234",
		"id": "2056237265560535041",
		...
		"bodyItem": [
			{
				...
				"customer": "1515940421356224512",
				"customerCode": "0805",
				"employee": "18609880694579527",
				"employeeCode": "9dff758d-bfc2-4b90",
				"funder": "1732613498433699840",
				"funder_code": "ZJDX230524000051",
...
data	object	否	返回应收发票数据
busiObj	string	否	业务对象编码 示例：receivable
oriCurrency	string	否	币种id 示例：1514492081064640537
financeOrg	string	否	开票组织id 示例：1544896420846764040
extSystemCode	string	否	外部系统编码 示例：AB1234
objectType	string	否	往来对象类型(枚举值 1:客户; 2:员工;3:资金业务对象) 示例：0
...
localTaxIncludedAmount	BigDecimal	否	本币含税金额 示例：307
...
blnWriteOff	boolean	否	是否冲销单(0:否; 1:是) 示例：false
...
cBillNo	string	否	单据编码 示例：receivable
...
oriTaxIncludedAmount	BigDecimal	否	含税金额 示例：30
...
businessType	string	否	业务类型(枚举值 0:应收发票; 1:收款退款单; 2:收款退款; 3:应付发票; 4:收款退款单; 5:付款退款) 示例：0
