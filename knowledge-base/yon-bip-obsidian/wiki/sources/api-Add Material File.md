...
请求地址	/yonbip/digitalModel/product/add
...
| code | string | 否 | 是 | 物料编码，编码不存在时为新增操作 示例：物料编码 |
...
| inspectionUnit | long | 否 | 否 | 检验单位id(id和编码二选一必填，同时填入时以code为准) 示例：2006391747603 |
| inspectionUnitCode | string | 否 | 否 | 检验单位编码(id和编码二选一必填，同时填入时以code为准) 示例：检验单位编码 |
| purchasePriceUnit | long | 否 | 否 | 采购计价单位id(id和编码二选一必填，同时填入时以code为准) 示例：2006391747604 |
| purchasePriceUnitCode | string | 否 | 否 | 采购计价单位编码(id和编码二选一必填，同时填入时以code为准) 示例：采购计价单位编码 |
| stockUnit | long | 否 | 否 | 库存单位id(id和编码二选一必填，同时填入时以code为准) 示例：2006391747605 |
| stockUnitCode | string | 否 | 否 | 库存单位编码(id和编码二选一必填，同时填入时以code为准) 示例：库存单位编码 |
| produceUnit | long | 否 | 否 | 生产单位id(id和编码二选一必填，同时填入时以code为准) 示例：2006391747606 |
| produceUnitCode | string | 否 | 否 | 生产单位编码(id和编码二选一必填，同时填入时以code为准) 示例：生产单位编码 |
| batchPriceUnit | long | 否 | 否 | 批发计价单位id(id和编码二选一必填，同时填入时以code为准) 示例：2006391747607 |
| batchPriceUnitCode | string | 否 | 否 | 批发计价单位编码(id和编码二选一必填，同时填入时以code为准) 示例：批发计价单位编码 |
| batchUnit | long | 否 | 否 | 批发销售单位id(id和编码二选一必填，同时填入时以code为准) 示例：2006391747608 |
| batchUnitCode | string | 否 | 否 | 批发销售单位编码(id和编码二选一必填，同时填入时以code为准) 示例：批发销售单位编码 |
| onlineUnit | long | 否 | 否 | 线上零售单位id(id和编码二选一必填，同时填入时以code为准) 示例：2006391747609 |
| onlineUnitCode | string | 否 | 否 | 线上零售单位编码(id和编码二选一必填，同时填入时以code为准) 示例：线上零售单位编码 |
| offlineUnit | long | 否 | 否 | 线下零售单位id(id和编码二选一必填，同时填入时以code为准) 示例：2006391747610 |
| offlineUnitCode | string | 否 | 否 | 线下零售单位编码(id和编码二选一必填，同时填入时以code为准) 示例：线下零售单位编码 |
| requireUnit | long | 否 | 否 | 要货单位id(id和编码二选一必填，同时填入时以code为准) 示例：2006391747611 |
...
| singleInspection | int | 否 | 否 | 单件检验（1表示开启单件检验，0表示不开启） 示例：1 |
...
| status | string | 否 | 否 | status操作状态：新增：Insert 示例：Insert 默认值：Insert |
...
Url: /yonbip/digitalModel/product/add?access_token=访问令牌
Body: {
	"orgCode": "管理组织编码",
	"orgId": "2006391747600",
	"code": "物料编码",
	...
	"detail": {
		...
		"inspectionUnit": 2006391747603,
		"inspectionUnitCode": "检验单位编码",
		"purchasePriceUnit": 2006391747604,
		"purchasePriceUnitCode": "采购计价单位编码",
		"stockUnit": 2006391747605,
		"stockUnitCode": "库存单位编码",
		"produceUnit": 2006391747606,
		"produceUnitCode": "生产单位编码",
		"batchPriceUnit": 2006391747607,
		"batchPriceUnitCode": "批发计价单位编码",
		"batchUnit": 2006391747608,
		"batchUnitCode": "批发销售单位编码",
		"onlineUnit": 2006391747609,
		"onlineUnitCode": "线上零售单位编码",
		"offlineUnit": 2006391747610,
		"offlineUnitCode": "线下零售单位编码",
		"requireUnit": 2006391747611,
