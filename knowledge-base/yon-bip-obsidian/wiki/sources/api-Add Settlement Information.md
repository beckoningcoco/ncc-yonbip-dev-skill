...
请求地址	/yonbip/AMP/fiercl/rclSettleInfo/add
...
Url: /yonbip/AMP/fiercl/rclSettleInfo/add?access_token=访问令牌
Body: {
	"data": {
		...
		"rclSettleInfoBodyList": [
			{
				...
				"localPrice": "1",
				"settleStartPeriod": "1556256609240350727",
				"settleStartPeriod_code": "1980-01",
				"settleEndPeriod": "1556256609240350727",
...
| data | object | 否 | 返回值 |
| busiObj | string | 否 | 业务对象 示例：rclSettleInfo |
| applyQuantityTotal | BigDecimal | 否 | 累计申请数量 示例：10 |
| accentity | string | 否 | 会计主体 示例：1525652227072458756 |
| dr | short | 否 | 逻辑删除(0-正常;1-已删除) 示例：0 |
| exchangeRate | BigDecimal | 否 | 汇率 示例：1 |
| id | string | 否 | 主表id 示例：1688132214264954887 |
| metaFullName | string | 否 | 元数据全名称 示例：RCL.RCL.RclSettleInfoHeader |
| billDate | string | 否 | 单据日期 示例：2023-03-25 |
| ytenantId | string | 否 | 租户id 示例：0000L6YQ8AVLFUZPXD0000 |
| _status | short | 否 | 状态 示例：0 |
| verifyState | short | 否 | 单据状态 示例：0 |
| code | string | 否 | 编码 示例：SEI20230325001122 |
| cBillNo | string | 否 | 页面编码 示例：rclSettleInfo |
| applyAmountTotal | BigDecimal | 否 | 累计申请金额 示例：100 |
| pubts | string | 否 | 时间戳 示例：2023-03-25 14:07:35 |
| creator | string | 否 | 创建人 示例：99ea7655-00a2-4bda-b23c-19ade37ea574 |
| postState | string | 否 | 过账状态(1-未过账;2-不过账;3-过账中;4-过账成功;5-过账失败) 示例：1 |
| oriCurrtype | string | 否 | 币种id 示例：1525643705855773169 |
| bustype | string | 否 | 交易类型 示例：1658667158153986049 |
| exchangeRateType | string | 否 | 汇率类型 示例：1679114604013158401 |
| exchangeRateDate | string | 否 | 汇率日期 示例：2023-03-22 |
| createTime | string | 否 | 创建时间 示例：2023-03-25 13:56:42 |
| remarks | string | 否 | 备注 示例：备注 |
| customer | string | 否 | 客户 示例：1526228774896533510 |
| contractConversionParams | string | 否 | 折算参数(1-按原币折算;3-按本币折算) 示例：1 |
| rclSettleInfoBodyList | object | 是 | 结算信息子表 |
