...
请求地址	/yonbip/RDC/openapi/prjTask/v1/saveOrUpdate
请求方式	POST
...
Body参数
名称	类型	数组	必填	描述
parent	string	否	否	父任务ID 示例：2459282390857547796
ownedorg	string	否	否	所属组织ID 示例：1980670408412626952
submitPerson	string	否	否	提交人ID 示例：1981140234900340744
path	string	否	否	路径 示例：2459282390857547796|707956777885|
planStarttime	string	否	否	计划开始时间 示例：2026-01-28
id	number
小数位数:0,最大长度:12	否	否	ID 示例：707956777885
ownedorg_name	string	否	否	所属组织 示例：智石开
specialTask	string	否	否	特殊任务 示例：false
inputMaterialExpRecipeList	object	是	否	实验配方输入物
expRecipe_owner	string	否	否	所有者ID 示例：1980670880874233864
expRecipe_createOrg	string	否	否	创建组织ID 示例：1980670408412626952
expRecipe_createOrg_name	string	否	否	创建组织 示例：智石开
expRecipe_experimentRecipeType	string	否	否	类型ID 示例：2157049236217659399
expRecipe_owner_name	string	否	否	所有者 示例：ycc
expRecipe_modifier	string	否	否	修改人 示例：bbbccde0-06a4-4af3-b7f0-eb5a44cdbf69
source	string	否	否	来源 示例：1
expRecipe_code	string	否	否	编码 示例：202412160003
expRecipe_name	string	否	否	名称 示例：yc实验配方001
id	string	否	否	ID 示例：2500079692488900613
task	number
小数位数:0,最大长度:12	否	否	任务ID 示例：707956777885
expRecipe	string	否	否	实验配方ID 示例：2157087787816321031
expRecipe_ownedDept_name	string	否	否	所属部门 示例：测试部门
_status	string	否	否	状态,新增：Insert; 更新：Update；删除：Delete 示例：Insert
typeId_code	string	否	否	任务类别编码 示例：1001
principalDept_name	string	否	否	责任人部门 示例：测试部门
lifecycleAttrpoolId_code	string	否	否	生命周期状态池编码 示例：Executing
secretLevel	string	否	否	密级ID 示例：2303450034061967361
classifyId_name	string	否	否	任务状态 示例：执行中
principalRole	string	否	否	责任人角色 示例：1992435990284730377
typeId	string	否	否	任务类别ID 示例：1980667178594598921
lifecycleUseId	string	否	否	生命周期应用模板ID 示例：1980667178617667749
lifecycleAttrId	string	否	否	生命周期模板ID 示例：1980667178617667703
verifystate	number
小数位数:0,最大长度:10	否	否	审批状态 示例：2
lifecycleAttrpoolId_name	string	否	否	生命周期状态池 示例：执行中
typeId_name	string	否	否	任务类别 示例：通用类型(内置)
classifyId_code	string	否	否	任务状态编码 示例：PROGRESS
lifecycleId_name	string	否	否	生命周期模板 示例：项目任务-默认模板
hasOutputPackage	string	否	否	是否有输出包 示例：Y
secretLevel_secretCode	string	否	否	密级编码 示例：GK
planType	string	否	否	计划类型ID 示例：2459282390857547792
grpMember	string	否	否	项目组成员ID 示例：2482107144133410824
prjTaskFormList	object	是	否	流程单据
taskId_name	string	否	否	任务名称 示例：DDD
delType_cardKey	string	否	否	卡片页单据号 示例：Process
code	string	否	否	编码 示例：ycc22025091200002
appDept	string	否	否	申请部门ID
ownedorg	string	否	否	所属部门 示例：智石开
delType_deliverablePubRef	string	否	否	交付物参照 示例：zskpd.RefTable_3180091130
delType	string	否	否	交付物类型ID 示例：2435334787087990786
billState	string	否	否	单据状态 示例：0
billDate	string	否	否	单据日期 示例：2025-09-12 16:10:00
billNum	string	否	否	单据号 示例：Process
delType_serviceCode	string	否	否	服务编码 示例：plm_pbm_tech_pss
applicant	string	否	否	申请人 示例：ycc
delType_fullname	string	否	否	交付物fullname 示例：ZSKPD.ZSKPD.Process
delType_systemType	string	否	否	系统类别 示例：external
delType_serviceDomain	string	否	否	服务域 示例：zskpd
name	string	否	否	名称 示例：行行行
delType_name	string	否	否	交付物类型 示例：工艺路线
formDataId	string	否	否	交付物ID 示例：2357632962080014344
id	string	否	否	ID 示例：2500079692488900615
taskId	number
小数位数:0,最大长度:12	否	否	任务ID 示例：707956777885
_status	string	否	否	状态,新增：Insert; 更新：Update；删除：Delete 示例：Insert
principalYht	string	否	否	任务负责人友互通 示例：bbdbaa4c-3d3a-4cc0-a806-81c8415c94f2
principalDept	string	否	否	责任人部门ID 示例：1980670605981122561
wfId	string	否	否	审批流水 示例：1980667178617667728
projectId_code	string	否	否	项目编码 示例：20260127000001
createorg_name	string	否	否	创建组织 示例：智石开
WBSEdit	boolean	否	否	WBS编辑权限 示例：true
classifyId	string	否	否	任务状态ID 示例：1980667178594598916
planEndtime	string	否	否	计划结束时间 示例：2026-02-10
Terminate	boolean	否	否	终止权限 示例：true
lifecycleAttrpoolId	string	否	否	生命周期状态池ID 示例：1980667178617667683
realStarttime	string	否	否	实际开始时间 示例：2026-03-23
projectId_name	string	否	否	项目名称 示例：gggggggggg
lifecycleUseId_name	string	否	否	生命周期应用模板 示例：项目任务-默认模板使用
typeId_type	string	否	否	任务类别 示例：1001
publishDept	string	否	否	发布部门ID 示例：1980670605981122561
owner_name	string	否	否	所有者 示例：黎二
parent_id	string	否	否	父任务ID 示例：2459282390857547796
name	string	否	否	名称 示例：DDD
publisher	string	否	否	发布人ID 示例：1981140234900340744
creationtime	string	否	否	创建时间 示例：2026-01-27 15:56:36
wfstate	number
小数位数:0,最大长度:10	否	否	流程标志 示例：0
projectId	string	否	否	项目ID 示例：2459282390857547785
createorg	string	否	否	创建组织ID 示例：1980670408412626952
code	string	否	否	编码 示例：task56655889
projectId_classifyId	string	否	否	项目类型ID 示例：1992351860465336328
principalRole_name	string	否	否	责任人角色 示例：导入9
principal	string	否	否	责任人ID 示例：1981140234900340744
taskType	string	否	否	任务类型信息 示例：TASK
lifecycleAttrId_name	string	否	否	生命周期状态 示例：执行中
grpMember_name	string	否	否	项目组成员 示例：黎一
prjTaskDesignDocLinkList	object	是	否	设计规范
taskId_name	string	否	否	任务名称 示例：DDD
docRevision	string	否	否	文档版本 示例：A/1
docId	string	否	否	文档ID 示例：2344197668705140738
docCode	string	否	否	文档编码 示例：0013
projectId_name	string	否	否	项目名称 示例：gggggggggg
id	string	否	否	ID 示例：2500079692488900614
docId_secretLevel_secretName	string	否	否	文档密级 示例：公开
projectId	string	否	否	项目ID 示例：2459282390857547785
docId_name	string	否	否	文档名称 示例：afterSaveTest
docId_secretLevel	string	否	否	文档密级ID 示例：2303450034061967361
taskId	number
小数位数:0,最大长度:12	否	否	任务ID 示例：707956777885
docVersion	string	否	否	文档版次 示例：1
_status	string	否	否	状态,新增：Insert; 更新：Update；删除：Delete 示例：Insert
owner	string	否	否	所有者ID 示例：1985904049416306698
severity	string	否	否	严重程度 示例：GENERAL_TASK
collabPrepareState	number
小数位数:0,最大长度:10	否	否	协同编制状态 示例：2
lifecycleId	string	否	否	生命周期模板ID 示例：1980667178617667691
startState	string	否	否	启动状态 示例：0
_status	string	否	否	状态,新增：Insert; 更新：Update 示例：Update

## 3. 请求示例

Url: /yonbip/RDC/openapi/prjTask/v1/saveOrUpdate?access_token=访问令牌
Body: {
