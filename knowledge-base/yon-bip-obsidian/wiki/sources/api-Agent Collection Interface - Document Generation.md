...
date: 2026-06-07
ingested: 2026-06-07
tags: [YonBIP, OpenAPI, Agent Collection]
...
> `ContentType	application/json` 请求方式	POST | 分类: Agent Collection (ctm)
...
请求地址	/yonbip/FCC/fdtr/agentcollection/openapi/save
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| agentCollections | object | 是 | 否 | 代理收款单对象列表 |
...
| tradeType | string | 否 | 是 | 交易类型，支持编码和名称 示例：代理收款：stct_batch_agentcollection，现金存入：stct_batch_agentcollection_cashDeposite |
| remark | string | 否 | 否 | 备注 |
| sourceId | string | 否 | 否 | 上游业务单据主表id 示例：2414626636856033285 |
| details | object | 是 | 否 | 代理收款单明细 |

## 3. 请求示例

Url: /yonbip/FCC/fdtr/agentcollection/openapi/save?access_token=访问令牌
Body: {
	"agentCollections": [
		{
			"sourceSystem": "3",
			...
			"verifyState": NaN,
			"oriCurrency": "人民币或CNY",
			"tradeType": "代理收款：stct_batch_agentcollection，现金存入：stct_batch_agentcollection_cashDeposite",
			"remark": "",
