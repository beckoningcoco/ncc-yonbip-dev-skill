...
请求地址	/yonbip/scm/arrivalorder/confirm
...
| signForCompletionStatus | boolean | 否 | 是 | 签收完成状态。true：单据协同状态变成已签收，单据状态变成已审核；false：单据状态为开立态，单据协同状态为部分签收。若不传默认为true。优先取主表的签收完成状态。若主表签收完成状态=true，则所有子表签收完成状态设置为true；若主表签收完成状态为false，则子表的签收完成状态按照子表参数(签收完成状态)进行赋值。 示例：true |
| headParallel!signUserId | long | 否 | 否 | 签收人id 示例：3423423423 |
| headParallel!signUser | string | 否 | 否 | 签收人姓名 示例：张三 |
| arrivalOrders | object | 是 | 是 | 到货单子表数据 |

## 3. 请求示例

Url: /yonbip/scm/arrivalorder/confirm?access_token=访问令牌
Body: {
	"data": {
		"id": 1598979980472614913,
		"signForCompletionStatus": true,
		"headParallel!signUserId": 3423423423,
		"headParallel!signUser": "张三",
		"arrivalOrders": [
			{
...
| isWfControlled | boolean | 否 | 是否审批流控制，true表示开启审批流程，false表示不开启 示例：false |
| isContract | boolean | 否 | 是否需要与供应商协同，true表示需要与供应商协同，false表示不需要与供应商协同 示例：false |
...
| pubts | Date | 否 | 时间戳 示例：2022-12-01 09:57:42 |
| headParallel | object | 否 | 主表平行表数据 |
...

{
	...
	"data": {
		...
		"pubts": "2022-12-01 09:57:42",
		"headParallel": {
			"status": 1,
			"yuncaiStatus": "confirm",
