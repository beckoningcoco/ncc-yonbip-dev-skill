...
请求地址	/yonbip/AMP/fipub/basedoc/querybd/basedocmapping
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| fields | string | 是 | 否 | 需要查询的数据列，默认为id、code、name三列（取值枚举 id：唯一标识，code：基础档案对照编码，name：基础档案对照名称，ts：创建时间） 示例：["id","code","name"] |
| pageIndex | long | 否 | 否 | 查询页码，默认为1 示例：1 |
| pageSize | long | 否 | 否 | 查询数量，默认1000 示例：3 |
| conditions | object | 是 | 否 | 查询条件数组 |
| field | string | 否 | 否 | 查询条件中的字段名称（取值枚举 id：唯一标识，code：基础档案对照编码，name：基础档案对照名称，ts：创建时间） 示例：ts |
| value | string | 否 | 否 | 查询条件中字段的值 示例：2019-10-23 14:00:37 |
| operator | string | 否 | 否 | 查询条件中字段的比对方式，默认是“=”（取值 "<"，"="，">"，"<="，">="，"<>"） 示例：>= |

## 3. 请求示例

Url: /yonbip/AMP/fipub/basedoc/querybd/basedocmapping?access_token=访问令牌
Body: {
	"fields": [
		"id",
		"code",
...
| id | string | 否 | 基础档案对照ID 示例：00D6AE84-05E3-40DF-BD2C-690381CD8C85 |
| code | string | 否 | 基础档案对照code 示例：0506 |
| name | string | 否 | 基础档案对照名称 示例：0506肯德基国际准则账簿 |
| ts | string | 否 | 基础档案对照创建时间 示例：2020-10-23 14:00:37 |
