---
title: "员工信息新增修改_全球化_上传下发"
apiId: "2144567623078117385"
apiPath: "请求方式	POST"
method: "ContentType	application/json"
category: "Employee Information"
domain: "CHR"
date: 2026-06-07
ingested: 2026-06-07
tags: [YonBIP, OpenAPI, Employee Information]
platform_version: "BIP"
source_type: community-api-docs
---

# 员工信息新增修改_全球化_上传下发

>  请求方式	POST | Employee Information (CHR)


## 1. 请求说明

请求域名	开放API: 动态域名，获取方式详见 获取租户所在数据中心域名
集成API: 详细域名信息，请见 连接配置
请求地址	/yonbip/hrcloud/byd/save
请求方式	POST
ContentType	application/json
应用场景	开放API/集成API
API类别	批量保存/更新
事务和幂等性	无
用户身份	支持传递普通用户身份，详细说明见开放平台用户认证接入规范
来源	系统级

## 2. 请求参数

名称	类型	参数位置	必填	描述
access_token	string	query	是	调用方应用token
Body参数
名称	类型	数组	必填	描述
data	object	是	是	数据
code	string	否	否	人员编码 当编码规则设置手工编号时必填 示例：2511871812849920
name	string	否	是	员工姓名 示例：测试1234
certType	string	否	是	证件类型 传入id或code 示例：0010-3600-41e2-8568-42fbc6c96c9d
certNo	string	否	是	证件号 示例：0909765437777
sex	string	否	否	性别；1：男；2：女；0：未知 示例：2
birthDate	string	否	否	出生日期 示例：2013-11-09
mobile	string	否	否	手机号 如需生成用户，请开启公共参数，并与邮箱二者填其一 示例：+86-13842985777
dataCreateSource	string	否	否	数据来源，仅新增赋值，不可更新
dataCreateBillid	string	否	否	数据来源单据号，仅新增赋值，不可更新
officeTel	string	否	否	办公电话 示例：1546816777
email	string	否	否	邮箱 如需生成用户，请开启公共参数，并与邮箱二者填其一 示例：321321@777.com
selfEmail	string	否	否	个人邮箱 示例：7777@qq.com
joinWorkDate	string	否	否	参加工作日期 示例：2020-11-01
linkAddr	string	否	否	联系地址 示例：沈阳
education	string	否	否	学历 传入id或code 示例：2476728905519360
degree	string	否	否	学位 传入id或code 示例：2476730873303296
marital	string	否	否	婚姻状况 传入id或code 示例：2475282874225665
bloodType	string	否	否	血型 传入id或code 示例：41eb36a1-4f93-4504-b4ae-91837e5b1a7e
characterrpr	string	否	否	户口性质 传入id或code 示例：5db087f8-8dbf-4426-9b61-9c3886f812a3
permanreside	string	否	否	户口所在地 传入id或code 示例：bfbd1620-5cf3-11e9-817e-7cd30abea0c0
political	string	否	否	政治面貌 传入id或code 示例：2508983411364096
joinPolityDate	string	否	否	加入党团时间 示例：2017-11-07
nationality	string	否	否	民族 传入id或code 示例：694d8570-3da8-4162-b521-1d0dfa2a13ea
country	string	否	否	国籍地区 传入id或code 示例：6cefdb2b-180f-4ba5-8710-fa5a27a108bb
origin	string	否	否	籍贯 传入id或code 示例：c04c67b2-5cf3-11e9-817e-7cd30abea0c0
qq	string	否	否	QQ 示例：7787845448
linkedIn	string	否	否	领英 示例：0215141185
formerName	string	否	否	曾用名 示例：测试5678
englishName	string	否	否	英文名 示例：test1234
documentValidity	string	否	否	证件有效期 示例：2014-03-23
height	string	否	否	身高 示例：173
weight	string	否	否	体重 示例：70
postcode	string	否	否	邮政编码 示例：431728
birthPlace	string	否	否	出生地 传入id或code 示例：36276271113322
religion	string	否	否	宗教信仰 传入id或code 示例：1493439902010001
hobby	string	否	否	特长爱好
bizManTag	string	否	否	业务员，1：是；0：否 示例：false
shopAssisTag	string	否	否	是否店员，1：是；0：否 示例：true
attachlist	string	否	否	附件 通过协同云的文件上传api，上传本地文件，绑定id，保存信息时附件设置为该id实现附件上传 示例：3f9ebc70-4058-11ec-863d-bb0606e88ccf
weiXin	string	否	否	微信
middleName	string	否	否	中间名
firstName	string	否	否	姓
creator	string	否	否	创建人
shortName	string	否	否	姓名简拼
modifier	string	否	否	修改人
modifyTime	string	否	否	修改时间
createTime	string	否	否	创建时间
tenant	string	否	否	租户id
yTenantId	string	否	否	y租户id
majorcorpid	string	否	否	合同主体单位
workAge	string	否	否	工龄
secretId	string	否	否	员工密级
unitId	string	否	否	业务单元
collectionstatus	string	否	否	采集状态
rejectiontimes	string	否	否	驳回次数
rejectionreason	string	否	否	最新驳回原因
attachIdList	string	否	否	附件ID串
postId	string	否	否	岗位
objid	string	否	否	nc主键
cadresShoworder	string	否	否	干部人员排序
iscadre	string	否	否	是否是干部
candidateFlag	string	否	否	候选人标识
cadreType	string	否	否	干部分类
manageOrg	string	否	否	管理单位
extension	string	否	否	扩展字段
lastName	string	否	否	名
departDate	string	否	否	离职日期
joinedDate	string	否	否	入职日期
nameDefines	string	否	否	姓名扩展
addrId	string	否	否	地点
jobGradeId	string	否	否	职级
currentJob	string	否	否	当前任职
staffLangAbility	object	是	否	语言能力
jobRankId	string	否	否	职等
staffTechPost	object	是	否	专业技术职务
staffAss	object	是	否	绩效记录
staffCertificate	object	是	否	证书子集
staffDotline	object	是	否	虚线上级
staffBankAcct	object	是	否	银行账户
staffCert	object	是	否	证件信息
staffJob	object	是	是	任职信息
staffTitle	object	是	否	职称记录
staffReward	object	是	否	奖励情况
staffProbation	object	是	否	试用信息
staffTrain	object	是	否	培训记录
staffResume	object	是	否	履历信息
staffEdu	object	是	否	学历信息
staffProqualify	object	是	否	职业资格
staffCtrt	object	是	否	合同信息
staffVODefine	object	否	否	基本信息自定义项
defineSet	object	是	否	自定义信息集名称，表名+List
isReturn	boolean	否	否	是否返聘员工 示例：true
isUseNewCode	boolean	否	否	是否使用新编码 示例：true
id	string	否	否	员工id，返聘时必填 示例：54365666666
staffCadre	object	是	否	任免信息
staffPunishment	object	是	否	惩处信息
staffHealthSituation	object	是	否	健康信息
idCardFront	string	否	否	身份证国徽面
staffSocietyEmployment	object	是	否	社会任职信息
staffDirectiorSupervisor	object	是	否	董监高信息
staffLeadershipTrade	object	是	否	党组/工会信息
staffTrialEvaluation	object	是	否	试用考核结果
staffProtocol	object	否	否	协议信息
idCardReverse	string	否	否	身份证人像面
wagegroup	string	否	否	薪资组
staffOrgRel	object	是	否	工作关系
dr	number
小数位数:0,最大长度:10	否	否	dr 默认值：0
enable	number
小数位数:0,最大长度:10	否	否	人员状态
deptId	string	否	否	所属部门

## 3. 请求示例

Url: /yonbip/hrcloud/byd/save?access_token=访问令牌
Body: {
	"data": [
		{
			"code": "2511871812849920",
			"name": "测试1234",
			"certType": "0010-3600-41e2-8568-42fbc6c96c9d",
			"certNo": "0909765437777",
			"sex": "2",
			"birthDate": "2013-11-09",
			"mobile": "+86-13842985777",
			"dataCreateSource": "",
			"dataCreateBillid": "",
			"officeTel": "1546816777",
			"email": "321321@777.com",
			"selfEmail": "7777@qq.com",
			"joinWorkDate": "2020-11-01",
			"linkAddr": "沈阳",
			"education": "2476728905519360",
			"degree": "2476730873303296",
			"marital": "2475282874225665",
			"bloodType": "41eb36a1-4f93-4504-b4ae-91837e5b1a7e",
			"characterrpr": "5db087f8-8dbf-4426-9b61-9c3886f812a3",
			"permanreside": "bfbd1620-5cf3-11e9-817e-7cd30abea0c0",
			"political": "2508983411364096",
			"joinPolityDate": "2017-11-07",
			"nationality": "694d8570-3da8-4162-b521-1d0dfa2a13ea",
			"country": "6cefdb2b-180f-4ba5-8710-fa5a27a108bb",
			"origin": "c04c67b2-5cf3-11e9-817e-7cd30abea0c0",
			"qq": "7787845448",
			"linkedIn": "0215141185",
			"formerName": "测试5678",
			"englishName": "test1234",
			"documentValidity": "2014-03-23",
			"height": "173",
			"weight": "70",
			"postcode": "431728",
			"birthPlace": "36276271113322",
			"religion": "1493439902010001",
			"hobby": "",
			"bizManTag": "false",
			"shopAssisTag": "true",
			"attachlist": "3f9ebc70-4058-11ec-863d-bb0606e88ccf",
			"weiXin": "",
			"middleName": "",
			"firstName": "",
			"creator": "",
			"shortName": "",
			"modifier": "",
			"modifyTime": "",
			"createTime": "",
			"tenant": "",
			"yTenantId": "",
			"majorcorpid": "",
			"workAge": "",
			"secretId": "",
			"unitId": "",
			"collectionstatus": "",
			"rejectiontimes": "",
			"rejectionreason": "",
			"attachIdList": "",
			"postId": "",
			"objid": "",
			"cadresShoworder": "",
			"iscadre": "",
			"candidateFlag": "",
			"cadreType": "",
			"manageOrg": "",
			"extension": "",
			"lastName": "",
			"departDate": "",
			"joinedDate": "",
			"nameDefines": "",
			"addrId": "",
			"jobGradeId": "",
			"currentJob": "",
			"staffLangAbility": [
				{
					"proLevel": "2475282966107140",
					"language": "f74e0f14-5970-45f0-a09e-77d9d73849a1",
					"certNo": "1651616",
					"langLevel": "高级",
					"memo": "备注",
					"getDate": "2020-11-22 00:00:00",
					"certName": "证书",
					"staffLangAbilityVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"recordnum": "",
					"creator": "",
					"modifier": "",
					"lastflag": "",
					"tenantid": "",
					"createTime": "",
					"modifyTime": "",
					"extension": "",
					"yTenantId": "",
					"staffId": ""
				}
			],
			"jobRankId": "",
			"staffTechPost": [
				{
					"beginDate": "2020-11-06 00:00:00",
					"endDate": "2020-11-22 00:00:00",
					"title": "2508987893993728",
					"ratingOrg": "腾讯",
					"isPrefs": false,
					"certNo": "15615165",
					"memo": "备注",
					"channel": "途径",
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"recordnum": "",
					"lastflag": "",
					"modifier": "",
					"creator": "",
					"tenant": "",
					"modifyTime": "",
					"createTime": "",
					"extension": "",
					"yTenantId": "",
					"staffId": "",
					"professionalTitleSeries": "",
					"level": "",
					"subseries": "",
					"professionSpecialties": "",
					"qualificationNumber": "",
					"acquisitionMethod": ""
				}
			],
			"staffAss": [
				{
					"beginDate": "2019-11-13 00:00:00",
					"endDate": "2019-11-23 00:00:00",
					"score": "100",
					"assDoc": "151",
					"rank": "1",
					"assPeriod": "134768927873722",
					"memo": "备注",
					"staffAssVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"recordnum": "",
					"lastflag": "",
					"creator": "",
					"modifier": "",
					"modifyTime": "",
					"createTime": "",
					"tenant": "",
					"yTenantId": "",
					"staffId": ""
				}
			],
			"staffCertificate": [
				{
					"beginDate": "2021-11-18 00:00:00",
					"certificateCode": "156161",
					"endDate": "2021-12-18 00:00:00",
					"authority": "腾讯",
					"certificateName": "证书",
					"staffCertificateVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"certificateAnnex": "",
					"staffId": "",
					"createTime": "",
					"modifyTime": "",
					"extension": "",
					"yTenantId": "",
					"recordNum": "",
					"lastFlag": "",
					"modifier": ""
				}
			],
			"staffDotline": [
				{
					"endDate": "2020-11-09 00:00:00",
					"lineName": "40017894856903424",
					"beginDate": "2020-11-08 00:00:00",
					"lineType": "2476736877794560",
					"memo": "备注",
					"staffDotlineVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"lastflag": "",
					"recordnum": "",
					"modifier": "",
					"creator": "",
					"yTenantId": "",
					"extension": "",
					"modifyTime": "",
					"createTime": "",
					"tenant": "",
					"staffId": ""
				}
			],
			"staffBankAcct": [
				{
					"openBank": "2493691868090624",
					"sysid": "diwork",
					"isDefaultCard": 0,
					"bank": "2482441004520705",
					"acctType": "BAT00001",
					"currency": "2475282863117312",
					"account": "165161",
					"accountName": "2345",
					"staffBankAcctVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"lastFlag": "",
					"recordNum": "",
					"modifier": "",
					"tenant": "",
					"creator": "",
					"yTenantId": "",
					"modifyTime": "",
					"createTime": "",
					"extension": "",
					"staffId": ""
				}
			],
			"staffCert": [
				{
					"certNo": "152020",
					"certType": "0017-2647-4d9f-81ea-4d6a96be26b2",
					"endDate": "2021-11-19 12:56:55",
					"memo": "说明",
					"beginDate": "2021-11-19 12:56:55",
					"extendOrg": "阿里巴巴",
					"staffCertVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"modifier": "",
					"lastFlag": "",
					"creator": "",
					"recordNum": "",
					"extension": "",
					"modifyTime": "",
					"createTime": "",
					"tenant": "",
					"staffId": ""
				}
			],
			"staffJob": [
				{
					"endDate": "2021-11-02 00:00:00",
					"orgId": "2475303020597504",
					"newAttachment": "051636af-ac64-4aea-ba67-7b556c3f37af",
					"staffFte": "100%",
					"psnclId": "d1170ea764cb4d95a30554cda58866c9",
					"trnsType": "2475282865689711",
					"trnsEvent": 1,
					"deptId": "2475353647993088",
					"beginDate": "2020-11-01 00:00:00",
					"memo": "备注",
					"jobId": "88462662626",
					"jobTypeId": "88462662626",
					"postStatus": "",
					"addrId": "1515151",
					"trnsReason": "个人",
					"director": "",
					"jobRankId": "1",
					"jobGradeId": "59261",
					"postId": "152345345465434",
					"disexplain": "",
					"dispositon": "上海",
					"staffJobVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"isweaken": true,
					"effectiveNumber": 1,
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"bmLocation": "",
					"endFlag": "",
					"showOrder": "",
					"assgId": "",
					"modifier": "",
					"waStruct": "",
					"oriBillType": "",
					"isCadre": "",
					"businessOrg": "",
					"partTypeFixId": "",
					"businessDept": "",
					"empform": "",
					"entryProFlag": "",
					"sysid": "",
					"modifyTime": "",
					"createTime": "",
					"enable": "",
					"yTenantId": "",
					"extension": "",
					"isMainJob": "",
					"staffId": "",
					"attachIdList": "",
					"outerJobInfo": "",
					"partTypeStatus": "",
					"wageGroup": "",
					"businessTypeScope": "",
					"name": "",
					"staffjobId": "",
					"staffjobCode": "",
					"belongJob": "",
					"objId": "",
					"jobVid": "",
					"jobTypeVid": "",
					"jobRankVid": "",
					"newPostVid": "",
					"postVid": "",
					"jobGradeVid": "",
					"deptVid": "",
					"orgVid": "",
					"account_org_id": "",
					"country": "",
					"localOrgVid": "",
					"staffJobDefinesHK": "",
					"timezone": "",
					"planEndDate": "",
					"recordNum": "",
					"oribill_id": "",
					"creator": "",
					"partType": "",
					"dr": "",
					"orgRelId": "",
					"lastestjob": "",
					"lastFlag": true
				}
			],
			"staffTitle": [
				{
					"certType": "2476752253768960",
					"certCode": "02313",
					"endDate": "2021-11-11 00:00:00",
					"certRank": "2476752765736192",
					"isPrint": false,
					"beginDate": "2021-11-02 00:00:00",
					"assOrg": "华信",
					"memo": "备注",
					"proField": "英语",
					"staffTitleVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"creator": "",
					"modifier": "",
					"begindate": "",
					"recordNum": "",
					"lastFlag": "",
					"yTenantId": "",
					"extension": "",
					"modifyTime": "",
					"createTime": "",
					"tenant": "",
					"staffId": ""
				}
			],
			"staffReward": [
				{
					"rewardReason": "李浩",
					"rewardOrg": "华信",
					"rewardDate": "2021-11-02 00:00:00",
					"rewardItem": "大奖",
					"memo": "备注",
					"rewardItem_supplement": "奖励补充",
					"rewardCategory": "703",
					"awardLevel": "1970911435205115935",
					"awardYear": "2024-01-01",
					"awardGrade": "10",
					"approvalNo": "no33453",
					"certificateNo": "645465677",
					"rewardNature": "1",
					"incentives": "奖励",
					"approverCode": "REWARD_001",
					"approverLevel": "101",
					"approvalTime": "2024-02-03",
					"awardCancellation": "false",
					"awardCancellationTime": "2024-04-21",
					"cancellationReason": "",
					"newAttachment": "c6bce09e-f3e3-4142-903d-afc030a5d23a",
					"staffRewardVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"lastFlag": "",
					"recordNum": "",
					"creator": "",
					"tenant": "",
					"modifier": "",
					"yTenantId": "",
					"extension": "",
					"modifyTime": "",
					"createTime": "",
					"staffId": ""
				}
			],
			"staffProbation": [
				{
					"result": 1,
					"beginDate": "2020-11-01 00:00:00",
					"positiveDate": "2020-11-12 00:00:00",
					"endDate": "2020-11-07 00:00:00",
					"endFlag": false,
					"type": 1,
					"memo": "备注",
					"promonth": 0,
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"modifyTime": "",
					"modifier": "",
					"createTime": "",
					"creator": "",
					"extension": "",
					"tenant": "",
					"recordnum": "",
					"lastFlag": "",
					"yTenantId": "",
					"staffId": ""
				}
			],
			"staffTrain": [
				{
					"beginDate": "2021-11-03 00:00:00",
					"endDate": "2021-11-13 00:00:00",
					"content": "英语",
					"status": "1",
					"earnHours": "33",
					"earnCredit": "100",
					"courseId": "78456",
					"classifiCation": "",
					"hours": "3",
					"memo": "备注",
					"protocolNo": "",
					"fee": "500",
					"score": "100",
					"dayOrHours": "1",
					"staffTrainVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"trainProjectName": "",
					"trainProjectCode": "",
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"modifyTime": "",
					"modifier": "",
					"createTime": "",
					"creator": "",
					"staffId": "",
					"tenant": "",
					"yTenantId": "",
					"extension": ""
				}
			],
			"staffResume": [
				{
					"workCorp": "天海总公司",
					"beginDate": "2023-11-01 00:00:00",
					"workDept": "部门",
					"endDate": "2022-11-19 12:56:55",
					"memo": "备注",
					"workDuty": "劳动",
					"workPost": "华信",
					"workJob": "123",
					"certifier": "哈哈",
					"staffResumeVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"resumeType": 1,
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"recordNum": "",
					"creator": "",
					"lastFlag": "",
					"modifier": "",
					"modifyTime": "",
					"createTime": "",
					"staffId": ""
				}
			],
			"staffEdu": [
				{
					"education": "2476728898130176",
					"endDate": "2019-11-22 00:00:00",
					"degree": "2476730871353600",
					"beginDate": "2019-11-07 00:00:00",
					"newAttachment": "3e55f0e1-99bf-4cf5-93d9-47641d7c8709",
					"studyMode": "05c03019-156f-47b5-8d46-20f57944a594",
					"major": "2524738586759424",
					"memo": "备注",
					"eduSystem": "1",
					"certifCode": "2524738586759424",
					"educationCtifCode": "2524738586759424",
					"isPrefs": "false",
					"school": "",
					"staffEduVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"isHighestDegree": "false",
					"isHighestFullDegree": "false",
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"staffId": ""
				}
			],
			"staffProqualify": [
				{
					"beginDate": "2020-11-18 00:00:00",
					"getDate": "2021-11-01 00:00:00",
					"title": "a7baf563-d9a1-4ac5-9331-fb640bc04aae",
					"memo": "备注",
					"certNo": "123",
					"ratingOrg": "123551516",
					"isPrefs": "true",
					"endDate": "2021-11-01 00:00:00",
					"staffProqualifyVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"staffId": ""
				}
			],
			"staffCtrt": [
				{
					"continueTime": 1,
					"endDate": "2021-02-20 00:00:00",
					"termType": 1,
					"signDate": "2020-11-11 00:00:00",
					"probEndDate": "2020-12-20 00:00:00",
					"newAttachment": "76614af0-b8fb-45ec-95c9-27c2d062cfec",
					"termMonth": 3,
					"majorCorpId": "2479635332305152",
					"workAddr": "2479635732058368",
					"isReceive": false,
					"contType": 1,
					"beginDate": "2020-11-20 00:00:00",
					"probSalary": 200,
					"proMonth": 1,
					"contractNum": "CTRT000459",
					"neconomy": "100",
					"startSalary": "50000",
					"unchReason": "123",
					"memo": "备注",
					"contractCode": "120",
					"presenter": "",
					"breachMoney": "5000",
					"proBeginDate": "2020-11-20 00:00:00",
					"staffCtrtVODefine": {
						"define1": "242424",
						"define2_name": "00000004"
					},
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"staffId": ""
				}
			],
			"staffVODefine": {
				"define1": "242424",
				"define2_name": "00000004",
				"define3": "{\\\"linkText\\\":\\\"1234\\\",\\\"linkAddress\\\":\\\"http://aaa.com\\\"}"
			},
			"defineSet": [
				{}
			],
			"isReturn": true,
			"isUseNewCode": true,
			"id": "54365666666",
			"staffCadre": [
				{
					"FormOfAppointment": "",
					"appointmentApprovalUnit": "",
					"appointmentDeadline": "2026-06-07",
					"appointmentNumber": "",
					"appointmentReason": "",
					"department": "",
					"dismissalApprovalUnit": "",
					"dismissalNumber": "",
					"dismissalReason": "",
					"jobAttributes": "",
					"jobCategory": "",
					"jobGrade": "",
					"jobTitle": "",
					"remarks": "",
					"removalTime": "2026-06-07",
					"selectionMethod": "",
					"staffId": "",
					"workingTime": "2026-06-07",
					"workingUnit": "",
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": ""
				}
			],
			"staffPunishment": [
				{
					"approvalCancellationDocumentNo": "",
					"approvalOfDissolutionUnit": "",
					"approvalOfDissolutionUnitCode": "",
					"approvalOfDissolutionUnitLevel": "",
					"approvalReleaseTime": "2026-06-07",
					"dismissalOfPunishmentDocumentNo": "",
					"dismissalofpunishmentReason": "",
					"isTerminate": true,
					"punishedUnit": "",
					"punishedUnitCode": "",
					"punishedUnitLevel": "",
					"punishmentCancellationEffectiveTime": "2026-06-07",
					"punishmentMeasures": "",
					"punishmentName": "",
					"punishmentNameSupplement": "",
					"punishmentNo": "",
					"punishmentReason": "",
					"punishmentTime": "2026-06-07",
					"punishmentType": "",
					"remarks": "",
					"staffId": "",
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": ""
				}
			],
			"staffHealthSituation": [
				{
					"deformity": true,
					"deformityCertDate": "2026-06-07",
					"deformityCertNo": "",
					"deformityLevel": "",
					"healthCondition": "",
					"pastHistory": "",
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"staffId": ""
				}
			],
			"idCardFront": "",
			"staffSocietyEmployment": [
				{
					"beginDate": "2026-06-07",
					"certifier": "",
					"certifierMobile": "",
					"duties": "",
					"memo": "",
					"organization": "",
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"staffId": ""
				}
			],
			"staffDirectiorSupervisor": [
				{
					"beginDate": "2026-06-07",
					"endDate": "2026-06-07",
					"duties": "",
					"independentDirector": true,
					"organization": "",
					"termOfOffice": 0,
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"staffId": ""
				}
			],
			"staffLeadershipTrade": [
				{
					"commissionBeginDate": "2026-06-07",
					"commissionEndDate": "2026-06-07",
					"leadershipDuties": "",
					"leadershipName": "",
					"memo": "",
					"staffId": "",
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": ""
				}
			],
			"staffTrialEvaluation": [
				{
					"beginDate": "2026-06-07",
					"endDate": "2026-06-07",
					"devaluationLevel": "",
					"evaluationScore": "",
					"overallEvaluation": "",
					"memo": "",
					"electronicSignature": "",
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"id": "",
					"staffId": ""
				}
			],
			"staffProtocol": {
				"termMonth": 0,
				"endDate": "2020-11-26",
				"protocolNum": "PROT000057",
				"majorCorpId": "2475283024958467",
				"contType": 1,
				"signDate": "2020-11-12 00:00:00",
				"beginDate": "2020-11-21 00:00:00",
				"newAttachment": "c6bce09e-f3e3-4142-903d-afc030a5d23a",
				"id": "2524738586775815",
				"isReceive": "false",
				"memo": "备注",
				"workAddr": "工作地点",
				"belongJob": "2524738586775815",
				"dataCreateSource": "",
				"dataCreateBillid": "",
				"staffId": ""
			},
			"idCardReverse": "",
			"wagegroup": "",
			"staffOrgRel": [
				{
					"regDate": "",
					"creator": "",
					"seniorityDate": "",
					"endDate": "",
					"indocFlag": "",
					"entrySrc": "",
					"rmChannel": "",
					"seniorityAdj": "",
					"recordNum": "",
					"modifier": "",
					"isReturn": "",
					"isRegular": "",
					"endFlag": "",
					"tenant": "",
					"createTime": "",
					"modifyTime": "",
					"delayRegdate": "",
					"trialResult": "",
					"extension": "",
					"yTenantId": "",
					"id": "",
					"beginDate": "",
					"staffId": "",
					"relDefines": "",
					"orgId": "",
					"secretaryAge": "",
					"groupWorkAge": "",
					"employer": "",
					"groupDate": "",
					"groupWorkAgeAdj": "",
					"code": "",
					"staffCode": "",
					"employerVid": "",
					"entrySource": "",
					"dr": "",
					"lastFlag": true
				}
			],
			"dr": 0,
			"enable": 0,
			"deptId": ""
		}
	]
}

## 4. 返回值参数

名称	类型	数组	描述
billtype	string	否	事项类型, 1:销售发票、2:其它应收事项、3:销售发票(红字)、4:其它应收事项(红字)、5:订单日报、6:内部交易结算单、7:收款单、8:其它应付事项、9:客户退款单、10:付款单、11:供应商退款单、12:转账单、13:汇率损益单、14:外币兑换单、
creator	string	否	创建人
creator	string	否	创建人
displayCode	string	否	异常码
displayCode	string	否	异常码
message	string	否	信息 示例：操作成功
data	string	否	响应信息
salesOrgId	string	否	销售组织id
saleDepartmentId	string	否	销售部门id
transactionTypeId	string	否	交易类型id
settlementOrgId	string	否	开票组织id
bizId	string	否	商家id
createDate	string	否	创建时间
synSourceOrg	string	否	协同来源组织id
ecsuiteuser	string	否	气泡联系人
ecsuiteuser	string	否	气泡联系人
billtype	string	否	事项类型, 1:销售发票、2:其它应收事项、3:销售发票(红字)、4:其它应收事项(红字)、5:订单日报、6:内部交易结算单、7:收款单、8:其它应付事项、9:客户退款单、10:付款单、11:供应商退款单、12:转账单、13:汇率损益单、14:外币兑换单、
creator	string	否	创建人
displayCode	string	否	异常码
message	string	否	信息 示例：操作成功
data	string	否	响应信息
code	number
小数位数:0,最大长度:10	否	code 示例：200
message	string	否	message 示例：操作成功
data	object	否	data
count	number
小数位数:0,最大长度:10	否	count 示例：1
sucessCount	number
小数位数:0,最大长度:10	否	sucessCount 示例：1
failCount	number
小数位数:0,最大长度:10	否	failCount 示例：0
messages	object	否	messages
infos	object	是	infos
failInfos	object	否	failInfos

## 5. 正确返回示例

{
	"code": 200,
	"message": "操作成功",
	"data": {
		"count": 1,
		"sucessCount": 1,
		"failCount": 0,
		"messages": {},
		"infos": [
			{
				"code": "2511871812849920",
				"name": "测试1234",
				"certType": "0010-3600-41e2-8568-42fbc6c96c9d",
				"certNo": "0909765437777",
				"sex": "2",
				"birthDate": "2013-11-09",
				"mobile": "+86-13842985777",
				"dataCreateSource": "",
				"dataCreateBillid": "",
				"officeTel": "1546816777",
				"email": "321321@777.com",
				"selfEmail": "7777@qq.com",
				"joinWorkDate": "2020-11-01",
				"linkAddr": "沈阳",
				"education": "2476728905519360",
				"degree": "2476730873303296",
				"marital": "2475282874225665",
				"bloodType": "41eb36a1-4f93-4504-b4ae-91837e5b1a7e",
				"characterrpr": "5db087f8-8dbf-4426-9b61-9c3886f812a3",
				"permanreside": "bfbd1620-5cf3-11e9-817e-7cd30abea0c0",
				"political": "2508983411364096",
				"joinPolityDate": "2017-11-07",
				"nationality": "694d8570-3da8-4162-b521-1d0dfa2a13ea",
				"country": "6cefdb2b-180f-4ba5-8710-fa5a27a108bb",
				"origin": "c04c67b2-5cf3-11e9-817e-7cd30abea0c0",
				"qq": "7787845448",
				"linkedIn": "0215141185",
				"formerName": "测试5678",
				"englishName": "test1234",
				"documentValidity": "2014-03-23",
				"height": "173",
				"weight": "70",
				"postcode": "431728",
				"birthPlace": "36276271113322",
				"religion": "1493439902010001",
				"hobby": "",
				"bizManTag": "false",
				"shopAssisTag": "true",
				"attachlist": "3f9ebc70-4058-11ec-863d-bb0606e88ccf",
				"weiXin": "",
				"middleName": "",
				"firstName": "",
				"creator": "",
				"shortName": "",
				"modifier": "",
				"modifyTime": "",
				"createTime": "",
				"tenant": "",
				"yTenantId": "",
				"majorcorpid": "",
				"workAge": "",
				"secretId": "",
				"unitId": "",
				"collectionstatus": "",
				"rejectiontimes": "",
				"rejectionreason": "",
				"attachIdList": "",
				"postId": "",
				"objid": "",
				"cadresShoworder": "",
				"iscadre": "",
				"candidateFlag": "",
				"cadreType": "",
				"manageOrg": "",
				"extension": "",
				"lastName": "",
				"departDate": "",
				"joinedDate": "",
				"nameDefines": "",
				"addrId": "",
				"jobGradeId": "",
				"currentJob": "",
				"staffLangAbility": [
					{
						"proLevel": "2475282966107140",
						"language": "f74e0f14-5970-45f0-a09e-77d9d73849a1",
						"certNo": "1651616",
						"langLevel": "高级",
						"memo": "备注",
						"getDate": "2020-11-22 00:00:00",
						"certName": "证书",
						"staffLangAbilityVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"langDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"recordnum": "",
						"creator": "",
						"modifier": "",
						"lastflag": "",
						"tenantid": "",
						"createTime": "",
						"modifyTime": "",
						"extension": "",
						"yTenantId": "",
						"staffId": ""
					}
				],
				"jobRankId": "",
				"staffTechPost": [
					{
						"beginDate": "2020-11-06 00:00:00",
						"endDate": "2020-11-22 00:00:00",
						"title": "2508987893993728",
						"ratingOrg": "腾讯",
						"isPrefs": false,
						"certNo": "15615165",
						"memo": "备注",
						"channel": "途径",
						"tecDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"recordnum": "",
						"lastflag": "",
						"modifier": "",
						"creator": "",
						"tenant": "",
						"modifyTime": "",
						"createTime": "",
						"extension": "",
						"yTenantId": "",
						"staffId": "",
						"professionalTitleSeries": "",
						"level": "",
						"subseries": "",
						"professionSpecialties": "",
						"qualificationNumber": "",
						"acquisitionMethod": ""
					}
				],
				"staffAss": [
					{
						"beginDate": "2019-11-13 00:00:00",
						"endDate": "2019-11-23 00:00:00",
						"score": "100",
						"assDoc": "151",
						"rank": "1",
						"assPeriod": "134768927873722",
						"memo": "备注",
						"staffAssVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"assDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"recordnum": "",
						"lastflag": "",
						"creator": "",
						"modifier": "",
						"modifyTime": "",
						"createTime": "",
						"tenant": "",
						"yTenantId": "",
						"staffId": ""
					}
				],
				"staffCertificate": [
					{
						"beginDate": "2021-11-18 00:00:00",
						"certificateCode": "156161",
						"endDate": "2021-12-18 00:00:00",
						"authority": "腾讯",
						"certificateName": "证书",
						"staffCertificateVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"certificateDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"certificateAnnex": "",
						"staffId": "",
						"createTime": "",
						"modifyTime": "",
						"extension": "",
						"yTenantId": "",
						"recordNum": "",
						"lastFlag": "",
						"modifier": ""
					}
				],
				"staffDotline": [
					{
						"endDate": "2020-11-09 00:00:00",
						"lineName": "40017894856903424",
						"beginDate": "2020-11-08 00:00:00",
						"lineType": "2476736877794560",
						"memo": "备注",
						"staffDotlineVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"dotlineDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"lastflag": "",
						"recordnum": "",
						"modifier": "",
						"creator": "",
						"yTenantId": "",
						"extension": "",
						"modifyTime": "",
						"createTime": "",
						"tenant": "",
						"staffId": ""
					}
				],
				"staffBankAcct": [
					{
						"openBank": "2493691868090624",
						"sysid": "diwork",
						"isDefaultCard": 0,
						"bank": "2482441004520705",
						"acctType": "BAT00001",
						"currency": "2475282863117312",
						"account": "165161",
						"accountName": "2345",
						"staffBankAcctVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"bankDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"lastFlag": "",
						"recordNum": "",
						"modifier": "",
						"tenant": "",
						"creator": "",
						"yTenantId": "",
						"modifyTime": "",
						"createTime": "",
						"extension": "",
						"staffId": ""
					}
				],
				"staffCert": [
					{
						"certNo": "152020",
						"certType": "0017-2647-4d9f-81ea-4d6a96be26b2",
						"endDate": "2021-11-19 12:56:55",
						"memo": "说明",
						"beginDate": "2021-11-19 12:56:55",
						"extendOrg": "阿里巴巴",
						"staffCertVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"certDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"modifier": "",
						"lastFlag": "",
						"creator": "",
						"recordNum": "",
						"extension": "",
						"modifyTime": "",
						"createTime": "",
						"tenant": "",
						"staffId": ""
					}
				],
				"staffJob": [
					{
						"endDate": "2021-11-02 00:00:00",
						"orgId": "2475303020597504",
						"newAttachment": "051636af-ac64-4aea-ba67-7b556c3f37af",
						"staffFte": "100%",
						"psnclId": "d1170ea764cb4d95a30554cda58866c9",
						"trnsType": "2475282865689711",
						"trnsEvent": 1,
						"deptId": "2475353647993088",
						"beginDate": "2020-11-01 00:00:00",
						"memo": "备注",
						"jobId": "88462662626",
						"jobTypeId": "88462662626",
						"postStatus": "",
						"addrId": "1515151",
						"trnsReason": "个人",
						"director": "",
						"jobRankId": "1",
						"jobGradeId": "59261",
						"postId": "152345345465434",
						"disexplain": "",
						"dispositon": "上海",
						"staffJobVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"staffJobDefines": {},
						"isweaken": true,
						"effectiveNumber": 1,
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"bmLocation": "",
						"endFlag": "",
						"showOrder": "",
						"assgId": "",
						"modifier": "",
						"waStruct": "",
						"oriBillType": "",
						"isCadre": "",
						"businessOrg": "",
						"partTypeFixId": "",
						"businessDept": "",
						"empform": "",
						"entryProFlag": "",
						"sysid": "",
						"modifyTime": "",
						"createTime": "",
						"enable": "",
						"yTenantId": "",
						"extension": "",
						"isMainJob": "",
						"staffId": "",
						"attachIdList": "",
						"outerJobInfo": "",
						"partTypeStatus": "",
						"wageGroup": "",
						"businessTypeScope": "",
						"name": "",
						"staffjobId": "",
						"staffjobCode": "",
						"belongJob": "",
						"objId": "",
						"jobVid": "",
						"jobTypeVid": "",
						"jobRankVid": "",
						"newPostVid": "",
						"postVid": "",
						"jobGradeVid": "",
						"deptVid": "",
						"orgVid": "",
						"account_org_id": "",
						"country": "",
						"localOrgVid": "",
						"staffJobDefinesHK": "",
						"timezone": "",
						"planEndDate": "",
						"recordNum": "",
						"oribill_id": "",
						"creator": "",
						"partType": "",
						"dr": "",
						"orgRelId": "",
						"lastestjob": "",
						"lastFlag": true
					}
				],
				"staffTitle": [
					{
						"certType": "2476752253768960",
						"certCode": "02313",
						"endDate": "2021-11-11 00:00:00",
						"certRank": "2476752765736192",
						"isPrint": false,
						"beginDate": "2021-11-02 00:00:00",
						"assOrg": "华信",
						"memo": "备注",
						"proField": "英语",
						"staffTitleVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"titleDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"creator": "",
						"modifier": "",
						"begindate": "",
						"recordNum": "",
						"lastFlag": "",
						"yTenantId": "",
						"extension": "",
						"modifyTime": "",
						"createTime": "",
						"tenant": "",
						"staffId": ""
					}
				],
				"staffReward": [
					{
						"rewardReason": "李浩",
						"rewardOrg": "华信",
						"rewardDate": "2021-11-02 00:00:00",
						"rewardItem": "大奖",
						"memo": "备注",
						"rewardDefines": {},
						"rewardItem_supplement": "奖励补充",
						"rewardCategory": "703",
						"awardLevel": "1970911435205115935",
						"awardYear": "2024-01-01",
						"awardGrade": "10",
						"approvalNo": "no33453",
						"certificateNo": "645465677",
						"rewardNature": "1",
						"incentives": "奖励",
						"approverCode": "REWARD_001",
						"approverLevel": "101",
						"approvalTime": "2024-02-03",
						"awardCancellation": "false",
						"awardCancellationTime": "2024-04-21",
						"cancellationReason": "",
						"newAttachment": "c6bce09e-f3e3-4142-903d-afc030a5d23a",
						"staffRewardVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"lastFlag": "",
						"recordNum": "",
						"creator": "",
						"tenant": "",
						"modifier": "",
						"yTenantId": "",
						"extension": "",
						"modifyTime": "",
						"createTime": "",
						"staffId": ""
					}
				],
				"staffProbation": [
					{
						"result": 1,
						"beginDate": "2020-11-01 00:00:00",
						"positiveDate": "2020-11-12 00:00:00",
						"endDate": "2020-11-07 00:00:00",
						"endFlag": false,
						"type": 1,
						"memo": "备注",
						"probationDefines": {},
						"promonth": 0,
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"modifyTime": "",
						"modifier": "",
						"createTime": "",
						"creator": "",
						"extension": "",
						"tenant": "",
						"recordnum": "",
						"lastFlag": "",
						"yTenantId": "",
						"staffId": ""
					}
				],
				"staffTrain": [
					{
						"beginDate": "2021-11-03 00:00:00",
						"endDate": "2021-11-13 00:00:00",
						"content": "英语",
						"status": "1",
						"earnHours": "33",
						"earnCredit": "100",
						"courseId": "78456",
						"classifiCation": "",
						"hours": "3",
						"memo": "备注",
						"protocolNo": "",
						"fee": "500",
						"score": "100",
						"dayOrHours": "1",
						"staffTrainVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"trainDefines": {},
						"trainProjectName": "",
						"trainProjectCode": "",
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"modifyTime": "",
						"modifier": "",
						"createTime": "",
						"creator": "",
						"staffId": "",
						"tenant": "",
						"yTenantId": "",
						"extension": ""
					}
				],
				"staffResume": [
					{
						"workCorp": "天海总公司",
						"beginDate": "2023-11-01 00:00:00",
						"workDept": "部门",
						"endDate": "2022-11-19 12:56:55",
						"memo": "备注",
						"workDuty": "劳动",
						"workPost": "华信",
						"workJob": "123",
						"certifier": "哈哈",
						"staffResumeVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"resumeDefines": {},
						"resumeType": 1,
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"recordNum": "",
						"creator": "",
						"lastFlag": "",
						"modifier": "",
						"modifyTime": "",
						"createTime": "",
						"staffId": ""
					}
				],
				"staffEdu": [
					{
						"education": "2476728898130176",
						"endDate": "2019-11-22 00:00:00",
						"degree": "2476730871353600",
						"beginDate": "2019-11-07 00:00:00",
						"newAttachment": "3e55f0e1-99bf-4cf5-93d9-47641d7c8709",
						"studyMode": "05c03019-156f-47b5-8d46-20f57944a594",
						"major": "2524738586759424",
						"memo": "备注",
						"eduSystem": "1",
						"certifCode": "2524738586759424",
						"educationCtifCode": "2524738586759424",
						"isPrefs": "false",
						"school": "",
						"staffEduVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"eduDefines": {},
						"isHighestDegree": "false",
						"isHighestFullDegree": "false",
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"staffId": ""
					}
				],
				"staffProqualify": [
					{
						"beginDate": "2020-11-18 00:00:00",
						"getDate": "2021-11-01 00:00:00",
						"title": "a7baf563-d9a1-4ac5-9331-fb640bc04aae",
						"memo": "备注",
						"certNo": "123",
						"ratingOrg": "123551516",
						"isPrefs": "true",
						"endDate": "2021-11-01 00:00:00",
						"staffProqualifyVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"proqualifyDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"staffId": ""
					}
				],
				"staffCtrt": [
					{
						"continueTime": 1,
						"endDate": "2021-02-20 00:00:00",
						"termType": 1,
						"signDate": "2020-11-11 00:00:00",
						"probEndDate": "2020-12-20 00:00:00",
						"newAttachment": "76614af0-b8fb-45ec-95c9-27c2d062cfec",
						"termMonth": 3,
						"majorCorpId": "2479635332305152",
						"workAddr": "2479635732058368",
						"isReceive": false,
						"contType": 1,
						"beginDate": "2020-11-20 00:00:00",
						"probSalary": 200,
						"proMonth": 1,
						"contractNum": "CTRT000459",
						"neconomy": "100",
						"startSalary": "50000",
						"unchReason": "123",
						"memo": "备注",
						"contractCode": "120",
						"presenter": "",
						"breachMoney": "5000",
						"proBeginDate": "2020-11-20 00:00:00",
						"staffCtrtVODefine": {
							"define1": "242424",
							"define2_name": "00000004"
						},
						"ctrtDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"staffId": ""
					}
				],
				"staffVODefine": {
					"define1": "242424",
					"define2_name": "00000004",
					"define3": "{\\linkText\\:\\1234\\,\\linkAddress\\:\\http://aaa.com\\}"
				},
				"defineSet": [
					{}
				],
				"staffDefines": {},
				"isReturn": true,
				"isUseNewCode": true,
				"id": "54365666666",
				"staffCadre": [
					{
						"FormOfAppointment": "",
						"appointmentApprovalUnit": "",
						"appointmentDeadline": "",
						"appointmentNumber": "",
						"appointmentReason": "",
						"department": "",
						"dismissalApprovalUnit": "",
						"dismissalNumber": "",
						"dismissalReason": "",
						"jobAttributes": "",
						"jobCategory": "",
						"jobGrade": "",
						"jobTitle": "",
						"remarks": "",
						"removalTime": "",
						"selectionMethod": "",
						"staffId": "",
						"workingTime": "",
						"workingUnit": "",
						"staffCadreTZDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": ""
					}
				],
				"staffPunishment": [
					{
						"approvalCancellationDocumentNo": "",
						"approvalOfDissolutionUnit": "",
						"approvalOfDissolutionUnitCode": "",
						"approvalOfDissolutionUnitLevel": "",
						"approvalReleaseTime": "",
						"dismissalOfPunishmentDocumentNo": "",
						"dismissalofpunishmentReason": "",
						"isTerminate": true,
						"punishedUnit": "",
						"punishedUnitCode": "",
						"punishedUnitLevel": "",
						"punishmentCancellationEffectiveTime": "",
						"punishmentMeasures": "",
						"punishmentName": "",
						"punishmentNameSupplement": "",
						"punishmentNo": "",
						"punishmentReason": "",
						"punishmentTime": "",
						"punishmentType": "",
						"remarks": "",
						"staffId": "",
						"staffPunishmentTZDefines": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": ""
					}
				],
				"staffHealthSituation": [
					{
						"deformity": true,
						"deformityCertDate": "",
						"deformityCertNo": "",
						"deformityLevel": "",
						"healthCondition": "",
						"pastHistory": "",
						"healthSituationDefine": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"staffId": ""
					}
				],
				"idCardFront": "",
				"staffSocietyEmployment": [
					{
						"beginDate": "",
						"certifier": "",
						"certifierMobile": "",
						"duties": "",
						"memo": "",
						"organization": "",
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"staffId": ""
					}
				],
				"staffDirectiorSupervisor": [
					{
						"beginDate": "",
						"endDate": "",
						"duties": "",
						"independentDirector": true,
						"organization": "",
						"termOfOffice": 0,
						"societyEmploymentDefine": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"staffId": ""
					}
				],
				"staffLeadershipTrade": [
					{
						"commissionBeginDate": "",
						"commissionEndDate": "",
						"leadershipDuties": "",
						"leadershipName": "",
						"memo": "",
						"staffId": "",
						"leadershipTradeDefine": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": ""
					}
				],
				"staffTrialEvaluation": [
					{
						"beginDate": "",
						"endDate": "",
						"devaluationLevel": "",
						"evaluationScore": "",
						"overallEvaluation": "",
						"memo": "",
						"electronicSignature": "",
						"trialEvaluationDefine": {},
						"dataCreateSource": "",
						"dataCreateBillid": "",
						"id": "",
						"staffId": ""
					}
				],
				"staffProtocol": {
					"termMonth": 0,
					"endDate": "2020-11-26",
					"protocolNum": "PROT000057",
					"majorCorpId": "2475283024958467",
					"contType": 1,
					"signDate": "2020-11-12 00:00:00",
					"beginDate": "2020-11-21 00:00:00",
					"newAttachment": "c6bce09e-f3e3-4142-903d-afc030a5d23a",
					"id": "2524738586775815",
					"isReceive": "false",
					"memo": "备注",
					"workAddr": "工作地点",
					"protocolDefines": {},
					"belongJob": "2524738586775815",
					"dataCreateSource": "",
					"dataCreateBillid": "",
					"staffId": ""
				},
				"idCardReverse": "",
				"wagegroup": "",
				"staffOrgRel": [
					{
						"regDate": "",
						"creator": "",
						"seniorityDate": "",
						"endDate": "",
						"indocFlag": "",
						"entrySrc": "",
						"rmChannel": "",
						"seniorityAdj": "",
						"recordNum": "",
						"modifier": "",
						"isReturn": "",
						"isRegular": "",
						"endFlag": "",
						"tenant": "",
						"createTime": "",
						"modifyTime": "",
						"delayRegdate": "",
						"trialResult": "",
						"extension": "",
						"yTenantId": "",
						"id": "",
						"beginDate": "",
						"staffId": "",
						"relDefines": "",
						"orgId": "",
						"secretaryAge": "",
						"groupWorkAge": "",
						"employer": "",
						"groupDate": "",
						"groupWorkAgeAdj": "",
						"code": "",
						"staffCode": "",
						"employerVid": "",
						"entrySource": "",
						"dr": "",
						"lastFlag": true
					}
				],
				"dr": 0,
				"enable": 0,
				"deptId": ""
			}
		],
		"failInfos": {}
	}
}

## 6. 业务异常码

异常码	异常码信息	描述
暂时没有数据哦~

## 7. 错误返回码

错误码	错误信息	描述
1002	数据不存在	按照提示代码解决
1001	价税合计必须小于0	请确认价税合计金额
1002	数据不存在	按照提示代码解决
"failCount": 1	"证件类型id:0010-3600-41e2-8568-42fbc6c96c9未找到	修正证件类型

## 9. 接口变更日志

	序号	修改时间	变更内容概要
暂时没有数据哦~

## 1. 请求说明


## 2. 请求参数


## 3. 请求示例


## 4. 返回值参数


## 5. 正确返回示例


## 6. 业务异常码


## 7. 错误返回码


## 9. 接口变更日志

