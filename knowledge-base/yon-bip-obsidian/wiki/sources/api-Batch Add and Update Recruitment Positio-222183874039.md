...
请求地址	/yonbip/hrcloud/recruitPost/saveOrUpdateRecruitPost
请求方式	POST
...
_status	string	否	否	操作标识 示例："Insert":新增，"Delete":删除
id	string	否	否	部门子表主键id 示例：删除操作必填，新增操作不需要填写
recruitJobrankVO	object	是	否	职级数据
jobrankId	string	否	否	职级id
_status	string	否	否	操作标识 示例："Insert":新增，"Delete":删除
id	string	否	否	职级子表主键id 示例：删除操作必填，新增操作不需要填写
recruitJobgradeVO	object	是	否	职等数据
jobgradeId	string	否	否	职等id
_status	string	否	否	操作标识 示例："Insert":新增，"Delete":删除
id	string	否	否	职等子表主键id 示例：删除操作必填，新增操作不需要填写
recruitEducationVO	object	是	否	学历要求数据
education	string	否	否	学历id
_status	string	否	否	操作标识 示例："Insert":新增，"Delete":删除
id	string	否	否	学历要求子表主键id 示例：删除操作必填，新增操作不需要填写
recruitWorkAddressVO	object	是	否	工作地点-行政区划数据
workAddress	string	否	否	工作地点-行政区划id
_status	string	否	否	操作标识 示例："Insert":新增，"Delete":删除
id	string	否	否	工作地点-行政区划子表主键id 示例：删除操作必填，新增操作不需要填写
recruitMajorVO	object	是	否	专业类别数据
majorId	string	否	否	专业类别id
id	string	否	否	专业类别子表主键id 示例：删除操作必填，新增操作不需要填写
_status	string	否	否	"Insert":新增，"Delete":删除
_status	string	否	是	招聘职位数据操作标识 示例："Insert":新增，"Update":更新
id	string	否	否	招聘职位id 示例：更新操作必填，新增操作不需要填写

## 3. 请求示例

Url: /yonbip/hrcloud/recruitPost/saveOrUpdateRecruitPost?access_token=访问令牌
Body: [{
	"code": "",
	"name": "",
	"orgId": "",
