...
请求地址	/yonbip/digitalModel/addressarchives/batchSave
...
| isCashOnDelivery | boolean | 否 | 是 | 是否货到付款 示例：false |
| keyWord | object | 否 | 否 | keyWord_DESC |
| name | object | 否 | 是 | name_DESC |
| order | number |
| 小数位数:0,最大长度:20 | 否 | 是 | 顺序 示例：1 |
| parent | string | 否 | 否 | 上级ID（与parent___code二选一录入即可，默认id有值时，code字段不生效） 示例：2211575684096589833 |
| parent___code | string | 否 | 否 | 上级ID编码 示例：1 |
| showName | string | 否 | 否 | showName_DESC |
| zipCode | string | 否 | 否 | 邮政编码 |
| sourceUnique | string | 否 | 是 | 来源主键 |

## 3. 请求示例

Url: /yonbip/digitalModel/addressarchives/batchSave?access_token=访问令牌
Body: {
	"matchRule": "id",
