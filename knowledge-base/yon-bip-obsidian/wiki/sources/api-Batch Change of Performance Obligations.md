...
请求地址	/yonbip/AMP/fiercl/revenuecontract/batchUpdate
...
| extSrcSystem | string | 否 | 否 | 来源单据系统(外) |
| extSrcBillCode | string | 否 | 否 | 来源单据编码(外) |
| rclContractBaseInfoList | object | 是 | 否 | 合同基本信息子实体 |
...
| rclCriticalEventList | object | 是 | 否 | 合同大事记子实体 |
| isAsync | boolean | 否 | 否 | 是否异步 示例：false 默认值：false |

## 3. 请求示例

Url: /yonbip/AMP/fiercl/revenuecontract/batchUpdate?access_token=访问令牌
Body: {
	"data": [
		{
			...
			"rclContractBaseInfoList": [
				{
					...
					"preSubmissionIncomeRule": "13646781525450",
...
| count | number |
| 小数位数:0,最大长度:8 | 否 | 记录数 示例：5 |
| sucessCount | number |
| 小数位数:0,最大长度:8 | 否 | 成功数 示例：3 |
| failCount | number |
| 小数位数:0,最大长度:8 | 否 | 失败数 示例：2 |
