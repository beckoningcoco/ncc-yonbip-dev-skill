...
> `ContentType	application/json` 请求方式	POST | 分类: Impairment Unit (EFI)
...
请求地址	/yonbip/EFI/FixedAssetImpairmentUnit/batchdelete
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| billnum | string | 否 | 是 | 单据编码 示例：fa_impairmentunit_list 默认值：fa_impairmentunit_list |
| data | object | 是 | 是 | 减值单元信息 |
...
Url: /yonbip/EFI/FixedAssetImpairmentUnit/batchdelete?access_token=访问令牌
Body: {
	"billnum": "fa_impairmentunit_list",
	"data": [
		{
