...
请求地址	/yonbip/digitalModel/accountpurpose/batchSave
...
| id | string | 否 | 否 | 主键ID 示例：2217322771360776201 |
| code | string | 否 | 是 | 编码 示例：ghjkl |
| name | object | 否 | 是 | 名称，根据租户开通的语种传入 |
| description | string | 否 | 否 | 描述 示例：新加描述 |
| enable | number |
| 小数位数:2,最大长度:11 | 否 | 否 | 启用状态，0-未启用 1-启用 2-停用 示例：1 |
| isdefault | boolean | 否 | 是 | 是否默认，true-是 false-否 示例：false |
| sourceUnique | string | 否 | 是 | 来源系统id 示例：2217322771360776201 |

## 3. 请求示例

Url: /yonbip/digitalModel/accountpurpose/batchSave?access_token=访问令牌
Body: {
	"matchRule": "id",
	"unmatchedStrategy": "add",
	"data": [
		{
			"id": "2217322771360776201",
			"code": "ghjkl",
			"name": {
				"zh_CN": "简体中文",
			...
			"enable": 1,
			"isdefault": false,
			"sourceUnique": "2217322771360776201"
		}
	]
}
