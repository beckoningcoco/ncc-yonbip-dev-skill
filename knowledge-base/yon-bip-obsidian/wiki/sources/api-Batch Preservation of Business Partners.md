...
请求地址	/yonbip/digitalModel/businesspartner/batchsavebusinesspartner
...
| nameMultilingual | object | 否 | 是 | 名称 |
...
| partnerType | int | 否 | 否 | 伙伴类型（0：企业，1：个人，2：其他），默认为：0。 示例：0 默认值：0 |
...
| status | short | 否 | 否 | 启用状态（0：未启用，1：启用，2：停用），默认为：1 示例：1 默认值：1 |
| licenseType | short | 否 | 否 | 证照类型（0：统一社会信用代码，1：营业执照，2：其他证照，3：居民身份证，4：护照，5：其他身份证件）。当伙伴类型是企业时，传0或2，默认为0；伙伴类型是个人时，传3,4或者5，默认为3；当伙伴类型是其他时，传0，1或者2，默认为0。 示例：0 默认值：0 |
...
| currency | string | 否 | 否 | 注册资金币种id（注册资金币种id和注册资金币种简称选填一个即可，同时填入时以id为准） 示例：15256968861408 |
...
Url: /yonbip/digitalModel/businesspartner/batchsavebusinesspartner?access_token=访问令牌
Body: [{
	"code": "lctest0407",
	"nameMultilingual": {
		"simplifiedName": "业务伙伴",
...
| successCount | string | 否 | 成功总数 示例：1 |
| failCount | number |
| 小数位数:2,最大长度:10 | 否 | 失败总数 示例：1 |
| messages | object | 是 | 错误数据列表 示例：[ "[code111235]伙伴类型为企业、其他时，名称不能重复" ] |
...

{
	...
	"data": {
		...
		"failCount": 1,
		"messages": [
			"[code111235]伙伴类型为企业、其他时，名称不能重复"
		],
		"infos": [
			{
				"id": "1966472466151243793"
			}
		]
	}
}
