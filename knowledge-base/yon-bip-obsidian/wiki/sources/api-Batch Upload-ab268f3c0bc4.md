---
title: "批量上传"
apiId: "ab268f3c0bc4418ab11783cd2775fcf8"
apiPath: "请求方式	POST"
...
请求地址	/yonbip/sd/orders/batch/uploadNew
请求方式	POST
...
Body参数
名称	类型	数组	必填	描述
promotional_discount	BigDecimal	否	否	促销折扣额 示例：0.00
pay_details	object	是	否	支付方式
pay_sum	BigDecimal	否	否	支付金额 示例：1260
pay_code	string	否	否	支付方式编码 示例：1
pay_name	string	否	否	支付方式 示例：现金
mid	long	否	是	会员id 示例：2732903518933248
sum	BigDecimal	否	否	交易金额(退货单为负数) 示例：2100
discount_sum	BigDecimal	否	否	优惠总金额 示例：840
type	int	否	否	订单类型(0 商家自定,2 商城订单 3商城积分兑换订单) 示例：0
store_id	long	否	否	门店id 示例：1992269099749632
pay_sum	BigDecimal	否	否	支付金额(退货单为负数) 示例：1260
small_change_discount	BigDecimal	否	否	抹零金额 示例：0.00
trade_source	string	否	否	交易来源 示例：西城
deduction_apportionment	BigDecimal	否	否	抵扣分摊额 示例：0.00
trade_code	string	否	是	订单编号 示例：LS000120220519000011
spot_discount	BigDecimal	否	否	现场折扣额 示例：0.00
trade_date	DateTime	否	是	交易日期 示例：2022-05-19 11:23:49
out_sys_main_key	string	否	是	订单外键 示例：2780866669450752
membership_discount	BigDecimal	否	否	会员折扣额 示例：840
award_points	long	否	否	商品转移给客户的积分 示例：0
coupon_apportionment	BigDecimal	否	否	优惠券分摊额 示例：0.00
is_storage	boolean	否	否	是否钱包充值 示例：false
status	long	否	是	订单状态(1:已付款、2:已发货、3:已收货、4:已完成、99:已退货) 示例：4
details	object	是	是	订单详情数据
store_id	long	否	否	门店id 示例：1992269099749632
goods_name	string	否	否	商品名称 示例：百龄坛12年苏格兰威士忌（6%）
pay_sum	BigDecimal	否	否	支付金额(退货单为负数) 示例：1260
quantity	long	否	否	商品数量 示例：1
goods_code	string	否	否	商品编码 示例：000105
promotional_discount	BigDecimal	否	否	促销折扣额 示例：0.00
out_sys_key	string	否	是	订单详情外键 示例：2780866669467136
memo	string	否	否	备注 示例：西城:订单LS000120220519000011,消费1260.0元
goods_id	string	否	否	商品id 示例：1992316933804288
original_sum	BigDecimal	否	否	原价 示例：2100
sum	BigDecimal	否	否	交易金额 示例：2100
sku_id	long	否	否	skuid 示例：1992316949811456
trade_source	string	否	否	交易来源 示例：西城
relation_type	long	否	否	关联关系类型(0:商家自定 1: 主套餐 2 子套餐) 示例：0
deduction_apportionment	BigDecimal	否	否	抵扣分摊额 示例：840
spot_discount	BigDecimal	否	否	现场折扣额 示例：0.00
discount_rate	BigDecimal	否	否	折扣比例 示例：60
membership_discount	BigDecimal	否	否	会员折扣额 示例：840
award_points	long	否	否	商品转移给客户的积分 示例：0
use_points	long	否	否	使用积分数量 示例：0
goods_class_name	string	否	否	商品分类名称 示例：洋酒
goods_class_code	string	否	否	商品分类编码 示例：02
appId	string	否	否	会籍id(多会籍租户必传) 示例：yl_z5nqc7bDikBX

## 3. 请求示例

Url: /yonbip/sd/orders/batch/uploadNew?access_token=访问令牌
Body: [{
	...
	"trade_code": "LS000120220519000011",
	"spot_discount": 0,
	"trade_date": "2022-05-19 11:23:49",
	"out_sys_main_key": "2780866669450752",
	"membership_discount": 840,
	"award_points": 0,
	...
	"details": [
		{
			...
			"goods_code": "000105",
			"promotional_discount": 0,
			"out_sys_key": "2780866669467136",
			"memo": "西城:订单LS000120220519000011,消费1260.0元",
