...
请求地址	/yonbip/pm/workHour/batchSubmit
请求方式	POST
...
Url: /yonbip/pm/workHour/batchSubmit?access_token=访问令牌
Body: {
	"ids": [
		1
	],
...
errorMessage	string	否	错误信息(提交成功的，不会有这个属性） 示例：只有开立态的单据才可以提交,当前状态为:审核中
	...
	"data": [
		{
			...
			"errorMessage": "只有开立态的单据才可以提交,当前状态为:审核中"
		}
	]
}
