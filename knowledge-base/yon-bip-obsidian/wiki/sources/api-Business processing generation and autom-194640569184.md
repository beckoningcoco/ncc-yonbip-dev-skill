---
title: "业务办理生成及自动生成档案接口"
apiId: "1946405691844984832"
apiPath: "请求方式	POST"
...
请求地址	/yonbip/hrcloud/insuranceArchive/autoCreateArchive
请求方式	POST
...
Url: /yonbip/hrcloud/insuranceArchive/autoCreateArchive?access_token=访问令牌
Body: {
	"data": [
		{
			"staffId": "员工Id",
...
message	string	否	message 示例：调用失败时的错误信息
data	object	否	data
count	number
小数位数:0,最大长度:10	否	count 示例：103
successCount	number
小数位数:0,最大长度:10	否	successCount 示例：100
failCount	number
小数位数:0,最大长度:10	否	failCount 示例：3
...

{
	...
	"data": {
		...
		"infos": [
			{
				"data": {
					"sourceUnique": "数据唯一标识（来源数据唯一标识）",
					"targetUnique": "数据唯一标识（目标数据唯一标识）"
				},
				"staffId": "数据唯一标识",
				"staffName": "错误数据描述",
				"message": "成功"
