...
## 1. 通过OpenAPI支持删除租入合同，参数为租入合同ID。


## 2. 支持逐一及批量删除。


## 3. 传入的租入合同未发生租入交接、归还、结算等后续业务时支持删除。


## 1. 请求说明

请求域名	动态域名，获取方式详见 获取租户所在数据中心域名
请求地址	/yonbip/am/limrentincard/delete
请求方式	POST
...
Body参数
名称	类型	数组	必填	描述
data	object	是	是	租入合同对象
id	string	否	是	主表主键 示例：12132132323123

## 3. 请求示例

Url: /yonbip/am/limrentincard/delete?access_token=访问令牌
Body: {
	"data": [
		{
			"id": "12132132323123"
