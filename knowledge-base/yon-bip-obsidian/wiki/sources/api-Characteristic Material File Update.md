...
请求地址	/yonbip/digitalModel/product/newupdate
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| orgCode | string | 否 | 否 | 管理组织编码(组织id和编码同时填入时以code为准) 示例：666666 示例：global00 |
| orgId | string | 否 | 否 | 管理组织id(组织id和编码同时填入时以code为准) 示例：666666 示例：666666 |
...
| stopstatus | boolean | 否 | 否 | 启用状态，false代表启用，true代表停用。 示例：true |
| storeOffAndOffState | boolean | 否 | 否 | 商城上架, true:是、false:否。 示例：false |
| orderLoadAndUnloadStatus | boolean | 否 | 否 | 订货上架, true:是、false:否。 示例：false |
...
| unitExchangeType | int | 否 | 否 | 辅计量单位, 0:固定换算、1:浮动换算、 示例：0 |
...
Url: /yonbip/digitalModel/product/newupdate?access_token=访问令牌
Body: {
	"orgCode": "global00",
	"orgId": "666666",
	"code": "0012000488",
