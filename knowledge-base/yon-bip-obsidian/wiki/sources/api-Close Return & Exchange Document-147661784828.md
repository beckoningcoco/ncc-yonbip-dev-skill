...
请求地址	/yonbip/sd/dst/refundorder/refundclose
请求方式	POST
...
Body参数
名称	类型	数组	必填	描述
ids	string	否	是	退换货单的id，若是一次传多张用英文“,”分隔 示例：1065068164894976,1065703716704512
partParam	object	否	是	退换货单id为key，退换货单的时间戳（pubts）为value，组成的map信息

## 3. 请求示例

Url: /yonbip/sd/dst/refundorder/refundclose?access_token=访问令牌
Body: {
	"ids": "1065068164894976,1065703716704512",
	"partParam": {}
}
...
actionName	string	否	操作名称（退换货单关闭）
...
1002	数据不存在	按照提示代码解决
999	没有符合条件的单据或当前单据不是最新状态	单据时间戳必须是最新的
