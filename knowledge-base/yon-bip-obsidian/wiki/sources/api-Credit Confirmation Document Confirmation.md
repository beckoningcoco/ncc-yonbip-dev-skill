---
title: "红字确认单确认"
apiId: "1758093022548459526"
apiPath: "请求方式	POST"
method: "ContentType	application/json"
category: "Red Letter Application Form"
domain: "TAX"
date: 2026-06-07
ingested: 2026-06-07
tags: [YonBIP, OpenAPI, Red Letter Application Form]
platform_version: "BIP"
source_type: community-api-docs
---

# 红字确认单确认

> `ContentType	application/json` 请求方式	POST | 分类: Red Letter Application Form (TAX)


## 1. 请求说明

请求域名	动态域名，获取方式详见 获取租户所在数据中心域名

请求地址	/yonbip/tax/api/etax/redinfo-apply/confirm

请求方式	POST

ContentType	application/json

应用场景	开放API

事务和幂等性	无

用户身份	支持传递普通用户身份，详细说明见开放平台用户认证接入规范

来源	系统级


## 2. 请求参数

| 名称 | 类型 | 参数位置 | 必填 | 描述 |
| access_token | string | query | 是 | 调用方应用token |
Body参数

| 名称 | 类型 | 数组 | 必填 | 描述 |
| redApplyNo | string | 否 | 是 | 红字申请单号 示例：1232556 |
| qrlx | string | 否 | 是 | 确认类型，Y:同意 N:不同意 示例：Y |
| nsrsbh | string | 否 | 是 | 纳税人识别号 示例：1225522566 |
| allElcUserName | string | 否 | 否 | 数电发票用户名，sm4加密 示例：AA#@# |
| allElcPassWord | string | 否 | 否 | 数电发票密码，sm4加密 示例：ERCFCAC |

## 3. 请求示例

Url: /yonbip/tax/api/etax/redinfo-apply/confirm?access_token=访问令牌
Body: {
	"redApplyNo": "1232556",
	"qrlx": "Y",
	"nsrsbh": "1225522566",
	"allElcUserName": "AA#@#",
	"allElcPassWord": "ERCFCAC"
}

## 4. 返回值参数

| 名称 | 类型 | 数组 | 描述 |
| billtype | string | 否 | 事项类型, 1:销售发票、2:其它应收事项、3:销售发票(红字)、4:其它应收事项(红字)、5:订单日报、6:内部交易结算单、7:收款单、8:其它应付事项、9:客户退款单、10:付款单、11:供应商退款单、12:转账单、13:汇率损益单、14:外币兑换单、 |
| creator | string | 否 | 创建人 |
| creator | string | 否 | 创建人 |
| displayCode | string | 否 | 异常码 |
| displayCode | string | 否 | 异常码 |
| code | string | 否 | 状态码 示例：0000 |
| message | string | 否 | 信息说明 示例：操作成功 |

## 5. 正确返回示例

{
	"code": "0000",
	"message": "操作成功"
}

## 6. 业务异常码

暂时没有数据哦~


## 7. 错误返回码

9999	申请单号对应的红字信息表不存在	返回错误码


## 9. 接口变更日志

序号	修改时间	变更内容概要

1	2025-12-09

更新

请求说明

新增

请求参数 allElcUserName

新增

请求参数 allElcPassWord

2	2025-07-03

更新

请求说明


## 1. 请求说明


## 2. 请求参数


## 3. 请求示例


## 4. 返回值参数


## 5. 正确返回示例


## 6. 业务异常码


## 7. 错误返回码


## 9. 接口变更日志

