...
>  请求方式	GET | Testing Method (IMCS)


## 1. 请求说明

请求域名	动态域名，获取方式详见 获取租户所在数据中心域名
请求地址	/yonbip/imcs/detectionmethod/detail
请求方式	GET
...
id	long	query	否	检测方法详情ID, ID和编码不能同时为空，同时存在时以id为准    示例: 1513716839471906825
code	string	query	否	检测方法编码, 取样方式ID和取样方式编码不能同时为空，同时存在时以id为准    示例: ksw0802

## 3. 请求示例

Url: /yonbip/imcs/detectionmethod/detail?access_token=访问令牌&id=1513716839471906825&code=ksw0802
...
type	string	否	分类 示例：test123
edition	string	否	版本 示例：1.0
id	long	否	检测方法数据id 示例：1513716727802757000
code	string	否	检测方法编码 示例：ksw0802
name	string	否	检测方法名称 示例：ksw0802
validFrom	string	否	有效期从 示例：2022-08-02
expiryDate	string	否	有效期止 示例：2022-08-02
