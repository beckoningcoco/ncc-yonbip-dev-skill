...
请求地址	/yonbip/ctm/api/settlement/batch/feedback
...
| statementdetailstatus | int | 否 | 是 | 结算明细状态 3：结算成功 4：结算失败 7:部分成功；部分成功时可以增加代发明细数据进行回写 |
| actualExchangePaymentRate | string | 否 | 否 | 实际结算汇率 示例：7.23666 |
| actualExchangePaymentAmount | string | 否 | 否 | 实际结算金额 示例：72.86 |
| natRate | string | 否 | 否 | 本币汇率 示例：7.236 |
| natAmt | string | 否 | 否 | 本币金额 示例：78.23 |
...
| sourceUnique | string | 否 | 否 | 来源主键id |

## 3. 请求示例

Url: /yonbip/ctm/api/settlement/batch/feedback?access_token=访问令牌
Body: [{
	"settleDetailAId": "1741573848087134216",
	"statementdetailstatus": 0,
	"actualExchangePaymentRate": "7.23666",
	"actualExchangePaymentAmount": "72.86",
	"natRate": "7.236",
	"natAmt": "78.23",
...
| data | object | 否 | 返回结果 |
| count | string | 否 | 总数量 |
| successCount | string | 否 | 保存成功数量 示例：8 |
| failCount | string | 否 | 保存失败数量 示例：2 |
| messages | object | 是 | 成功数据库 |
