...
请求地址	/yonbip/tax/yonbip-fi-taxoth/api/tax-s-donation/batch-edit
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| data | object | 是 | 否 | 待编辑数据 |
| sourceUnique | string | 否 | 否 | 数据来源唯一标识 示例：111 |
| id | string | 否 | 是 | id 示例：123 |
| orgCode | string | 否 | 否 | 组织编码 示例：wwe |
| projectCode | string | 否 | 否 | 项目编码 示例：sw |
| donateDate | string | 否 | 否 | 捐赠日期(yyyy-mm-dd) 示例：2023-01-01 |
| accountTime | string | 否 | 否 | 凭证日期(yyyy-mm-dd) 示例：2023-01-01 |
...
| 小数位数:2,最大长度:15 | 否 | 否 | 捐赠金额 示例：13.00 |
| donateTargetCode | string | 否 | 否 | 捐赠对象编码 示例：wwsc |
...
Url: /yonbip/tax/yonbip-fi-taxoth/api/tax-s-donation/batch-edit?access_token=访问令牌
Body: {
	"data": [
		{
			"sourceUnique": "111",
