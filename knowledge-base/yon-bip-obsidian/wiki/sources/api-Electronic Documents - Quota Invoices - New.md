...
请求地址	/yonbip/FCC/v1/api/quota/batchInsert
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| quotaList | object | 是 | 是 | 定额发票信息列表 最大请求量：10 |
| invoiceCode | string | 否 | 是 | 发票代码 示例：12345678901234567890 |
| invoiceNum | string | 否 | 是 | 发票号码 示例：09876543210987654321 |
| invoiceNumSuf | string | 否 | 是 | 发票号码全文索引 |
| productName | string | 否 | 否 | 项目 示例：餐费 |
| totalAmount | number |
| 小数位数:2,最大长度:15 | 否 | 是 | 金额 示例：100.00 |
| receiveTicketOrganization | string | 否 | 否 | 受票组织 示例：某某公司 |
| busiType | number |
| 小数位数:0,最大长度:10 | 否 | 否 | 业务类型 10-报销 20-采购 30-不动产 40-海关缴款书 100-销售 110-销售&报销 示例：10 |
| fileName | string | 否 | 否 | 原始文件名，包含后缀名 示例：quota_invoice.pdf |
| fileUrl | string | 否 | 否 | 文件地址 示例：/files/quota_invoice_20230520.pdf |
| fileSize | number |
...
Url: /yonbip/FCC/v1/api/quota/batchInsert?access_token=访问令牌&orgCode=
Body: {
	"quotaList": [
		{
			"invoiceCode": "12345678901234567890",
			"invoiceNum": "09876543210987654321",
			"invoiceNumSuf": "",
			"productName": "餐费",
			"totalAmount": 100,
			"receiveTicketOrganization": "某某公司",
			"busiType": 10,
			"fileName": "quota_invoice.pdf",
			"fileUrl": "/files/quota_invoice_20230520.pdf",
			"fileSize": 102400,
