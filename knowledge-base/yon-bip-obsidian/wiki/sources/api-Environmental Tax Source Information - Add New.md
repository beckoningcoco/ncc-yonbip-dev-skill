...
请求地址	/yonbip/tax/yonbip-fi-taxoth/api/ept/base/b/add
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| orgCode | string | 否 | 是 | 组织编码 示例：bydxszz |
| bdeclareTime | string | 否 | 是 | 按次申报(Y:是，N:否) 示例：N |
| mainTaxOfficeName | string | 否 | 是 | 主管税务机关名称 示例：国家税务总局福建福安经济开发区税务局 |
| projectCode | string | 否 | 否 | 项目编码 示例：PC0001 |
| bmarineEngineering | string | 否 | 否 | 从事海洋工程(Y:是，N:否)，按期必填 示例：N |
| bsewageSites | string | 否 | 否 | 城乡污水集中处理场所(Y:是，N:否)，按期必填 示例：Y |
| bgarbageSites | string | 否 | 否 | 生活垃圾集中处理场所(Y:是，N:否)，按期必填 示例：Y |
| productSites | string | 否 | 否 | 生产经营场所地址，按期必填 示例：北京市海淀区1001号 |
| ecologyDepart | string | 否 | 否 | 生态环境主管部门，按期必填 示例：北京市海淀区环境保护委员会 |
...
Url: /yonbip/tax/yonbip-fi-taxoth/api/ept/base/b/add?access_token=访问令牌
Body: [{
	"orgCode": "bydxszz",
	"bdeclareTime": "N",
	"mainTaxOfficeName": "国家税务总局福建福安经济开发区税务局",
	"projectCode": "PC0001",
	"bmarineEngineering": "N",
	"bsewageSites": "Y",
	"bgarbageSites": "Y",
	"productSites": "北京市海淀区1001号",
	"ecologyDepart": "北京市海淀区环境保护委员会",
