...
请求地址	/yonbip/EFI/FaInventory/unsubmit
...
Url: /yonbip/EFI/FaInventory/unsubmit?access_token=访问令牌
Body: {
	"data": {
...
| data | object | 否 | 返回数据 |
| id | string | 否 | 固定资产盘点表头主键 示例：1483387421116596230 |
| busiObj | string | 否 | 业务对象 示例：FaInventory |
| cBillNo | string | 否 | 单据编码 示例：FaInventory |
| verifyState | number |
| 小数位数:0,最大长度:6 | 否 | 审核状态 0开立态 1审核中 2已审核 3终止态 4驳回到制单 示例：0 |
| pubts | string | 否 | 时间戳 示例：2025-01-02 14:37:06 |
| metaFullName | string | 否 | 元数据全称 示例：EFA.inventory.FaInventory |
| fullName | string | 否 | 全称 示例：EFA.inventory.FaInventory |
| ytenantId | string | 否 | 租户id 示例：0000L6YQ8AVLFUZPXD0000 |
| _status | number |
| 小数位数:0,最大长度:10 | 否 | 状态 示例：1 |
| verifystate | number |
| 小数位数:0,最大长度:6 | 否 | 审核状态 0开立态 1审核中 2已审核 3终止态 4驳回到制单 示例：0 |
| FaInventoryDetailList | object | 是 | 盘点单子表 |
...

{
	...
	"data": {
		...
		"metaFullName": "EFA.inventory.FaInventory",
		"fullName": "EFA.inventory.FaInventory",
