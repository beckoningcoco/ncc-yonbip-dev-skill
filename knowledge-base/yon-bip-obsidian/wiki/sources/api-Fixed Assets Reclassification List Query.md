...
> `ContentType	application/json` 请求方式	POST | 分类: Fixed Asset Category Reclassfication (EFI)
...
请求地址	/yonbip/EFI/ReclassBill/list
...
| code | string | 否 | 否 | 重分类编码 示例：ZCDR000006 |
| assetId_code | string | 否 | 否 | 固定资产编码 示例：112233 |
| creator | string | 否 | 否 | 创建人名称 示例：昵称-Ian |
| verifyState | string | 否 | 否 | 审核状态, 枚举值(0:初始开立,1:审批中,2:审批完成,3:不通过流程终止,4:驳回到制单) 示例：1 |
| auditor | string | 否 | 否 | 审核人名称 示例：昵称-Ian |
| busiPostStatus | string | 否 | 否 | 过账状态,枚举值(0:初始值,1:过账成功,2:过账失败,3:过账中,4:不过账) 示例：1 |
| simple | object | 否 | 否 | simple |
| open_vouchdate_begin | string | 否 | 否 | 重分类开始日期,(格式yyyy-MM-dd) 示例：2022-09-02 |
| open_vouchdate_end | string | 否 | 否 | 重分类结束日期,(格式yyyy-MM-dd) 示例：2022-09-02 |
...
| open_pubts_begin | string | 否 | 否 | pubts开始时间(大于等于), 格式为:yyyy-MM-dd HH:mm:ss 示例：2024-03-31 00:00:00 |
| open_pubts_end | string | 否 | 否 | pubts结束时间(小于等于), 格式为:yyyy-MM-dd HH:mm:ss 示例：2024-03-31 23:59:59 |

## 3. 请求示例

Url: /yonbip/EFI/ReclassBill/list?access_token=访问令牌
Body: {
	"pageIndex": "1",
	"pageSize": "10",
	...
	"simple": {
		...
		"open_pubts_begin": "2024-03-31 00:00:00",
		"open_pubts_end": "2024-03-31 23:59:59"
	}
}
