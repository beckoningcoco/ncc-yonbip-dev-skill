...
请求地址	/yonbip/QMS_QIT/inspectorder/fullsave
请求方式	POST
...
vapplycheckcode	string	否	是	来源单据号：需要在系统中存在。 示例：LLQJ202308040006
code	string	否	否	检验单号：优先按编码规则，编码规则若为自动编码（不可改），按自动编码规则生成，若为自动编码（可改），以输入为准 若为手工编码，以输入为准，后台保存时校验必输。 示例：LLJY202301010001
trantype	string	否	否	交易类型id：编码和id 任一不为空；在库检验传值的交易类型参数中在库检验类型必须为临时检验；工序检验时按工序检验单的默认交易类型赋值；产品检验时按产品检验单的默认交易类型赋值。 示例：2302300205764864
trantypeCode	string	否	否	交易类型编码：编码和id 任一不为空；在库检验传值的交易类型参数中在库检验类型必须为临时检验；工序检验时按工序检验单的默认交易类型赋值；产品检验时按产品检验单的默认交易类型赋值。 示例：001
checkPlanDetailId	string	否	否	检验方案详情id：编码和id均存在时以id为准。传入的检验方案需符合物料检验方案匹配规则。
checkPlanCode	string	否	否	检验方案编码
inspectDate	date
格式:yyyy-MM-dd HH:mm:ss	否	否	检验开始日期 示例：2023-11-02 00:00:00
inspecter	string	否	否	检验员id 示例：1503246482288410629
inspecterCode	string	否	否	检验员编码
destroySimpleNum	number
小数位数:8,最大长度:28	否	否	样本破坏数量 示例：1
destroySimpleMagType	number
小数位数:0,最大长度:1	否	否	破坏样本处理方式：0-丢弃、1-按废品管理，样本破坏数量大于0时必输。 示例：0
destroySimpleResp	number
小数位数:0,最大长度:1	否	否	破坏样本承担方：0-我方、1-供应商，样本破坏数量大于0且检验类型=采购检验/委外检验时必输。 示例：0
...
Url: /yonbip/QMS_QIT/inspectorder/fullsave?access_token=访问令牌
Body: {
	"data": {
		"resubmitCheckKey": "",
		"vapplycheckcode": "LLQJ202308040006",
		"code": "LLJY202301010001",
		"trantype": "2302300205764864",
		"trantypeCode": "001",
		"checkPlanDetailId": "",
		"checkPlanCode": "",
		"inspectDate": "2023-11-02 00:00:00",
		"inspecter": "1503246482288410629",
		"inspecterCode": "",
		"destroySimpleNum": 1,
		...
		"qms_qit_incominspectorder_bList": [
			{
				...
				"inspectorId": "",
				"inspectorCode": "",
				"inspectDate": "2026-06-07 13:18:32",
				"inspectTime": "2026-06-07 13:18:32",
				"note": "",
				"coord": true,
				"qmsEquipIdName": "",
		...
		"qms_qit_incominspectorder_resultList": [
			{
				...
				"stockStatusCode": "",
				"inStockBatchCode": "",
				"produceDate": "2026-06-07 13:18:32",
				"validityDate": "2026-06-07 13:18:32",
