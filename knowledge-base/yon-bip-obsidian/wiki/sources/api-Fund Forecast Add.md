...
请求地址	/yonbip/ctm/fundpredictionpool/predictiondatainterface
...
| receiptType | short | 否 | 否 | 收支类型，枚举值：1 收入、2 支出；isBeginReceType为0 必传 示例：1 |
...
| businessDetailType | string | 否 | 否 | 业务单据明细类型，枚举值：交易成交登记(176)、交易平仓登记(1001)、交易交割登记(1002)、申购登记(171)、投资赎回(172)、投资收息(173)、投资付费(174)、投资分红(175)、融入登记(167)、融资还本(168)、融资付息(169)、融资付费(170) 示例：100 |
...
| relBusinessDetailnum | string | 否 | 否 | 来源业务单据编号 示例：来自于前端系统，前端系统如果需要标识本数据(B)为A数据的“实际发生数”，则需要在本字段传入A数据的“业务单据明细ID” |
| channel | number |
| 小数位数:0,最大长度:6 | 否 | 否 | 收付渠道.1-财务公司 2-境内外部银行 3-华盛公司 4-境外外部银行 示例：1 |
| type | number |
| 小数位数:0,最大长度:10 | 否 | 否 | 收付类型.3-集团内部内转 4-集团外部内转 5-集团内部外转 6-集团外部外转 示例：6 |
| purpose | string | 否 | 否 | 用途 示例：支付使用 |

## 3. 请求示例

Url: /yonbip/ctm/fundpredictionpool/predictiondatainterface?access_token=访问令牌
Body: {
	"data": [
		{
			...
			"relBusinessDetailnum": "来自于前端系统，前端系统如果需要标识本数据(B)为A数据的“实际发生数”，则需要在本字段传入A数据的“业务单据明细ID”",
			"channel": 1,
			"type": 6,
			"purpose": "支付使用"
		}
