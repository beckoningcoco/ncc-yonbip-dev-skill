---
title: "集团司库数据上报银行贷款明细新增"
apiId: "2183253458579619840"
apiPath: "请求方式	POST"
...
请求地址	/yonbip/ctm/submit/api/group/bank/detail/insert
请求方式	POST
...
loanDay	string	否	是	放款日 yyy-MM-dd 示例：2025-01-01
loanTerm	number
小数位数:-1,最大长度:32	否	是	贷款期限
rateLevel	number
小数位数:2,最大长度:32	否	是	利率水平
loanBalance	number
小数位数:-1,最大长度:32	否	是	贷款余额
financeArea	string	否	是	国家地区
sourceId	string	否	是	外部数据来源id

## 3. 请求示例

Url: /yonbip/ctm/submit/api/group/bank/detail/insert?access_token=访问令牌
Body: {
	"dataDate": "2025-01-01",
	"submitType": "BANK",
	"detailList": [
...
sourceIds	string	是	存储成功的外部数据id
