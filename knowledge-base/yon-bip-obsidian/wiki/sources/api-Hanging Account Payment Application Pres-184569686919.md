...
请求地址	/yonbip/znbz/rbsm/api/bill/paymentapply/save
请求方式	POST
...
code	string	否	否	单据号(设置手工编号时必输，自动编号时输入无效) 示例：FKSQ23102300002
vouchdate	string	否	是	单据日期(格式：yyyy-MM-dd) 示例：2023-10-23 00:00:00
bustype	string	否	是	交易类型 示例：1658463224516313092
isdeferpaymentapply	string	否	否	是否分摊(true：是，false:否) 示例：false
pk_handlepsn	string	否	是	申请人(支持id和code) 示例：1549357070803795972
vhandledeptid	string	否	是	申请人部门(支持id和code) 示例：1524154726676430853
chandleorg	string	否	是	申请人组织(支持id和code) 示例：1524154503347568646
vnatcurrency_moneyDigit	BigDecimal	否	否	组织本币金额精度 示例：2
nnatbaseexchrate	BigDecimal	否	否	组织本币企业汇率 示例：1
vfinacedeptid	string	否	是	费用承担部门(支持id和code) 示例：1524154726676430853
cfinaceorg	string	否	是	费用承担组织(支持id和code) 示例：1524154503347568646
caccountorg	string	否	是	会计主体(支持id和code) 示例：1524154503347568646
vreason	string	否	是	申请说明 示例：ceshi
ismulticurrency	string	否	否	是否多币种(true:是;false:不是) 示例：true
vnatcurrency	string	否	是	组织本币(支持id和code) 示例：1523413904973627420
vcurrency	string	否	是	报销币种(支持id和code) 示例：1523413904973627420
vcurrency_moneyDigit	int	否	否	报销币种金额精度 示例：2
vnatexchratetype	string	否	是	汇率类型(支持id和code) 示例：0000L6UFPA4E2D845F0000
vnatexchratetype_digit	int	否	否	汇率精度 示例：6
dnatexchratedate	string	否	否	组织本币汇率日期(格式：yyyy-MM-dd) 示例：2023-10-23 00:00:00
nnatexchrate	BigDecimal	否	是	组织本币汇率 示例：1
vattachmentass	string	否	否	单据附件id 示例：a0cb6413cb2f4034b5e499731e42b917
creatorId	string	否	否	创建人ID(不传值默认YonSuite默认用户) 示例：1549357079382196226
creator_code	string	否	否	创建人编码(不传值默认YonSuite默认用户)
creator	string	否	否	创建人 示例：昵称-任文君
createDate	string	否	否	创建日期 示例：2023-10-23 20:15:12
createTime	string	否	否	创建时间 示例：2023-10-23 20:15:12
paymentapplysettleinfos	object	是	是	结算表体信息
pk_expensebill	string	否	否	源单单据主键
_status	string	否	是	操作标识, Insert:新增、Update:更新 示例:Insert 示例：Insert
id	long	否	否	单据id(_status为Update更新时必填)

## 3. 请求示例

Url: /yonbip/znbz/rbsm/api/bill/paymentapply/save?access_token=访问令牌
Body: {
	"data": {
		"resubmitCheckKey": "",
		"code": "FKSQ23102300002",
		...
		"nnatexchrate": 1,
		"vattachmentass": "a0cb6413cb2f4034b5e499731e42b917",
		"creatorId": "1549357079382196226",
		"creator_code": "",
		"creator": "昵称-任文君",
		"createDate": "2023-10-23 20:15:12",
		"createTime": "2023-10-23 20:15:12",
		"paymentapplysettleinfos": [
			{
				...
				"vnatcurrency_moneyDigit": 2,
				"igathertype": 1,
				"pk_customer": 2019215903510784,
				"pk_customer_code": "",
				"pk_customerbank": 2019215903527168,
				"pk_customerbank_code": "",
				"pk_handlepsn": "2019215903527168",
				"pk_handlepsnbank": "2019215903527168",
...
pk_handlepsn_name	string	否	申请人名称 引用：员工 Staff 示例：任文君
...
caccountorg_name	string	否	会计主体名称 引用：会计主体 FinanceOrgVO 示例：费控A
