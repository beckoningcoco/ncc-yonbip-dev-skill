...
请求地址	/yonbip/hrcloud/checkStaffingByStaffs
请求方式	POST
...
Body参数
名称	类型	数组	必填	描述
staffList	object	是	是	员工列表
id	string	否	否	员工ID或编码 示例：2511871812849920
...
birthPlace	string	否	否	出生地 传入id或code 示例：73737271222231
...
dataCreateSource	string	否	否	数据来源，仅新增赋值，不可更新 示例：openApi
dataCreateBillid	string	否	否	数据来源单据号，仅新增赋值，不可更新 示例：123456
...
Url: /yonbip/hrcloud/checkStaffingByStaffs?access_token=访问令牌
Body: {
	"staffList": [
		{
			"id": "2511871812849920",
...
data	object	否	编制服务返回的结果
msg	string	否	消息 示例：操作成功
code	number
小数位数:0,最大长度:10	否	状态码(200-299 操作成功编码；300-399 警告编码; 400-499 错误编码) 示例：200
detail	string	否	详细错误信息 示例：未找到下达/分解数据
