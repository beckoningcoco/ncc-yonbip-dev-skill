---
title: "幂等发送工作通知"
apiId: "b26e711d23284329a84baef56bdae6df"
apiPath: "请求方式	POST"
...
请求地址	/yonbip/uspace/rest/openapi/idempotent/work/notify/push
请求方式	POST
...
appId	string	否	否	应用id, 取值:友空间的appid 或 工作台的服务编码serviceCode, 用于:移动端消息分类，用于后续标记已读的业务 示例：0
tabId	string	否	否	移动端自定义分类， 适配没有appid需要做分类的场景
esnData	string	是	否	业务属性集合 其中fromId 标记已读时必传 业务id 示例：{}
attributes	string	是	否	自定义拓展属性 示例：{}
catcode1st	string	否	否	分类id
serviceCode	string	否	否	服务编码，用于展示消息的icon,查找位置：工作台>我的应用>服务列表 示例：XXX

## 3. 请求示例

Url: /yonbip/uspace/rest/openapi/idempotent/work/notify/push?access_token=访问令牌
Body: {
