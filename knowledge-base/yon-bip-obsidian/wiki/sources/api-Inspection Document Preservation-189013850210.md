...
请求地址	/yonbip/QMS_QIT/inspectorder/save
请求方式	POST
...
inspectDate	Date	否	否	检验开始日期：无值时按当前业务日期赋值。 示例：2023-01-01
pk_inspecter	string	否	否	检验员id 示例：1539608448409796618
pk_inspecter_code	string	否	否	检验员编码
destroySimpleNum	BigDecimal	否	否	样本破坏数量 示例：1
destroySimpleMagType	string	否	否	破坏样本处理方式：0-丢弃、1-按废品管理，样本破坏数量大于0时必输。 示例：0
destroySimpleResp	string	否	否	破坏样本承担方：0-企业自检、1-供应商、2-第三方机构，样本破坏数量大于0且检验类型=采购检验/委外检验时必输。 示例：0

## 3. 请求示例

Url: /yonbip/QMS_QIT/inspectorder/save?access_token=访问令牌
Body: {
	"data": {
		"resubmitCheckKey": "",
		...
		"pk_inspecter": "1539608448409796618",
		"pk_inspecter_code": "",
		"destroySimpleNum": 1,
...
originalRecord	int	否	原始记录：1-是、0-否。 示例：0
...
manufacture_date	DateTime	否	生产日期 示例：2023-09-12 00:00:00
