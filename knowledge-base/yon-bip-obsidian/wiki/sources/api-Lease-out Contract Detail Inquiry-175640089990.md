...
## 2. 支持的查询条件及操作符：合同编码（等于）、合同号（等于）；

3. 返回信息包括租出合同表头、表体信息（字段与出厂预置的WEB端UI模板默认显示字段一致）；

## 1. 请求说明

请求域名	动态域名，获取方式详见 获取租户所在数据中心域名
请求地址	/yonbip/am/rentoutcard/detail
请求方式	POST
...
bill_code	string	否	否	合同编码（合同编码或合同号，二者需起码必填一个值） 示例：ZCCZ202303100001
contract_code	string	否	否	合同号（合同编码或合同号，二者需起码必填一个值) 示例：12322111
bodyvos	string	是	否	需要返回的子表（rent_basic,rent_detailed,rent_calculation），不填返回全部

## 3. 请求示例

Url: /yonbip/am/rentoutcard/detail?access_token=访问令牌
Body: {
	"externalData": {
		"bill_code": "ZCCZ202303100001",
		"contract_code": "12322111",
		"bodyvos": [
			""
		]
	}
