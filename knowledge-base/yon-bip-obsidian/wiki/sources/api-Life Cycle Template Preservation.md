...
请求地址	/yonbip/digitalModel/lifeCycleTemplate/save
...
| simplifiedName | string | 否 | 否 | 简体中文 示例：0114 |
| englishName | string | 否 | 否 | 英文 示例：0114 |
| traditionalName | string | 否 | 否 | 繁体中文 示例：0114 |
| stopstatus | string | 否 | 否 | 是否停用（1：是；0：否）若为空默认为 0 示例：1 |
| lifeCycleDetail | object | 是 | 否 | 生命周期模板明细信息 |
| materialStatusCode | string | 否 | 否 | 物料状态编码 示例：21 |
| isDefault | string | 否 | 否 | 是否默认（1：是；0：否）若为空 默认为 0 示例：0 |
| entityStatus | string | 否 | 否 | 操作符（新增：Insert；修改：Update；删除：Delete）若为空默认为Insert 示例：Insert |

## 3. 请求示例

Url: /yonbip/digitalModel/lifeCycleTemplate/save?access_token=访问令牌
Body: {
	"code": "0114",
	"name": {
		"simplifiedName": "0114",
