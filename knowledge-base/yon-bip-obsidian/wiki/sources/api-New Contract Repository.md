...
请求地址	/yonbip/CLMS/v1.0/contractLib/create
...
| _status | string | 否 | 是 | 单据状态 目前仅支持Insert 示例：Insert 默认值：Insert |
| sourceSys | string | 否 | 是 | 合同来源 1-CLM合同 2-旗舰版-采购 3-旗舰版-销售 4-高级版-采购 5-高级版-销售 6-第三方 示例：1 |
| title | string | 否 | 否 | 合同名称 示例：我的open-api12-405 |
...
| status | string | 否 | 是 | 状态 3-已审核 8-生效 默认已审核 示例：3 |
| ctPeriod | string | 否 | 否 | 合同期限 0-固定期限 1-无固定期限 示例：0 |
...
| signAddress | string | 否 | 否 | 签订地点 示例：北京 |
| performAddress | string | 否 | 否 | 履约地点 示例：用友 |
| ctSummary | string | 否 | 否 | 合同概要 示例：概要 |
| appendix | string | 否 | 否 | 合同附件 |
| memo | string | 否 | 否 | 备注 示例：备注1 |
| currencyId | string | 否 | 否 | 币种ID(本币)，币种编码(本币)和币种ID(本币)同时存在时，以ID为准 示例：1817179643556921384 |
| currencyId___code | string | 否 | 否 | 币种编码(本币) 示例：EUR |
| mny | BigDecimal | 否 | 否 | 本币合同金额 示例：100 |
| oriCurrencyId | string | 否 | 否 | 原币 示例：14141341431 |
| oriCurrencyId___code | string | 否 | 否 | 原币编码 |
| oriHaveTaxMny | BigDecimal | 否 | 否 | 原币含税金额 示例：1431 |
| oriNoTaxMny | BigDecimal | 否 | 否 | 原币无税金额 示例：1431 |
| oriTaxAmount | BigDecimal | 否 | 否 | 原币合同税额 示例：1431 |
| exchangeRate | BigDecimal | 否 | 否 | 汇率 示例：100 |
| rateDate | Date | 否 | 否 | 汇率日期 示例：2021-01-01 |
| rateType | string | 否 | 否 | 汇率类型ID 示例：134234289 |
| rateType___code | string | 否 | 否 | 汇率类型编码 |
| beginningFlag | string | 否 | 否 | 期初标记 0-否 1-是 示例：134234289 |
| urgentLevel | string | 否 | 否 | 紧急程度 1-一般 2-紧急 3-特急 示例：2 |
| signOrgid | string | 否 | 否 | 签约组织ID，签约组织编码和签约组织ID至少录入一项，签约组织编码和签约组织ID同时存在时，以ID为准 示例：1817417739359748103 |
| signOrgid___code | string | 否 | 否 | 签约组织编码 示例：qwert |
| clmContractFileId | string | 否 | 否 | 合同文本对象Id 示例：1817417739359748103 |
| clmContractFileId___code | string | 否 | 否 | 合同文本对象编码 |
| orgId | string | 否 | 否 | 承办组织ID，承办组织编码和承办组织ID同时存在时，以ID为准 示例：1817417739359748103 |
| orgId___code | string | 否 | 否 | 承办组织编码 示例：qwert |
| isHaveTax | string | 否 | 是 | 合同金额是否含税 0-否 1是 示例：1 |
| billtypeId | string | 否 | 否 | 单据类型ID 示例：1862442766275544737 |
| billtypeId___code | string | 否 | 否 | 单据类型编码 |
| bustype | string | 否 | 否 | 交易类型ID，交易类型编码和交易类型ID同时存在时，以ID为准 示例：1818179305792864264 |
| bustype___code | string | 否 | 否 | 交易类型编码 示例：WZLCGHT |
| signSubjectId | string | 否 | 否 | 承办组织ID 示例：1818179305792864264 |
| signSubjectId___code | string | 否 | 否 | 承办组织编码 |
| origcurrencyId | string | 否 | 否 | 币种ID，币种编码和币种ID同时存在时，以ID为准 示例：1817179643556921384 |
| origcurrencyId___code | string | 否 | 否 | 币种编码 示例：EUR |
| econtractTempCode | string | 否 | 否 | 合同模板编码 示例：1853639388987654150 |
| econtractVersion | string | 否 | 否 | 合同模板版本号 示例：000001 |
| econtractTempId | string | 否 | 否 | 合同模板ID |
| econtractTempId___code | string | 否 | 否 | 合同模板编码 |
| offStandardFile | string | 否 | 否 | 非标准合同文本 示例：djgldsjg |
| offStandardCause | string | 否 | 否 | 选择非标准合同的原因 示例：djgldsdsfjg |
| invaliCause | string | 否 | 否 | 合同终止原因 示例：djgldsdsfjg |
| finishFile | string | 否 | 否 | 定稿文件/送签文件 示例：定稿文件 |
| isEcontract | string | 否 | 否 | 是否为电子合同 0-否 1-是 示例：1 |
| label | string | 否 | 否 | 合同标签 示例：sdfsf |
| taxAmount | BigDecimal | 否 | 否 | 本币税额 示例：100 |
| noTaxMny | BigDecimal | 否 | 否 | 本币无税金额 示例：100 |
| haveTaxMny | BigDecimal | 否 | 否 | 本币含税金额 示例：100 |
| changeTimes | int | 否 | 否 | 变更次数 示例：100 |
| changeCreator | string | 否 | 否 | 变更申请人 示例：13413414 |
| changeCreator___code | string | 否 | 否 | 变更申请人编码 |
| finalizeStatus | string | 否 | 否 | 定稿状态 0-未定稿 1-已定稿 示例：1 |
| signEcontractFile | string | 否 | 否 | 签署电子版 示例：定稿文件 |
| changeStatus | string | 否 | 否 | 变更状态 1-未提交 2-变更中 3-变更不通过 4-变更完成 0-未变更 示例：1 |
| signPaperFile | string | 否 | 否 | 签署纸质回传版 示例：定稿文件 |
| repleFile | string | 否 | 否 | 补充协议文件 示例：定稿文件 |
| contractJournalId | string | 否 | 否 | 合同台账ID 示例：定稿文件 |
| contractFileStatus | string | 否 | 否 | 合同归档状态 0-未归档 1-已归档 示例：1 |
| bWatermark | string | 否 | 否 | 是否无水印 0-无 1-有 示例：1 |
| changeContractId | string | 否 | 否 | 变更合同id 示例：13413414 |
| changeContractId___code | string | 否 | 否 | 变更合同编码 |
| signProcessId | string | 否 | 否 | 签署流程id 示例：13413414 |
| businessFileId | string | 否 | 否 | 合同文本附件 id 示例：13413414 |
| settleOrgid | string | 否 | 否 | 结算组织ID，结算组织编码和结算组织ID同时存在时，以ID为准 示例：1817417739359748103 |
...
| groupId | string | 否 | 否 | 合同分组ID 示例：1815417739359748122 |
| groupId___code | string | 否 | 否 | 合同分组编码 |
...
| physicalStamp | string | 否 | 否 | 物理印章ID，物理印章编码和物理印章ID同时存在时，以ID为准 示例：1817180167541359459 |
| physicalStamp___code | string | 否 | 否 | 物理印章编码 示例：system_0004 |
| stampStatus | string | 否 | 否 | 用印状态 示例：1 |
...
| secretLevel | string | 否 | 否 | 密级 1-公开 2-限制 3-秘密 4-机密 5-绝密 示例：0 |
| importantLevel | string | 否 | 否 | 重要程度 1-一般 2-普通 3-重大 示例：2 |
...
| signPriority | string | 否 | 否 | 签署优先级 示例：1 |
| backDateCause | string | 否 | 否 | 倒签原因 |
| createMethod | string | 否 | 是 | 合同生成方式 0-非标准合同1-标准合同 示例：1 |
...
| econtractFile | string | 否 | 否 | 合同文本信息 |
...
| srcContractId | string | 否 | 否 | 来源合同ID，来源合同编码和来源合同ID同时存在时，以ID为准 示例：1815417739359748122 |
| srcContractId___code | string | 否 | 否 | 来源合同编码 示例：2023000046 |
| settlementBasis | string | 否 | 否 | 结算依据 示例：依据物料明细进行结算 |
| paidOrigMny | BigDecimal | 否 | 否 | 已付款金额 |
| sourceId | string | 否 | 否 | 合同来源id 示例：1873853918878892035 |
| code | string | 否 | 否 | 合同编码 示例：2023000048 |
| sourceBusiObj | string | 否 | 否 | 来源单据业务对象编码 |
| sourceCode | string | 否 | 否 | 来源单据编码 |
| bizFlowMakeBillCode | string | 否 | 否 | 单据转换规则编码 示例：clmContract2ContractJournal |
...
| archivedStatus | string | 否 | 否 | 归档状态 0-未归档 1-已归档 示例：0 |
| contractLibCounterpartList | object | 是 | 是 | 合同库相对方 |
| contractLibChangeList | object | 是 | 否 | 合同库变更信息 |

## 3. 请求示例

Url: /yonbip/CLMS/v1.0/contractLib/create?access_token=访问令牌
Body: {
	"data": {
		"_status": "Insert",
		"sourceSys": "1",
		...
		"contractLibChangeList": [
			{
				...
				"changeCreateTime": "2021-01-01 11:00:00",
...
| message | string | 否 | 状态信息 示例：操作成功 默认值：true |
| data | object | 否 | data |
...
| rateDate | Date | 否 | 汇率日期 示例：2021-01-01 |
		...
		"contractLibCounterpartList": [
			{
				...
				"creator": "",
				"modifier": "",
				"createTime": "2026-06-07 11:58:41",
				"modifyTime": "2026-06-07 11:58:41",
				"pubts": "2026-06-07 11:58:41"
			}
		],
		"contractLibChangeList": [
			{
