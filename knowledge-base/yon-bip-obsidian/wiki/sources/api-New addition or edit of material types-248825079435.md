...
请求地址	/yonbip/RDC/materialClassify/api/v1/singleSave
请求方式	POST
...
cOwnOrgId_name	string	否	否	所属组织名称 示例：test001
...
sort	string	否	是	排序号 示例：0 默认值：0
path	string	否	是	路径 示例：| 默认值：|
level	number
小数位数:0,最大长度:10	否	是	层级 示例：1 默认值：1
isEnd	string	否	是	是否末级(true-是) 示例：true 默认值：true
dr	string	否	是	删除标志(0-未删除) 示例：0 默认值：0
...
Url: /yonbip/RDC/materialClassify/api/v1/singleSave?access_token=访问令牌
Body: {
