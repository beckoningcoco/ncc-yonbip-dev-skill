---
title: "合同涉税信息-非框架合同信息、印花税、契税税源联动新增"
apiId: "2029719630672560129"
apiPath: "请求方式	POST"
...
请求地址	/yonbip/tax/yonbip-fi-taxbd/bd/api/contract/add-contract-deed
请求方式	POST
...
name	string	否	否	合同名称（上送，默认等于合同编码） 示例：合同名称
contractIdentifiNumber	string	否	否	合同识别号 示例：2312321
diworkProjectCode	string	否	否	项目编码 示例：AD1
sisNonTaxContract	string	否	是	不征税合同 示例：Y
isContainsTax	string	否	是	是否含税金额计税 示例：Y
...
contractLegalCode	string	否	否	合同法律类别编码 示例：112233
billTypeCode	string	否	否	单据类型编码 示例：0011
transTypeCode	string	否	否	交易类型编码 示例：0011
stampTaxItem	string	否	是	印花税目 示例：05_024
...
amount	number
小数位数:2,最大长度:15	否	否	发生金额（含税） 示例：98
amountExcludingTax	number
小数位数:2,最大长度:15	否	否	发生金额（不含税） 示例：100
contractOriginId	string	否	否	合同来源的id 示例：12345678910
houseAccountCode	string	否	否	房产税源编号 示例：FCSY20240723000007
landCode	string	否	否	土地税源编号 示例：TDSY20240724000100
realEstateRegistrationType	string	否	是	不动产登记类型，枚举,{0:增量房交易,1:存量房交易,2:土地出让,3:土地转让} 示例：0
...
province	string	否	否	省 示例：北京市
city	string	否	否	市 示例：市辖区
area	string	否	否	区 示例：海淀区
town	string	否	否	城镇地区 示例：万寿路街道
houseLocation	string	否	否	详细地址 示例：详细地址1234
baseRegionCode	string	否	否	行政区划代码 示例：1233
...
periodType	string	否	否	印花税填报周期 （1 -季度 3-年 4-次 ） 示例：4
...
Url: /yonbip/tax/yonbip-fi-taxbd/bd/api/contract/add-contract-deed?access_token=访问令牌
Body: [{
	"orgCode": "hehehe",
	"code": "hecode",
	"name": "合同名称",
	"contractIdentifiNumber": "2312321",
	...
	"houseAccountCode": "FCSY20240723000007",
	"landCode": "TDSY20240724000100",
	"realEstateRegistrationType": "0",
...
contractDetailId	string	否	明细id 示例：321321
stampId	string	否	印花税id 示例：231321
taxDeedId	string	否	契税id 示例：66666
