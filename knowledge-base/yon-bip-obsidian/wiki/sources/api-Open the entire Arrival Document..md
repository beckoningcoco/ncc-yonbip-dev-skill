...
请求地址	/yonbip/scm/arrivalorder/batchopen
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| data | object | 是 | 是 | 入参 |
| id | long | 否 | 是 | 到货单主表id 示例：2305517240128256 |

## 3. 请求示例

Url: /yonbip/scm/arrivalorder/batchopen?access_token=访问令牌
Body: {
	"data": [
		{
			"id": 2305517240128256
...
| infos | string | 是 | 调用成功返回的信息 示例：["操作成功"] |
| failInfos | string | 是 | 调用成功返回的信息 示例：["操作失败"] |
