---
title: "委外核销调整单详情查询"
apiId: "b0250b095aee47199104eee628332d84"
apiPath: "请求方式	GET"
...
>  请求方式	GET | Subcontracting Settlement Adj (MF)


## 1. 请求说明

请求域名	动态域名，获取方式详见 获取租户所在数据中心域名
请求地址	/yonbip/mfg/verificationadjust/detail
请求方式	GET
...
id	long	query	否	核销调整单id。核销调整单id和核销调整单编码不能同时为空，id和编码同时存在，以id为准    示例: 1967600514994432
code	string	query	否	核销调整单编码。核销调整单id和核销调整单编码不能同时为空，id和编码同时存在，以id为准    示例: HXTZ202103260019

## 3. 请求示例

Url: /yonbip/mfg/verificationadjust/detail?access_token=访问令牌&id=1967600514994432&code=HXTZ202103260019
...
id	long	否	核销调整单id 示例：2569918922740224
orgId	string	否	委外组织id 示例：2325505713575680
orgName	string	否	委外组织名称 示例：资产管理公司
vendorId	string	否	委外商id 示例：2325541812343040
vendorName	string	否	委外商名称 示例：WWS1
code	string	否	单据号 示例：HX20211221011014
status	int	否	单据状态：0-开立；1-已审核 示例：1
transTypeId	string	否	交易类型id 示例：2325273783046416
transTypeName	string	否	交易类型名称 示例：HX01
transTypeCode	string	否	交易类型编码 示例：核销调整
...
creator	string	否	创建人名称 示例：Yanjha
...
verificationUserName	string	否	核销人名称 示例：Yanjha
osmInId	long	否	委外入库单id 示例：2563386232295680
purinrecordsId	long	否	委外入库单行id 示例：2563386232295681
purinrecordsCode	string	否	委外入库单号 示例：OSMI20211216000003
purinrecordsRowno	int	否	委外入库单行号 示例：1
isMerge	boolean	否	合并：false-否；true-是 示例：false
purinOrgId	string	否	库存组织id 示例：2325505713575680
purinrecordOrgName	string	否	库存组织名称 示例：资产管理公司
...
financeOrgName	string	否	会计主体组织名称 示例：资产管理公司
...
isUpdateCost	boolean	否	更新存货成本：false-否；true-是 示例：false
		...
		"verificationDetail": [
			{
				...
				"curOsmInVerificationQuantity": "10",
