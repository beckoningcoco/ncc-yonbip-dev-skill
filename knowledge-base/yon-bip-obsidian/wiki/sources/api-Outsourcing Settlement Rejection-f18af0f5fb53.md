---
title: "委外核销单弃审"
apiId: "f18af0f5fb534de19c274589006ce61e"
apiPath: "请求方式	POST"
...
请求地址	/yonbip/mfg/verification/batchunaudit
请求方式	POST
...
id	long	否	否	委外核销单ID，委外核销单ID和委外核销单号不能同时为空，ID和编码同时存在，以ID为准 示例：1967600514994432
code	string	否	否	委外核销单编码，委外核销单ID和委外核销单号不能同时为空，ID和编码同时存在，以ID为准 示例：WWDDJT202108040002

## 3. 请求示例

Url: /yonbip/mfg/verification/batchunaudit?access_token=访问令牌
Body: {
	"data": [
		{
			"id": 1967600514994432,
...
messages	string	是	操作失败提示信息 示例：["自动核销不允许修改"]
