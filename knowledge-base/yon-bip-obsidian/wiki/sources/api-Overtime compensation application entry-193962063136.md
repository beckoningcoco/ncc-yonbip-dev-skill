...
请求地址	/yonbip/hrcloud/time/overtime/instead/save
请求方式	POST
...
staffId	string	否	否	申请人员工id（可在假勤档案节点查询），userId和staffId两则必传其一。 示例：1737740314994540546
userId	string	否	否	申请人用户id（可在员工信息节点查询），userId和staffId两则必传其一 示例：18b1ffca-b45c-453d-94c7-220aca807b5d
code	string	否	否	单据编码
approveStatus	number
小数位数:2,最大长度:10	否	否	1表示待提交，4表示审批通过 示例：4
...
userId	string	否	否	用户id（可在员工信息节点查询），userId和staffId两则必传其一 示例：18b1ffca-b45c-453d-94c7-220aca807b5d
staffId	string	否	否	员工id（可在假勤档案节点查询），userId和staffId两则必传其一 示例：1737740314994540546
...
Url: /yonbip/hrcloud/time/overtime/instead/save?access_token=访问令牌
Body: {
...
message	string	否	返回信息，如果成功，message会返回加班主表id 示例：test_20240222_main_1
data	object	否	返回结果集
id	string	否	申请主表id，和传入参数保持一致 示例：test_20240222_main_1
msg	string	否	错误信息
status	number
小数位数:0,最大长度:10	否	状态码，0=写入成功，1=写入失败 示例：0
