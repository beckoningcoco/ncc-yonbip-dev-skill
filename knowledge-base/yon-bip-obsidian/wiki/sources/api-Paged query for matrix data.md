...
请求地址	/yonbip/digitalModel/openapi/query/matrix/data
...
| pageSize | number |
| 小数位数:0,最大长度:10 | 否 | 是 | 每页数量（最大1000） |

## 3. 请求示例

Url: /yonbip/digitalModel/openapi/query/matrix/data?access_token=访问令牌
Body: {
	"matrixId": "",
	"pageNo": 0,
	"pageSize": 0
...
| 名称 | 类型 | 数组 | 描述 |
| code | string | 否 | 接口响应编码（200表示成功） 示例：200 |
...
| errmsg | string | 否 | 接口响应内容（成功标记、报错信息等） 示例：ok |
