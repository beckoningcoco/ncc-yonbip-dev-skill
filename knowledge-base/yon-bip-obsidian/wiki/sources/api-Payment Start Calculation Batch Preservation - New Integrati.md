...
date: 2026-06-07
ingested: 2026-06-07
tags: [YonBIP, OpenAPI, Payment Start Date]
...
> `ContentType	application/json` 请求方式	POST | 分类: Payment Start Date (MD)
...
请求地址	/yonbip/digitalModel/basedoc/paystartbase/batchSave
...
| data | object | 是 | 否 | 请求体 |
| id | string | 否 | 否 | 主键 示例：1604976699624129608 |
| code | string | 否 | 是 | 编码 示例：OrderBillDate |
| name | object | 否 | 是 | 名称，根据租户开通的语种传入 |
| enable | number |
| 小数位数:0,最大长度:10 | 否 | 否 | 状态，1：启用 2：停用 示例：1 |
| order | number |
| 小数位数:0,最大长度:10 | 否 | 否 | 顺序 示例：2 |
| sourceUnique | string | 否 | 是 | 源系统唯一键 示例：1604976699624129608 |

## 3. 请求示例

Url: /yonbip/digitalModel/basedoc/paystartbase/batchSave?access_token=访问令牌
Body: {
	"matchRule": "id",
	...
	"data": [
		{
			...
			"enable": 1,
			"order": 2,
			"sourceUnique": "1604976699624129608"
		}
	]
}
