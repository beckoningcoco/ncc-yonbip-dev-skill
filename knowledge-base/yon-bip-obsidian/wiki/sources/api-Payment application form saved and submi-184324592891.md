...
>  请求方式	POST | Payment Application (EFI)
...
请求地址	/yonbip/EFI/paymentApply/saveandsubmit
请求方式	POST
...
code	string	否	否	单据编号 (设置自动编码时按编码规则取值，接口不用传值；手工编码时需要传值) 示例：xx157611823880
...
Url: /yonbip/EFI/paymentApply/saveandsubmit?access_token=访问令牌
Body: {
	"data": {
		"resubmitCheckKey": "",
...
data	object	否	返回数据
id	string	否	付款申请单主键 示例：1514492081064640537
