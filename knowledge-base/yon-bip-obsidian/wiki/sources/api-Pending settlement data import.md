---
title: "待结算数据导入"
apiId: "f6970775cd9649b7a9c5371366fe01c9"
apiPath: "请求方式	POST"
...
请求地址	/yonbip/ctm/api/datasettled/recdatasettled
...
| publicPrivate | short | 否 | 否 | 对公对私：2：对公，1：对私 示例：1 |
...
| noteno | string | 否 | 否 | 票据号：期望结算方式为银行转账/现金/支票时：非必填期望结算方式的业务属性为“票据业务”：必填 示例：002 |
...
| issinglebatch | string | 否 | 是 | 单笔/批量 单笔 0 只支持单笔 0 示例：0 |
| remark | string | 否 | 否 | 备注 示例：待结算备注 |
| toAccntType | string | 否 | 否 | 对方类型 客户 1 供应商 2 员工 3 其他 4 资金业务对象 5 内部单位 6 示例：4 |
...
| accountName | string | 否 | 否 | 对方银行账户名，填入时会根据对方银行账号+对方银行账户名的组合来确定对方银行相关信息 |
...
| linkpayment | string | 否 | 否 | 是否联动支付 是1 否 0 示例：0 |
...
| csplSummaryDetail | string | 否 | 否 | 资金计划明细主键ID 示例：1232389349593485 |
...
| checkIdentificationCode | string | 否 | 否 | 财资统一对账码 |
...
| expenseitem | string | 否 | 否 | 费用项目id 示例：2185325490639872 |
| expenseitem_code | string | 否 | 否 | 费用项目编码, id与编码任意提一个 示例：费用项目编码 |
| isdirectacc | boolean | 否 | 否 | 是否直连支付 示例：false |
| innerNoteAccount_code | string | 否 | 否 | 内部票据账户 示例：02280001 |
| imputationType | number |
| 小数位数:-1,最大长度:2 | 否 | 否 | 归集类型 示例：1上收 ，2 下拨 |
| dataSettledCrossBorder | object | 否 | 否 | 待结算数据跨境信息 |
| dept | string | 否 | 否 | 部门id |
| dept_code | string | 否 | 否 | 部门编码 |
| exchangeRateTypeCode | string | 否 | 否 | 结算汇率类型编码 示例：NVY |
| exchangePaymentRate | string | 否 | 否 | 结算汇率 示例：7.312855 |
| exchangePaymentAmount | string | 否 | 否 | 预估结算金额 示例：78 |

## 3. 请求示例

Url: /yonbip/ctm/api/datasettled/recdatasettled?access_token=访问令牌
Body: {
	"data": {
		"wdataOrigin": "20",
		"billRangeStart": "",
		...
		"csplSummaryDetail": "1232389349593485",
		...
		"expenseitem": "2185325490639872",
		"expenseitem_code": "费用项目编码",
		"isdirectacc": false,
		...
		"dept": "",
		"dept_code": "",
		"exchangeRateTypeCode": "NVY",
		"exchangePaymentRate": "7.312855",
		"exchangePaymentAmount": "78"
	}
}
...
| expenseitem | string | 否 | 费用项目档案主键 示例：2163654053351680 |
...
请求参数 exchangeRateTypeCode

新增

请求参数 exchangePaymentRate

新增

请求参数 exchangePaymentAmount
