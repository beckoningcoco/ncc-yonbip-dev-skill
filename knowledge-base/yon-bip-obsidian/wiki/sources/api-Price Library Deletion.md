...
请求地址	/yonbip/EFI/pricerepositoryBll/batchDelete
...
| dataSrc | string | 否 | 是 | 数据来源 1用户新增 2成本估算 示例：1 默认值：1 |
| accbook | string | 否 | 否 | 账簿id 账簿id和账簿编码二选一必输 id和编码同时存在 以id为准 示例：1608891622525239312 |
| accbook_code | string | 否 | 否 | 账簿编码 账簿id和账簿编码二选一必输 id和编码同时存在 以id为准 示例：76 |
| accentity | string | 否 | 否 | 会计主体id 示例：1593328190065475592 |
| pubts | string | 否 | 否 | 单据时间戳,格式yyyy-MM-dd HH:mm:ss 示例：2023-11-23 19:38:32 |

## 3. 请求示例

Url: /yonbip/EFI/pricerepositoryBll/batchDelete?access_token=访问令牌
Body: {
	"data": [
		{
...
999	单据删除报错,错误原因:当前单据不是最新状态，请刷新重试	重新查询单据后再删除
