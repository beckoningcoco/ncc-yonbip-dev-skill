...
请求地址	/yonbip/RDC/openapi/prjTaskType/v1/saveOrUpdate
请求方式	POST
...
Body参数
名称	类型	数组	必填	描述
code	string	否	是	编码 示例：000024
path	string	否	否	路径 示例：2160177767707574275|
id	string	否	否	ID 示例：2160177767707574275
createorg	string	否	否	创建组织ID 示例：1985902709371305991
ownedorg	string	否	否	所属组织ID 示例：1985902709371305991
lifecycleUseId	string	否	是	生命周期模板应用ID 示例：1980667178617667749
name	string	否	是	名称 示例：交付物类型测试
lifecycleUseId_name	string	否	否	生命周期应用模板 示例：项目任务-默认模板使用
type	string	否	否	任务类别 示例：1001
state	number
小数位数:0,最大长度:10	否	否	启用状态 示例：1
parent_name	string	否	否	父节点名称
creationtime	string	否	否	创建时间 示例：2024-12-20 14:56:30
lifecycleId	string	否	是	生命周期模板ID 示例：1980667178617667691
lifecycleAttrpoolId	string	否	是	生命周期状态池ID 示例：1980667032588779557
lifecycleAttrId	string	否	是	生命周期属性ID 示例：1980667178617667702
modifiedtime	string	否	否	修改时间 示例：2024-12-20 14:57:46
level	number
小数位数:0,最大长度:10	否	否	层级 示例：1
sort	number
小数位数:0,最大长度:10	否	否	排序号 示例：10
isEnd	boolean	否	否	末级节点 示例：true
pubts	string	否	否	时间戳 示例：2024-12-20 14:57:46
taskAuthList	object	是	否	WBS可见授权
name	string	否	否	名称 示例：ggggggggggg
authPoint	string	否	否	授权点 示例：1
authSubject	string	否	否	授权主体ID 示例：1980670880874233864
rule	string	否	否	规则ID 示例：2500851575419305984
ruleExpression	string	否	否	规则 示例：编码 等于 fff
taskAuthSetList	object	是	否	WBS可见授权设置
_status	string	否	否	状态,新增：Insert; 更新：Update 示例：Insert
taskCgrDefLinkList	object	是	否	自定义项
definecode	string	否	否	自定义项编码 示例：prjTask1
_status	string	否	否	状态,新增：Insert; 更新：Update；删除：Delete 示例：Insert
outputLinkList	object	是	否	交付物类型
isDefault	string	否	否	默认交付物类型 示例：0
output_name	string	否	否	交付物类型 示例：研发物料类型（预置）
output	string	否	否	交付物类型ID 示例：1980667178594598926
code	string	否	否	编码 示例：0002
_status	string	否	否	状态,新增：Insert; 更新：Update；删除：Delete 示例：Insert
shareOrgList	object	是	否	共享组织
shareOrg	string	否	否	组织ID 示例：666666
shareOrg_name	string	否	否	组织名称 示例：企业账号级
shareOrg_fullname	string	否	否	组织名称 示例：企业账号级
shareOrg_code	string	否	否	组织编码 示例：global00
containChildren	boolean	否	否	包含下级 示例：false
_status	string	否	否	状态,新增：Insert; 更新：Update；删除：Delete 示例：Insert
colorSettingList	object	是	否	甘特图颜色配置
state	string	否	否	启用状态 示例：1
colorValue	string	否	否	任务颜色 示例：rgba(255,255,255,1)
stageName	string	否	否	阶段任务名称 示例：fff
prjCgr_name	string	否	否	项目类型 示例：22223
prjCgr	string	否	否	项目类型ID 示例：1981476891607957507
ownedorg_name	string	否	否	所属组织 示例：智石开测试组织
ownedorg	string	否	否	所属组织ID 示例：2117850633073065990
_status	string	否	否	状态,新增：Insert; 更新：Update；删除：Delete 示例：Insert
_status	string	否	否	状态,新增：Insert; 更新：Update 示例：Update

## 3. 请求示例

Url: /yonbip/RDC/openapi/prjTaskType/v1/saveOrUpdate?access_token=访问令牌
Body: {
	"code": "000024",
	...
	"modifiedtime": "2024-12-20 14:57:46",
	"level": 1,
	"sort": 10,
	"isEnd": true,
	"pubts": "2024-12-20 14:57:46",
	"taskAuthList": [
		{
			"name": "ggggggggggg",
			"authPoint": "1",
