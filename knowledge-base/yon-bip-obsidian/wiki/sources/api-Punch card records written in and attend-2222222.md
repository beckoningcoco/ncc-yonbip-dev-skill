...
请求地址	/yonbip/hrcloud/time/sign/attend/saveAttendRecordAndSendMessage
请求方式	POST
...
Body参数
名称	类型	数组	必填	描述
matchType	int	否	否	人员匹配方式:1按考勤卡号；2按员工编号；4按员工id处理 示例：2 默认值：2
isDeduplicated	boolean	否	否	是否去重:true是,false否.为空时默认true 示例：true 默认值：true
staffCode	string	否	是	员工编号或考勤卡号或员工id,根据matchType匹配 示例：1007
code	string	否	否	编码 示例：FW20241030000001
date	string	否	是	打卡日期 示例：2020-03-17
time	string	否	是	打卡时间 示例：14:00
...
sendMessage	boolean	否	否	是否发送消息 默认值：false
...
Url: /yonbip/hrcloud/time/sign/attend/saveAttendRecordAndSendMessage?access_token=访问令牌
Body: {
	"matchType": 2,
	"isDeduplicated": true,
	"staffCode": "1007",
...
1002	数据不存在	按照提示代码解决
401	下列员工编号在假勤档案中找不到对应的人员10071	确认人员编码
