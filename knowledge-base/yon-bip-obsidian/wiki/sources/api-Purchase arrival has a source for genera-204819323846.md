...
>  请求方式	POST | Arrival Notice (MM)


## 1. 请求说明

请求域名	动态域名，获取方式详见 获取租户所在数据中心域名
请求地址	/yonbip/scm/arrivalorder/addArrivalOrderBySource
请求方式	POST
...
makeRuleCode	string	否	是	生单规则编号（预置的单据转换规则编号），采购订单生成到货单：st_purchaseorder2 示例：st_purchaseorder2
bMergeData	boolean	否	否	此标识是为了校验分单的场景，为是的时候只生成一单采购订单，当存在分单情况会提示异常，为否则可以支持分单；示例：true 默认值：true 示例：true 默认值：true
code	string	否	否	到货单单据编码，以系统编码规则配置为准：系统设置为手工编号时必输，系统设置为自动编号时非必输 示例：DH21042200000005
isContract	boolean	否	否	是否需要与供应商协同：true or false 示例：false
vouchdate	string	否	否	单据日期，格式为:yyyy-MM-dd HH:mm:ss 示例：2021-04-23 00:00:00
busType	string	否	否	推单后到货单交易类型没有默认值时必填，录入交易类型的code或id 示例：2043576921746833408 示例：1988954591564032
...
exchRate	number
小数位数:8,最大长度:28	否	否	汇率，最多是20位数，最大精度为8位 示例：1
taxRate	number
小数位数:8,最大长度:28	否	否	税率，最多是20位数，最大精度为8位 示例：9
totalQuantity	number
小数位数:8,最大长度:20	否	否	总数量，最多是20位数，最大精度为8位 示例：0.1
nativeSum	number
小数位数:8,最大长度:20	否	否	本币含税金额；小数位数取自单据上[本币]对应的币种档案上设置的“金额精度”，最多是20位数，最大精度为8位 示例：0.103
...
Url: /yonbip/scm/arrivalorder/addArrivalOrderBySource?access_token=访问令牌
Body: {
	"data": {
		"resubmitCheckKey": "",
