...
请求地址	/yonbip/scm/purinrecord/single/update
...
| memo | string | 否 | 否 | 备注 |
| purInRecords | object | 是 | 否 | 采购入库单子表[st.purinrecord.PurInRecords] |
| ignoreValidation | boolean | 否 | 否 | 参数设置为true时，请慎重使用，允许仅更新当前单据的自定义项特征字段值，不改变下游单据，不重新过账财务事项。 示例：false 默认值：false |

## 3. 请求示例

Url: /yonbip/scm/purinrecord/single/update?access_token=访问令牌
Body: {
	"data": {
		"resubmitCheckKey": "",
		"id": 0,
...
请求参数 ignoreValidation

UKC-177050-采购入库更新接口，支持更新单据自定义项特征
