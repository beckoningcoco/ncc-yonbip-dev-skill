...
请求地址	/yonbip/tax/yonbip-fi-taxpubdoc/openapi/taxRate/findById
...
| id | string | query | 是 | 税率主键id    示例: 10000 |

## 3. 请求示例

Url: /yonbip/tax/yonbip-fi-taxpubdoc/openapi/taxRate/findById?access_token=访问令牌&id=10000
...
| taxfree | boolean | 否 | 免税 示例：false |
| notaxation | boolean | 否 | 不征税 示例：false |
| scope | long | 否 | 适用范围（应用场景）(1-全部，2-采购，3-销售) 示例：1 |
| dr | int | 否 | 删除 示例：0 |
...
| enable | int | 否 | 启用状态（1-启用，0停用） 示例：0 |
| sysid | string | 否 | 应用标识 示例：diwork |
| isGlobal | string | 否 | 是否预置数据（Y-是，N-否） 示例：N |
