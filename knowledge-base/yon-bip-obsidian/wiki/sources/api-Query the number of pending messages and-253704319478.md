...
请求地址	/yonbip/SCCC/todo/items/openapi/query
请求方式	POST
...
Body参数
名称	类型	数组	必填	描述
itemStatus	string	否	否	枚举：todo=待办，done=已办 示例：todo
sortType	string	否	否	排序方式 示例：desc
sortFiled	string	否	否	排序字段
search	string	否	否	搜索关键字 示例：测试搜索关键词
pageNo	number
小数位数:0,最大长度:10	否	否	页码 示例：1
pageSize	number
小数位数:0,最大长度:10	否	否	单页条目数 示例：10
createStartTs	number
小数位数:0,最大长度:13	否	否	消息创建时间筛选-开始时间 示例：1738444800000
createEndTs	number
小数位数:0,最大长度:13	否	否	消息创建时间筛选-结束时间 示例：1739049600000
finishStartTs	number
小数位数:0,最大长度:13	否	否	消息完成时间筛选-开始时间
finishEndTs	number
小数位数:0,最大长度:15	否	否	消息完成时间筛选-结束时间
catid2nd	string	否	否	分类ID
orgid	string	否	否	组织ID 示例：ORG_001
transType	string	否	否	交易类型编码

## 3. 请求示例

Url: /yonbip/SCCC/todo/items/openapi/query?access_token=访问令牌
Body: {
	"itemStatus": "todo",
	"sortType": "desc",
	"sortFiled": "",
	...
	"pageNo": 1,
	"pageSize": 10,
	"createStartTs": 1738444800000,
	"createEndTs": 1739049600000,
	"finishStartTs": 0,
	"finishEndTs": 0,
	"catid2nd": "",
	"orgid": "ORG_001",
