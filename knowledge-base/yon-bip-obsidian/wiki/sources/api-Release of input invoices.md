...
请求地址	/yonbip/tax/api/bill/release
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| nsrsbh | string | 否 | 否 | 纳税人识别号 纳税人识别号、税务组织编码、税务组织id至少填一个 示例：112331233123 |
| orgCode | string | 否 | 否 | 税务组织编码 纳税人识别号、税务组织编码、税务组织id至少填一个 示例：hhswfktest1 |
| orgId | string | 否 | 否 | 税务组织id 纳税人识别号、税务组织编码、税务组织id至少填一个 示例：1233123 |
| systemCode | string | 否 | 是 | 系统编码 示例：BIP-RBSM |
| dzbs | string | 否 | 否 | 独占标识 默认0 1-系统独占 0-非独占 示例：1 |
| items | object | 是 | 是 | 明细 |
...
| jshj | number |
| 小数位数:2,最大长度:15 | 否 | 否 | 占用或释放的价税合计 示例：1403 |
| summaryId | string | 否 | 否 | 摘要id 示例：12313123 |

## 3. 请求示例

Url: /yonbip/tax/api/bill/release?access_token=访问令牌
Body: {
	"nsrsbh": "112331233123",
	"orgCode": "hhswfktest1",
	"orgId": "1233123",
