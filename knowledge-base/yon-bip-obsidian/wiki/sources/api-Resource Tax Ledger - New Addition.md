...
请求地址	/yonbip/FCC/TAX/TAXOHTER/yonbip-fi-taxoth/api/taxRCollection/add
...
| srcAccountingItemNo | string | 否 | 否 | 资源税台账来源科目/业务编号 |
| orgCode | string | 否 | 是 | 组织编码 |
| code | string | 否 | 是 | 资源税税源编号 |
| period | string | 否 | 是 | 税款所属期（格式2025-10-01 按期申报台账，请上送当月1日） 示例：2025-10-01 |
...
| purchaseNum | number |
| 小数位数:6,最大长度:15 | 否 | 否 | 从量-准予扣减的外购应税产品购进数量(资源税税源为从量计征时，可上送，否则不允许上送) |
| taxPaid | number |
| 小数位数:2,最大长度:15 | 否 | 否 | 从价&从量-已缴税额 |

## 3. 请求示例

Url: /yonbip/FCC/TAX/TAXOHTER/yonbip-fi-taxoth/api/taxRCollection/add?access_token=访问令牌
Body: [{
	"sourceId": "",
	"srcAccountingItemNo": "",
	"orgCode": "",
