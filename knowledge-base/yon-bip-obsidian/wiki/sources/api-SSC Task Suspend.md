...
| handlingSuggestion | string | 否 | 是 | 挂起原因：用户输入挂起原因，最大长度不能超过255 |
| externalData | string | 否 | 否 | 扩展参数：用于客开扩展逻辑参数传递，可以是json字符串 |

## 3. 请求示例

Url: /yonbip/ssc/task/hold?access_token=访问令牌
Body: {
	"data": {
		"resubmitCheckKey": "",
...
1023020503001	输入值超出允许的最大范围。	挂起原因最长不能超过255
