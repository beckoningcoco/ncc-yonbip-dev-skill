...
> `ContentType` 请求方式	GET | 分类: Sales Contract Change (SCMSD)
...
| id | string | query | 否 | 合同变更单id，如果传此字段，code、sactOid可不传。查询优先级：id>sactOid>code    示例: 6666666666666666 |
| sactOid | string | query | 否 | 原销售合同id，如果传此字段，id、code可不传。查询优先级：id>sactOid>code |
| code | string | query | 否 | 单据编码，如果传此字段，id、code可不传。查询优先级：id>sactOid>code |
| versionCode | string | query | 否 | 销售合同变更版本号，如果不传默认查最新版本 |

## 3. 请求示例

Url: /yonbip/SCC/revise/detail?access_token=访问令牌&id=6666666666666666&sactOid=&code=&versionCode=
