...
请求地址	/yonbip/QMS_QIT/retentionsampleprocess/list
请求方式	POST
...
Body参数
名称	类型	数组	必填	描述
orgId	string	否	否	质检组织
status	number
小数位数:0,最大长度:1	否	否	单据状态：0-开立、1-已审核、3-审核中
code	string	否	否	单据编号
processType	string	否	否	处理类型：0-检验、1-销毁
pageIndex	number
小数位数:0,最大长度:10	否	是	页号 默认值:1 默认值：1
pageSize	number
小数位数:0,最大长度:10	否	是	每页行数 默认值:10 默认值：10
pubts	string	否	否	时间戳，支持最高10位数字的秒级时间戳，默认起始时间戳，如果需要结束时间戳，则用 | 进行分割，如1717038743|1717038999
simple	object	否	否	拼接条件
open_vouchdate_begin	date
格式:yyyy-MM-dd HH:mm:ss	否	否	单据日期起
open_vouchdate_end	date
格式:yyyy-MM-dd HH:mm:ss	否	否	单据日期止
detailList.sampleRecordId.code	string	否	否	样品编号
detailList.sampleRecordId.name	string	否	否	样品名称
detailList.productId	string	否	否	物料
detailList.batchNo	string	否	否	批次号
detailList.supplierId	string	否	否	供应商

## 3. 请求示例

Url: /yonbip/QMS_QIT/retentionsampleprocess/list?access_token=访问令牌
Body: {
	"orgId": "",
	"status": 0,
	"code": "",
	...
	"pubts": "",
	"simple": {
		"open_vouchdate_begin": "2026-06-07 13:18:13",
		"open_vouchdate_end": "2026-06-07 13:18:13"
	},
	"detailList.sampleRecordId.code": "",
	"detailList.sampleRecordId.name": "",
	"detailList.productId": "",
	"detailList.batchNo": "",
	"detailList.supplierId": ""
}
