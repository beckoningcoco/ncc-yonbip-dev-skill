---
title: "同步采购订单审批状态"
apiId: "c4ff7a6f764043f9b42fba18cdb23bb3"
apiPath: "请求方式	POST"
...
请求地址	/yonbip/cpu/puorder/approveOrderToYC
请求方式	POST
...
Body参数
名称	类型	数组	必填	描述
supplydatas	object	否	否	supplydatas
pk_supplier	string	否	否	供应商ID 示例：供应商ID,必填
orderdatas	object	否	否	orderdatas
Id	string	否	否	云采订单主表ID 示例：云采订单主表ID
orderSourceId	string	否	否	订单主表ID 示例：订单主表ID，必填
...
erpVender	string	否	否	供应商ID 示例：供应商ID,必填
memo	string	否	否	备注 示例：备注
subject	string	否	否	订单标题 示例：订单标题，必填
orderno	string	否	否	云采订单编码 示例：云采订单编码，必填
orderStatus	string	否	否	审核状态，7 卖方已接受 10订单已生效 示例：审核状态，必填
creater	string	否	否	制单人 示例：制单人
itemdatas	object	否	否	itemdatas
id	string	否	否	云采订单子表ID 示例：云采订单子表ID
orderDetailId	string	否	否	订单子表ID 示例：订单子表ID
orderOtherId	string	否	否	订单子表编码 示例：订单子表编码,必填
...
unitCode	string	否	否	计价code
priceExchRate	string	否	否	计价换算率
purchaseNum	BigDecimal	否	否	采购数量
purchaseUnitId	string	否	否	采购单位id
purchaseUnitCode	string	否	否	采购单位code
purchaseUnitName	string	否	否	采购单位名称
purchaseExchRate	BigDecimal	否	否	采购换算率
mainNum	BigDecimal	是	否	主数量
mainMetrologyId	string	否	否	主数量id
mainMetrologyCode	string	否	否	主数量code
mainMetrologyName	string	否	否	主数量名称

## 3. 请求示例

Url: /yonbip/cpu/puorder/approveOrderToYC?access_token=访问令牌
Body: {
	"supplydatas": {
...
1002	数据不存在	按照提示代码解决
001	审批后的订单表头接收的请求数据为空,不能进行更新云采的订单	向服务方咨询
