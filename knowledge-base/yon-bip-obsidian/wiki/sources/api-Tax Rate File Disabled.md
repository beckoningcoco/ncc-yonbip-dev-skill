...
请求地址	/yonbip/tax/yonbip-fi-taxpubdoc/openapi/taxRate/disable
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| data | object | 否 | 是 | 停用的税率档案 |
| id | long | 否 | 是 | 主键 示例：1577764267860426800 |

## 3. 请求示例

Url: /yonbip/tax/yonbip-fi-taxpubdoc/openapi/taxRate/disable?access_token=访问令牌
Body: {
	"data": {
		"id": 1577764267860426800
	}
}
...
| scope | int | 否 | 范围（应用场景）(1-全部，2-采购，3-销售) 示例：1 |
| dr | int | 否 | 逻辑删除 示例：0 |
| ytenantId | string | 否 | 租户id 示例：mdhpm1a4 |
| createTime | DateTime | 否 | 创建时间（yyyy-MM-dd HH:mm:ss） 示例：2022-10-27 14:01:07 |
| modifyTime | DateTime | 否 | 修改时间（yyyy-MM-dd HH:mm:ss） 示例：2022-11-12 14:37:23 |
...
| pubts | string | 否 | 更新时间（yyyy-MM-dd HH:mm:ss） 示例：2022-11-12 14:37:23 |
| enable | int | 否 | 启用状态（0-停用，1-启用） 示例：1 |
...
999	traceId:86e5d51632408a12;错误信息:停用税率档案入参不能为空	入参不正确或服务端逻辑异常
