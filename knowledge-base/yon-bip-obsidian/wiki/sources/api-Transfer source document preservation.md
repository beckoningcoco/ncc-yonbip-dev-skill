...
请求地址	/yonbip/scm/storein/mergeSourceData/save
...
| inwarehouse | string | 否 | 否 | 调入仓库id或编码，上游无值时必传，上游有值不可以传。 |
...
Url: /yonbip/scm/storein/mergeSourceData/save?access_token=访问令牌
Body: {
	"data": {
		"resubmitCheckKey": "",
...
| srcBillType | string | 否 | 来源单据类型, st_storeout:调出单 |
| creator | string | 否 | 创建人 |
...
| inwarehouse_iSerialManage | boolean | 否 | 仓库是否序列号严格管理 |
...
| details | object | 是 | 调入单子表[st.storein.StoreInDetail] |
