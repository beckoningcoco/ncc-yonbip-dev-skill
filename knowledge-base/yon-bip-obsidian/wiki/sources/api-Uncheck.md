...
请求地址	/yonbip/tax/invoiceclient-web/api/vat/cxgx
...
| 名称 | 类型 | 数组 | 必填 | 描述 |
| hmdms | object | 是 | 否 | 发票信息 |
| fpDm | string | 否 | 是 | 发票代码 没有时传空字符串 示例：3100171130 |
| fpHm | string | 否 | 是 | 发票号码 示例：11404542 |
| kprq | string | 否 | 是 | 开票日期yyyyMMdd 示例：20170831 |
| deductMode | long | 否 | 否 | 抵扣类型，预勾选此参数⽆效 1.撤销抵扣(不填默认为撤销抵扣） 2.撤销不抵扣 示例：1 |
...
| mobileOrEmail | string | 否 | 否 | 勾选人信息 勾选人在税务云的手机或邮箱,仅 “预勾选” 生效 示例：13901234567 |

## 3. 请求示例

Url: /yonbip/tax/invoiceclient-web/api/vat/cxgx?access_token=访问令牌
Body: {
	"hmdms": [
		{
			"fpDm": "3100171130",
