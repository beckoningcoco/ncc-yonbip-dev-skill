...
请求地址	/yonbip/tax/other-tax/api/land-tax-info/save
...
| landTaxArea | number |
| 小数位数:2,最大长度:13 | 否 | 是 | 占用土地面积（平米），0<值<=土地台账基础信息中的土地面积。 示例：2000.00 |
| landLevel | string | 否 | 是 | 土地等级，需与纳税主体中的【纳税人基本信息】中-【税费种认定信息】城镇土地使用税的税目保持一致 示例：环保税税目 |
| effectiveDate | date |
| 格式:yyyy-MM-dd | 否 | 是 | 纳税义务起始时间，不得早于土地台账“基础信息”中“土地取得时间” 示例：2024-01-20 |
| code | string | 否 | 否 | 土地税源编号，土地编号、土地税源编号、土地税源来源id不可同时为空，同时上送时需要校验匹配关系，用于识别土地税源 示例：TDSY20250703000002 |
| lyid | string | 否 | 否 | 土地税源来源id，土地编号、土地税源编号、土地税源来源id不可同时为空，同时上送时需要校验匹配关系，用于识别土地税源 示例：4233245345 |

## 3. 请求示例

Url: /yonbip/tax/other-tax/api/land-tax-info/save?access_token=访问令牌
Body: [{
