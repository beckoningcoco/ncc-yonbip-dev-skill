...
请求地址	/yonbip/mfg/workhoursmaintain/batchunaudit
请求方式	POST
...
id	string	否	否	单据id，id和code同时传入，以id为准，无论id是否正确，id优先级大于code 示例：2247035758837235720
code	string	否	否	单据code，id和code同时传入，以id为准，无论id是否正确，id优先级大于code 示例：WH20250416000021

## 3. 请求示例

Url: /yonbip/mfg/workhoursmaintain/batchunaudit?access_token=访问令牌
Body: {
	"data": [
		{
			"id": "2247035758837235720",
...
messages	string	是	结果信息 示例：该单据不存在或已失效
infos	object	是	单据信息
