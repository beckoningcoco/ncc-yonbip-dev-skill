---
title: "页面状态预置说明"
source: "https://c2.yonyoucloud.com/iuap-yonbuilder-designer/ucf-wh/docs-mdf/mdf/index.html#/spec/02-page-status-manage"
section: "协议"
date: 2026-06-07
ingested: 2026-06-07
tags: [MDF, 前端框架, 协议]
...
> 来源：https://c2.yonyoucloud.com/iuap-yonbuilder-designer/ucf-wh/docs-mdf/mdf/index.html#/spec/02-page-status-manage | 所属：协议

# [](#页面状态预置说明)页面状态预置说明

### [](#1表单命令和工具栏)1. 表单命令和工具栏

**表单命令**：服务接口数据

**表单工具栏**：工具栏UI元数据，工具栏按钮依赖某个表单命令
...
||align||varchar(100)||NO||工具栏对齐位置，可选值：left\right	op\bottom\center|
||subid||varchar(10)||YES||子产品号|
||system||int(11)||YES||元数据分层项目ID，默认值0表示系统预置|
||pubts||timestamp||NO||时间戳，不需要预置|

#### [](#13工具栏按钮预置)1.3. 工具栏按钮预置
...
||type||varchar(100)||YES||类型：button\menu\menuitem等，具体支持的类型请前端确认|
||style||int(11)||YES||样式，显示为图标、文本、图标+文本等，可暂不预置，需要前端确认|
...
||billnumber||varchar(200)||NO||状态组编码，自定义公共状态组|
...
#### [](#22栏目和按钮状态预置)2.2. 栏目和按钮状态预置

||Field||Type||Null||Key|
||id||bigint(20)||NO||自动编号|
||billnumber||varchar(100)||NO||状态组编码，对应bill_staus表的billnumber|
||status||varchar(100)||NO||状态码，外键关联bill_status.code|
||item||varchar(200)||NO||对象标识（实体字段名或按钮名称），外键关联billitem_base.cName（栏目）或bill_toolbar_item.name（按钮）|
...
#### [](#24状态模板默认值配置的合并处理)2.4. 状态模板、默认值、配置的合并处理
...
例如表单A基于模板B设置状态，其中bill_status表中：

- 模板B定义有状态：新增、编辑、修改
- 表单A定义有状态：新增、审核

则运行时合并后，表单A具有以下状态：新增、编辑、修改、审核，其中新增状态使用表单A定义的属性覆盖模板B定义的新增状态属性。

**按钮、栏目状态配置优先级**

例如表单A基于模板B设置状态，某一按钮或栏目Item，其状态值确定优先级为：

- 优先取bill_status_profile表billNumber=A的Item记录
- 步骤1无记录时，取bill_status_profile表billNumber=B的Item记录
- 步骤2无记录时，取bill_status表billNumber=A的相应默认值
- 步骤3无记录时，取bill_status表billNumber=B的相应默认值