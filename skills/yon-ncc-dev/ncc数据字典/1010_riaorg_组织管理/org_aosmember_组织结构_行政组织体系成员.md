# |<<

**组织结构_行政组织体系成员 (org_aosmember / nc.vo.om.aos.AOSMemberVO)**

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/3993.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_aosmember | 行政组织体系成员主键 | pk_aosmember | char(20) | √ | 主键 (UFID) |
| 2 | pk_org | 对应组织 | pk_org | varchar(20) |  | 组织 (org) |
| 3 | pk_fatherorg | 上级组织 | pk_fatherorg | varchar(20) |  | 组织 (org) |
| 4 | innercode | 内部编码 | innercode | varchar(200) |  | 字符串 (String) |
| 5 | pk_group | 所属集团 | pk_group | varchar(20) |  | 组织_集团 (group) |
| 6 | seq | 序号 | seq | varchar(50) |  | 字符串 (String) |
| 7 | dataoriginflag | 分布式 | dataoriginflag | int |  | 数据来源 (dataorigin) | 0 | 0=本级产生; |