# |<<

**账表一致性配置 (iufo_account_set / nc.vo.ufoe.accountset.AccountSetVO)**

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/3384.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_accountset | 主键 | pk_accountset | char(20) | √ | 主键 (UFID) |
| 2 | pk_task | 任务 | pk_task | varchar(20) |  | 任务 (task) |
| 3 | pk_account | 账簿主键 | pk_account | varchar(20) |  | 字符串 (String) |
| 4 | accountname | 账簿名称 | accountname | varchar(200) |  | 字符串 (String) |
| 5 | pk_org | 所属组织 | pk_org | varchar(20) |  | 组织 (org) |
| 6 | pk_group | 所属集团 | pk_group | varchar(20) |  | 组织_集团 (group) |
| 7 | creator | 创建人 | creator | varchar(20) |  | 用户 (user) |
| 8 | creationtime | 创建时间 | creationtime | char(19) |  | 日期时间 (UFDateTime) |
| 9 | modifier | 最后修改人 | modifier | varchar(20) |  | 用户 (user) |
| 10 | modifiedtime | 最后修改时间 | modifiedtime | char(19) |  | 日期时间 (UFDateTime) |