...
**采购入库单明细 (ia_i2bill_b / nc.vo.ia.mi2.entity.I2ItemVO)**

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/2815.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | cbill_bid | 采购入库单明细 | cbill_bid | char(20) | √ | 主键 (UFID) |
...
| 28 | nreasonalwastnum | 合理损耗数量 | nreasonalwastnum | decimal(28, 8) |  | 数值 (UFDouble) |
| 29 | nreasonalwastprice | 合理损耗单价 | nreasonalwastprice | decimal(28, 8) |  | 数值 (UFDouble) |
| 30 | nreasonalwastmny | 合理损耗金额 | nreasonalwastmny | decimal(28, 8) |  | 数值 (UFDouble) |
| 31 | nredbacknum | 累计冲回数量 | nredbacknum | decimal(28, 8) |  | 数值 (UFDouble) |
| 32 | cprofitcenterid | 利润中心 | cprofitcenterid | varchar(20) |  | 利润中心 (profitcenter) |
| 33 | cprofitcentervid | 利润中心版本 | cprofitcentervid | varchar(20) |  | 利润中心版本 (profitcenterVersion) |
...
| 37 | vfirsttype | 源头单据类型 | vfirsttype | varchar(20) |  | 单据类型 (BilltypeVO) |
| 38 | vfirsttrantype | 源头交易类型 | vfirsttrantype | varchar(20) |  | 单据类型 (BilltypeVO) |
| 39 | vfirstcode | 源头单据号 | vfirstcode | varchar(40) |  | 字符串 (String) |
| 40 | cfirstid | 源头单据 | cfirstid | varchar(20) |  | 字符串 (String) |
| 41 | cfirstbid | 源头单据分录 | cfirstbid | varchar(20) |  | 字符串 (String) |
| 42 | vfirstrowno | 源头单据行号 | vfirstrowno | varchar(20) |  | 字符串 (String) |
| 43 | vsrctype | 来源单据类型 | vsrctype | varchar(20) |  | 单据类型 (BilltypeVO) |
| 44 | vsrctrantype | 来源交易类型 | vsrctrantype | varchar(20) |  | 单据类型 (BilltypeVO) |
| 45 | vsrccode | 来源单据号 | vsrccode | varchar(40) |  | 字符串 (String) |
| 46 | csrcid | 来源单据 | csrcid | varchar(20) |  | 字符串 (String) |
| 47 | vsrcrowno | 来源单据行号 | vsrcrowno | varchar(20) |  | 字符串 (String) |
| 48 | csrcbid | 来源单据分录 | csrcbid | varchar(20) |  | 字符串 (String) |
| 49 | cicitemid | 库存子表 | cicitemid | varchar(20) |  | 字符串 (String) |
| 50 | pk_group | 所属集团 | pk_group | varchar(20) |  | 组织_集团 (group) |
| 51 | pk_book | 所属账簿 | pk_book | varchar(20) |  | 账簿_核算账簿 (accbook) |
| 52 | pk_org | 成本域 | pk_org | varchar(20) |  | 组织_成本域 (costregion) |
| 53 | caccountperiod | 会计期间 | caccountperiod | char(7) |  | 字符串 (String) |
| 54 | dbilldate | 单据日期 | dbilldate | char(19) |  | 日期 (UFDate) |
| 55 | ccalcthreadid | 计算线索 | ccalcthreadid | varchar(20) |  | 字符串 (String) |
| 56 | ccalcid | 计算关系 | ccalcid | varchar(20) |  | 字符串 (String) |
| 57 | fcalcbizflag | 计算关系的业务类型 | fcalcbizflag | int |  | 计算业务类型 (calcbiztype) |  | 1=普通计算类型; |