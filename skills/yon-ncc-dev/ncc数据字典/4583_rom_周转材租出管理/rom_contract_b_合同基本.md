# |<<

**合同基本 (rom_contract_b / nc.vo.rom.contract.ContractBasicVO)**

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/4985.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_contract_b | 合同基本主键 | pk_contract_b | char(20) | √ | 主键 (UFID) |
| 2 | pk_contract_b_base | 合同基本源版主键 | pk_contract_b_base | char(20) |  | 字符串 (String) |
| 3 | pk_org | 租出组织最新版本 | pk_org | varchar(20) |  | 组织_业务单元_库存组织 (stockorg) |
| 4 | pk_org_v | 租出组织 | pk_org_v | varchar(20) |  | 组织_业务单元_库存组织版本信息 (stockorg_v) |
| 5 | pk_group | 集团 | pk_group | varchar(20) |  | 组织_集团 (group) |
| 6 | rowno | 行号 | rowno | varchar(30) |  | 字符串 (String) |
| 7 | pk_material | 物料最新版本 | pk_material | varchar(20) |  | 物料基本信息 (material_v) |
| 8 | pk_material_v | 物料 | pk_material_v | varchar(20) |  | 物料基本信息（多版本） (material) |
| 9 | pk_marbaseclass | 物料分类 | pk_marbaseclass | varchar(20) |  | 物料基本分类 (marbasclass) |
| 10 | nbatchcode | 批次号 | nbatchcode | varchar(40) |  | 字符串 (String) |
| 11 | pk_supplier | 供应商 | pk_supplier | varchar(20) |  | 供应商基本信息 (supplier) |
| 12 | pk_producer | 生产厂商 | pk_producer | varchar(20) |  | 生产厂商(自定义档案) (Defdoc-BD006_0xx) |
| 13 | pk_unit | 单位 | pk_unit | varchar(20) |  | 计量单位 (measdoc) |
| 14 | pk_main_unit | 主单位 | pk_main_unit | varchar(20) |  | 计量单位 (measdoc) |
| 15 | pk_rent_unit | 报价单位 | pk_rent_unit | varchar(20) |  | 计量单位 (measdoc) |
| 16 | num | 数量 | num | decimal(28, 8) |  | 数值 (UFDouble) |  | [0 , ] |
| 17 | main_num | 主数量 | main_num | decimal(28, 8) |  | 数值 (UFDouble) |  | [0 , ] |
| 18 | rent_num | 报价数量 | rent_num | decimal(28, 8) |  | 数值 (UFDouble) |  | [0 , ] |
| 19 | changerate | 换算率 | changerate | varchar(50) |  | 字符串 (String) |
| 20 | rent_changerate | 报价单位换算率 | rent_changerate | varchar(50) |  | 字符串 (String) |
| 21 | rent_price | 租金单价(原币) | rent_price | decimal(28, 8) |  | 金额 (UFMoney) |  | [0 , ] |
| 22 | disused_price | 报废赔偿单价(原币) | disused_price | decimal(28, 8) |  | 金额 (UFMoney) |
| 23 | lose_price | 遗失赔偿单价(原币) | lose_price | decimal(28, 8) |  | 金额 (UFMoney) |
| 24 | stoprent_price | 停租单价(原币) | stoprent_price | decimal(28, 8) |  | 金额 (UFMoney) |
| 25 | comp_price | 赔偿单价(原币) | comp_price | decimal(28, 8) |  | 金额 (UFMoney) |  | [0 , ] |
| 26 | rent_num_out | 已租出报价数量 | rent_num_out | decimal(28, 8) |  | 数值 (UFDouble) |  | [0 , ] |
| 27 | rent_num_return | 已还回报价数量 | rent_num_return | decimal(28, 8) |  | 数值 (UFDouble) |  | [0 , ] |
| 28 | rent_num_stop | 停租报价数量 | rent_num_stop | decimal(28, 8) |  | 数值 (UFDouble) |  | [0 , ] |
| 29 | rent_num_comp | 赔偿报价数量 | rent_num_comp | decimal(28, 8) |  | 数值 (UFDouble) |  | [0 , ] |
| 30 | rent_num_lost | 遗失报价数量 | rent_num_lost | decimal(28, 8) |  | 数值 (UFDouble) |
| 31 | rent_num_discard | 报废报价数量 | rent_num_discard | decimal(28, 8) |  | 数值 (UFDouble) |
| 32 | adjust_num | 调整报价数量 | adjust_num | decimal(28, 8) |  | 数值 (UFDouble) |
| 33 | pk_jobmngfil | 项目 | pk_jobmngfil | varchar(20) |  | 项目 (project) |
| 34 | memo | 备注 | memo | varchar(200) |  | 字符串 (String) |
| 35 | vfree1 | 自由辅助属性1 | vfree1 | varchar(101) |  | 自由项 (Custom) |
| 36 | vfree2 | 自由辅助属性2 | vfree2 | varchar(101) |  | 自由项 (Custom) |
| 37 | vfree3 | 自由辅助属性3 | vfree3 | varchar(101) |  | 自由项 (Custom) |
| 38 | vfree4 | 自由辅助属性4 | vfree4 | varchar(101) |  | 自由项 (Custom) |
| 39 | vfree5 | 自由辅助属性5 | vfree5 | varchar(101) |  | 自由项 (Custom) |
| 40 | vfree6 | 自由辅助属性6 | vfree6 | varchar(101) |  | 自由项 (Custom) |
| 41 | vfree7 | 自由辅助属性7 | vfree7 | varchar(101) |  | 自由项 (Custom) |
| 42 | vfree8 | 自由辅助属性8 | vfree8 | varchar(101) |  | 自由项 (Custom) |
| 43 | vfree9 | 自由辅助属性9 | vfree9 | varchar(101) |  | 自由项 (Custom) |
| 44 | vfree10 | 自由辅助属性10 | vfree10 | varchar(101) |  | 自由项 (Custom) |
| 45 | def1 | 自定义项1 | def1 | varchar(101) |  | 自定义项 (Custom) |
| 46 | def2 | 自定义项2 | def2 | varchar(101) |  | 自定义项 (Custom) |
| 47 | def3 | 自定义项3 | def3 | varchar(101) |  | 自定义项 (Custom) |
| 48 | def4 | 自定义项4 | def4 | varchar(101) |  | 自定义项 (Custom) |
| 49 | def5 | 自定义项5 | def5 | varchar(101) |  | 自定义项 (Custom) |
| 50 | def6 | 自定义项6 | def6 | varchar(101) |  | 自定义项 (Custom) |
| 51 | def7 | 自定义项7 | def7 | varchar(101) |  | 自定义项 (Custom) |
| 52 | def8 | 自定义项8 | def8 | varchar(101) |  | 自定义项 (Custom) |
| 53 | def9 | 自定义项9 | def9 | varchar(101) |  | 自定义项 (Custom) |
| 54 | def10 | 自定义项10 | def10 | varchar(101) |  | 自定义项 (Custom) |
| 55 | def11 | 自定义项11 | def11 | varchar(101) |  | 自定义项 (Custom) |
| 56 | def12 | 自定义项12 | def12 | varchar(101) |  | 自定义项 (Custom) |
| 57 | def13 | 自定义项13 | def13 | varchar(101) |  | 自定义项 (Custom) |
| 58 | def14 | 自定义项14 | def14 | varchar(101) |  | 自定义项 (Custom) |
| 59 | def15 | 自定义项15 | def15 | varchar(101) |  | 自定义项 (Custom) |
| 60 | def16 | 自定义项16 | def16 | varchar(101) |  | 自定义项 (Custom) |
| 61 | def17 | 自定义项17 | def17 | varchar(101) |  | 自定义项 (Custom) |
| 62 | def18 | 自定义项18 | def18 | varchar(101) |  | 自定义项 (Custom) |
| 63 | def19 | 自定义项19 | def19 | varchar(101) |  | 自定义项 (Custom) |
| 64 | def20 | 自定义项20 | def20 | varchar(101) |  | 自定义项 (Custom) |