# |<<

**用户组和部门关系 (sm_usergroup_uh_dept / nc.vo.nhbd.res.UserGrpDeptVO)**

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/5365.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_userdept | 用户部门主键 | pk_userdept | char(20) | √ | 主键 (UFID) |
| 2 | pk_group | 所属集团 | pk_group | varchar(20) |  | 组织_集团 (group) |
| 3 | pk_org | 所属机构 | pk_org | varchar(20) |  | 组织 (org) |
| 4 | pk_usergroup | 用户组 | pk_usergroup | varchar(20) |  | 用户组 (userGroup) |
| 5 | sortno | 序号 | sortno | int |  | 整数 (Integer) |
| 6 | pk_dept | 部门 | pk_dept | varchar(20) |  | 组织_部门 (dept) |
| 7 | flag_def | 默认标志 | flag_def | char(1) | √ | 布尔类型 (UFBoolean) |
| 8 | creator | 创建人 | creator | varchar(20) |  | 用户 (user) |
| 9 | creationtime | 创建时间 | creationtime | char(19) |  | 日期时间 (UFDateTime) |
| 10 | modifier | 修改人 | modifier | varchar(20) |  | 用户 (user) |
| 11 | modifiedtime | 修改时间 | modifiedtime | char(19) |  | 日期时间 (UFDateTime) |