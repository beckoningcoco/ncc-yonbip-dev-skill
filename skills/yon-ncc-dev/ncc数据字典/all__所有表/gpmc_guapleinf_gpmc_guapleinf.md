# |<<

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/8610.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_guapledgeinfo_b | pk_guapledgeinfo_b | pk_guapledgeinfo_b | char(20) | √ |
| 2 | fsrcbilltypecode | fsrcbilltypecode | fsrcbilltypecode | varchar2(50) |
| 3 | glcrate | glcrate | glcrate | number(28, 8) |
| 4 | gllcrate | gllcrate | gllcrate | number(28, 8) |
| 5 | guapropertyid | guapropertyid | guapropertyid | varchar2(20) |
| 6 | iunitinid | iunitinid | iunitinid | varchar2(20) |  |  | '~' |
| 7 | lcrate | lcrate | lcrate | number(28, 8) |
| 8 | makedate | makedate | makedate | char(19) |
| 9 | maxpledge | maxpledge | maxpledge | number(28, 8) |
| 10 | olcrate | olcrate | olcrate | number(28, 8) |
| 11 | owner | owner | owner | number(38, 0) |
| 12 | ownunit | ownunit | ownunit | varchar2(20) |  |  | '~' |
| 13 | partnerid | partnerid | partnerid | varchar2(20) |  |  | '~' |
| 14 | pk_currtype | pk_currtype | pk_currtype | varchar2(20) |  |  | '~' |
| 15 | pk_fsrcbill | pk_fsrcbill | pk_fsrcbill | varchar2(50) |
| 16 | pk_fsrcbilltypeid | pk_fsrcbilltypeid | pk_fsrcbilltypeid | varchar2(50) |
| 17 | pk_group | pk_group | pk_group | varchar2(20) |  |  | '~' |
| 18 | pk_guacontract | pk_guacontract | pk_guacontract | char(20) | √ |
| 19 | pk_org | pk_org | pk_org | varchar2(20) |  |  | '~' |
| 20 | pk_org_v | pk_org_v | pk_org_v | varchar2(20) |  |  | '~' |
| 21 | pk_srcbill | pk_srcbill | pk_srcbill | varchar2(50) |
| 22 | pk_srcbillrowid | pk_srcbillrowid | pk_srcbillrowid | varchar2(50) |
| 23 | pk_srcbilltypeid | pk_srcbilltypeid | pk_srcbilltypeid | varchar2(50) |
| 24 | pledgename | pledgename | pledgename | varchar2(200) |
| 25 | pledgeno | pledgeno | pledgeno | varchar2(20) |  |  | '~' |
| 26 | restpledge | restpledge | restpledge | number(28, 8) |
| 27 | rowno | rowno | rowno | varchar2(50) |
| 28 | sourcesystypecode | sourcesystypecode | sourcesystypecode | varchar2(50) |
| 29 | srcbillno | srcbillno | srcbillno | varchar2(50) |
| 30 | summary | summary | summary | varchar2(200) |
| 31 | totalpledge | totalpledge | totalpledge | number(28, 8) |
| 32 | unitname | unitname | unitname | varchar2(200) |
| 33 | usingamount | usingamount | usingamount | number(28, 8) |
| 34 | usinglcamount | usinglcamount | usinglcamount | number(28, 8) |
| 35 | vbillno | vbillno | vbillno | varchar2(50) |
| 36 | vdef1 | vdef1 | vdef1 | varchar2(101) |
| 37 | vdef2 | vdef2 | vdef2 | varchar2(101) |
| 38 | vdef3 | vdef3 | vdef3 | varchar2(101) |
| 39 | vdef4 | vdef4 | vdef4 | varchar2(101) |
| 40 | vdef5 | vdef5 | vdef5 | varchar2(101) |
| 41 | vdef6 | vdef6 | vdef6 | varchar2(101) |
| 42 | vdef7 | vdef7 | vdef7 | varchar2(101) |
| 43 | vdef8 | vdef8 | vdef8 | varchar2(101) |
| 44 | vdef9 | vdef9 | vdef9 | varchar2(101) |
| 45 | vdef10 | vdef10 | vdef10 | varchar2(101) |
| 46 | vdef11 | vdef11 | vdef11 | varchar2(101) |
| 47 | vdef12 | vdef12 | vdef12 | varchar2(101) |
| 48 | vdef13 | vdef13 | vdef13 | varchar2(101) |
| 49 | vdef14 | vdef14 | vdef14 | varchar2(101) |
| 50 | vdef15 | vdef15 | vdef15 | varchar2(101) |
| 51 | vdef16 | vdef16 | vdef16 | varchar2(101) |
| 52 | vdef17 | vdef17 | vdef17 | varchar2(101) |
| 53 | vdef18 | vdef18 | vdef18 | varchar2(101) |
| 54 | vdef19 | vdef19 | vdef19 | varchar2(101) |
| 55 | vdef20 | vdef20 | vdef20 | varchar2(101) |
| 56 | dr | dr | dr | number(10, 0) |  |  | 0 |
| 57 | ts | ts | ts | char(19) |  |  | to_char(sysdate,'yyyy-mm-dd hh24:mi:ss') |