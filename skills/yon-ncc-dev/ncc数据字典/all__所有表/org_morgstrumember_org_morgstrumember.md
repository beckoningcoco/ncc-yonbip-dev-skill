# |<<

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/10240.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_morgstrumember | pk_morgstrumember | pk_morgstrumember | char(20) | √ |
| 2 | code | code | code | varchar2(50) |
| 3 | idx | idx | idx | number(38, 0) |
| 4 | innercode | innercode | innercode | varchar2(50) |
| 5 | islastversion | islastversion | islastversion | char(1) |
| 6 | name | name | name | varchar2(300) |
| 7 | name2 | name2 | name2 | varchar2(300) |
| 8 | name3 | name3 | name3 | varchar2(300) |
| 9 | name4 | name4 | name4 | varchar2(300) |
| 10 | name5 | name5 | name5 | varchar2(300) |
| 11 | name6 | name6 | name6 | varchar2(300) |
| 12 | pk_entityorg | pk_entityorg | pk_entityorg | varchar2(20) |  |  | '~' |
| 13 | pk_fathermember | pk_fathermember | pk_fathermember | varchar2(20) |  |  | '~' |
| 14 | pk_fatherorg | pk_fatherorg | pk_fatherorg | varchar2(20) |  |  | '~' |
| 15 | pk_morgstru | pk_morgstru | pk_morgstru | varchar2(20) |  |  | '~' |
| 16 | pk_org | pk_org | pk_org | varchar2(20) |  |  | '~' |
| 17 | pk_role | pk_role | pk_role | varchar2(20) |  |  | '~' |
| 18 | pk_svid | pk_svid | pk_svid | varchar2(20) |  |  | '~' |
| 19 | pk_user | pk_user | pk_user | varchar2(20) |  |  | '~' |
| 20 | pk_vid | pk_vid | pk_vid | varchar2(50) |
| 21 | plan_flag | plan_flag | plan_flag | char(1) |
| 22 | venddate | venddate | venddate | char(19) |
| 23 | virtualorg | virtualorg | virtualorg | char(1) |
| 24 | vname | vname | vname | varchar2(50) |
| 25 | vno | vno | vno | varchar2(50) |
| 26 | vstartdate | vstartdate | vstartdate | char(19) |
| 27 | creationtime | creationtime | creationtime | char(19) |
| 28 | creator | creator | creator | varchar2(20) |  |  | '~' |
| 29 | dr | dr | dr | number(10, 0) |  |  | 0 |
| 30 | modifiedtime | modifiedtime | modifiedtime | char(19) |
| 31 | modifier | modifier | modifier | varchar2(20) |  |  | '~' |
| 32 | ts | ts | ts | char(19) |  |  | to_char(sysdate,'yyyy-mm-dd hh24:mi:ss') |