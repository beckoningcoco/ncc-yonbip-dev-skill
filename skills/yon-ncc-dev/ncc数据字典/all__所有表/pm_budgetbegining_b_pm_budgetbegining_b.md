# |<<

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/10544.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_budgetbegin_b | pk_budgetbegin_b | pk_budgetbegin_b | char(20) | √ |
| 2 | exe_mny | exe_mny | exe_mny | number(28, 8) |
| 3 | exe_mny_global | exe_mny_global | exe_mny_global | number(28, 8) |
| 4 | exe_mny_group | exe_mny_group | exe_mny_group | number(28, 8) |
| 5 | exe_unit_price | exe_unit_price | exe_unit_price | number(28, 8) |
| 6 | in_exe_mny | in_exe_mny | in_exe_mny | number(28, 8) |
| 7 | in_exe_mny_global | in_exe_mny_global | in_exe_mny_global | number(28, 8) |
| 8 | in_exe_mny_group | in_exe_mny_group | in_exe_mny_group | number(28, 8) |
| 9 | memo | memo | memo | varchar2(300) |
| 10 | nnum | nnum | nnum | number(28, 8) |
| 11 | ori_busidate | ori_busidate | ori_busidate | char(19) |
| 12 | pk_budgetbegin | pk_budgetbegin | pk_budgetbegin | char(20) | √ |
| 13 | pk_cbsnode | pk_cbsnode | pk_cbsnode | varchar2(20) |  |  | '~' |
| 14 | pk_currtype | pk_currtype | pk_currtype | varchar2(20) |  |  | '~' |
| 15 | pk_factor | pk_factor | pk_factor | varchar2(20) |  |  | '~' |
| 16 | pk_group | pk_group | pk_group | varchar2(20) |  |  | '~' |
| 17 | pk_material | pk_material | pk_material | varchar2(20) |  |  | '~' |
| 18 | pk_mcontr | pk_mcontr | pk_mcontr | varchar2(50) |
| 19 | pk_measdoc | pk_measdoc | pk_measdoc | varchar2(20) |  |  | '~' |
| 20 | pk_org | pk_org | pk_org | varchar2(20) |  |  | '~' |
| 21 | pk_org_v | pk_org_v | pk_org_v | varchar2(20) |  |  | '~' |
| 22 | pk_project | pk_project | pk_project | varchar2(20) |  |  | '~' |
| 23 | pk_supplier | pk_supplier | pk_supplier | varchar2(20) |  |  | '~' |
| 24 | pk_wbs | pk_wbs | pk_wbs | varchar2(20) |  |  | '~' |
| 25 | rowno | rowno | rowno | varchar2(50) |
| 26 | def1 | def1 | def1 | varchar2(101) |
| 27 | def2 | def2 | def2 | varchar2(101) |
| 28 | def3 | def3 | def3 | varchar2(101) |
| 29 | def4 | def4 | def4 | varchar2(101) |
| 30 | def5 | def5 | def5 | varchar2(101) |
| 31 | def6 | def6 | def6 | varchar2(101) |
| 32 | def7 | def7 | def7 | varchar2(101) |
| 33 | def8 | def8 | def8 | varchar2(101) |
| 34 | def9 | def9 | def9 | varchar2(101) |
| 35 | def10 | def10 | def10 | varchar2(101) |
| 36 | def11 | def11 | def11 | varchar2(101) |
| 37 | def12 | def12 | def12 | varchar2(101) |
| 38 | def13 | def13 | def13 | varchar2(101) |
| 39 | def14 | def14 | def14 | varchar2(101) |
| 40 | def15 | def15 | def15 | varchar2(101) |
| 41 | def16 | def16 | def16 | varchar2(101) |
| 42 | def17 | def17 | def17 | varchar2(101) |
| 43 | def18 | def18 | def18 | varchar2(101) |
| 44 | def19 | def19 | def19 | varchar2(101) |
| 45 | def20 | def20 | def20 | varchar2(101) |
| 46 | dr | dr | dr | number(10, 0) |  |  | 0 |
| 47 | ts | ts | ts | char(19) |  |  | to_char(sysdate,'yyyy-mm-dd hh24:mi:ss') |