# |<<

**财务统计体系含编码名称 (org_financestatstrumember / nc.vo.corg.FinanceStatStruMemberWithCodeNameVO)**

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/4061.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_fssmember | 财务统计体系成员主键 | pk_fssmember | char(20) | √ | 主键 (UFID) |
| 2 | pk_fss | 财务统计体系主键 | pk_fss | varchar(20) |  | 组织结构_财务统计体系 (financestatstru) |
| 3 | pk_org | 对应组织 | pk_org | varchar(20) |  | 组织_业务单元_财务组织 (financeorg) |
| 4 | pk_fatherorg | 上级组织 | pk_fatherorg | varchar(20) |  | 组织_业务单元_财务组织 (financeorg) |
| 5 | pk_fathermember | 上级成员 | pk_fathermember | varchar(20) |  | 财务统计体系成员 (financestatstrumember) |
| 6 | virtualorg | 虚组织 | virtualorg | char(1) |  | 布尔类型 (UFBoolean) |
| 7 | innercode | 内部编码 | innercode | varchar(200) |  | 字符串 (String) |
| 8 | idx | 排列顺序 | idx | int |  | 整数 (Integer) |
| 9 | code | 编码 | code | varchar(50) |  | 字符串 (String) |
| 10 | name | 名称 | name | varchar(200) |  | 多语文本 (MultiLangText) |
| 11 | creator | 创建人 | creator | varchar(20) |  | 用户 (user) |
| 12 | creationtime | 创建时间 | creationtime | char(19) |  | 日期时间 (UFDateTime) |
| 13 | modifier | 最后修改人 | modifier | varchar(20) |  | 用户 (user) |
| 14 | modifiedtime | 最后修改时间 | modifiedtime | char(19) |  | 日期时间 (UFDateTime) |