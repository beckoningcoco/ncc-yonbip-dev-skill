...
public class ArapDocPrintVerifyDetailDatasource<V>
implements IMetaDataDataSource {
    private static final long serialVersionUID = 1L;
    private final String[] pk_bills;

    public ArapDocPrintVerifyDetailDatasource(String[] pk_bills, String billType) {
    ...
    public Object[] getMDObjects() {
        block8: {
            try {
                Collection busiDataVOList = new BaseDAO().retrieveByClause(VerifyDetailVO.class, SqlUtils.getInStr((String)"pk_verifydetail", (String[])this.pk_bills));
                Object[] verifyDetailVOList = new VerifyDetailVO[busiDataVOList.size()];
                busiDataVOList.toArray(verifyDetailVOList);
                if (busiDataVOList.size() > 0) {
                    Collection verifyMainVOList = new BaseDAO().retrieveByClause(VerifyMainVO.class, SqlUtils.getInStr((String)"pk_verify", (String[])new String[]{verifyDetailVOList[0].getPk_verify()}));
                    if (verifyMainVOList != null && verifyMainVOList.size() > 0) {
                        String pkCurrtypeDe;
                        VerifyMainVO[] verifyMainVOArray = new VerifyMainVO[verifyMainVOList.size()];
                        verifyMainVOList.toArray(verifyMainVOArray);
                        Boolean isSensitiveTimeZone = InvocationInfoProxy.getInstance().getSensitiveTimeZone();
                        if (isSensitiveTimeZone.booleanValue()) {
                            ArapBillMulTime.processTimeZone((Object[])verifyDetailVOList);
                        }
                        if ((pkCurrtypeDe = verifyMainVOArray[0].getPk_currtype_de()) == null || "".equals(pkCurrtypeDe)) {
                            pkCurrtypeDe = verifyMainVOArray[0].getPk_currtype_cr();
                        }
                        Object[] verifDetailVOScale = new VerifyVOScaleUtil().verifDetailVOScale((VerifyDetailVO[])verifyDetailVOList, pkCurrtypeDe, verifyMainVOArray[0].getPk_currtype_cr());
                        return verifDetailVOScale;
                    }
                    break block8;
                }
                Boolean isSensitiveTimeZone = UFDateCfg.isSensitiveTimeZone();
                if (isSensitiveTimeZone.booleanValue()) {
                    ArapBillMulTime.processTimeZone((Object[])verifyDetailVOList);
                }
                return verifyDetailVOList;
            }
