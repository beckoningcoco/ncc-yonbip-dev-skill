...
public abstract class QueryBillServiceImpl
implements IQueryBillService {
    public PageQueryVO queryByQueryScheme(IQueryScheme qryScheme, String tradeType, Integer pageSize) throws BusinessException {
        Object beanid = qryScheme.get("bean_id");
        String selectSql = QueryBillServiceImpl.getSql(qryScheme, "N", beanid, tradeType);
        PaginationQueryVO vo = this.getQueryService().queryPaginationQueryVO(selectSql, Integer.valueOf(101));
    ...
    public static String getSql(IQueryScheme qryScheme, String initValue, Object beanid, String tradeType) throws BusinessException {
        String tableJoinFromWhereSQL;
        ...
        if (map != null && map.size() > 0 && (condVOs == null || ArrayUtils.isEmpty((Object[])condVOs))) {
            ...
            if (map.get("pk_org") != null) {
                QueryCondition condition = (QueryCondition)map.get("pk_org");
                SqlBuilder sql = new SqlBuilder();
                sql.append(table + "pk_org", condition.getValues());
                orgSql = sql.toString() + " AND ";
            }
        }
        ...
        if (!(tableJoinFromWhereSQL = qryScheme.getTableJoinFromWhereSQL().toString()).contains("pk_org")) {
            tableJoinFromWhereSQL = tableJoinFromWhereSQL + orgSql;
        }
        if (StringUtils.isNotBlank((CharSequence)tableJoinFromWhereSQL) && tableJoinFromWhereSQL.contains(table + "isrefund")) {
            tableJoinFromWhereSQL = tableJoinFromWhereSQL.replace(table + "isrefund", "isnull(" + table + "isrefund,'N')");
        }
        selectSql = selectSql + tableJoinFromWhereSQL;
        QueryCondition condition = qrySchemeProcessor.getQueryCondition("bisapproving");
        if (null != condition) {
            String bisapprovingJoinSql = " JOIN pub_workflownote wf ON " + table + tablepk + "=wf.billid AND wf.checkman= '" + AppContext.getInstance().getPkUser() + "' AND ( wf.dr=0 or isnull(wf.dr,0)=0 ) AND wf.actiontype in('Z','Z_A','Z_D') AND ischeck = 'N' AND wf.approvestatus<>4";
        ...
        if (!"W".equals(initValue)) {
            ...
            if (selectSql != null && selectSql.length() >= 0 && (selectSql.indexOf("where") >= 0 || selectSql.indexOf("WHERE") >= 0)) {
                String sql = selectSql.replace(" ", "");
                boolean combinAnd = sql.endsWith("WHERE") || selectSql.endsWith("where");
                selectSql = selectSql + (!combinAnd ? " AND " : " ") + table + "billstatus<>9 AND " + table + "isinit = '" + initValue + "' AND " + table + "pk_group = '" + pkGroup + "' ";
            } else {
                selectSql = " WHERE " + table + "isinit = '" + initValue + "' AND " + table + "billstatus<>9 AND " + table + "pk_group = '" + pkGroup + "' ";
            }
        }
        if (BillEnumCollection.VoucherStatus.VOUCHER_NOT_GENERATE.VALUE.equals(voucherStatus)) {
        ...
        if (tradeType != null) {
            selectSql = selectSql + " AND " + table + "pk_tradetype = '" + tradeType + "'";
        }
        if (verifyStatus != null && verifyStatus.length() == 1) {
    ...
    public abstract IQueryBillService getQueryService();
}
