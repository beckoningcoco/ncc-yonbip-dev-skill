 ...
 *  nccloud.vo.arap.bdprovision.ProvisionStatusEnum
 */
...
import nccloud.vo.arap.bdprovision.ProvisionStatusEnum;

public class ArapBDPConvert {
    public static final String PROVISIONBEGIN = "provisionBegin";
    public static final String PROVISIONEND = "provisionEnd";
    ...
    public String getConditionSql(Map<String, String> map, String tablename) throws BusinessException {
        ...
        String provisionBegin = map.get(PROVISIONBEGIN);
        String provisionEnd = map.get(PROVISIONEND);
        if (StringUtils.isEmpty((CharSequence)provisionBegin) || StringUtils.isEmpty((CharSequence)provisionEnd)) {
            return "1=2";
        }
        condition.append(" and (" + tablename + "." + "period" + ">='" + provisionBegin + "' ");
        condition.append(" and " + tablename + "." + "period" + "<='" + provisionEnd + "' )");
        ...
        if (!StringUtils.isEmpty((CharSequence)(prostatus = map.get("prostatus")))) {
            condition.append(" and ").append(tablename + "." + "prostatus" + "='" + this.getProStatusCode(prostatus) + "'");
        }
    ...
    private String getProStatusCode(String prostatus) {
        Integer status = null;
        if ("\u5df2\u8ba1\u63d0".equals(prostatus)) {
            status = ProvisionStatusEnum.PROVISIONED.VALUE;
        } else if ("\u672a\u8ba1\u63d0".equals(prostatus)) {
            status = ProvisionStatusEnum.UNPROVISION.VALUE;
        }
        return status == null ? null : status.toString();
    }
