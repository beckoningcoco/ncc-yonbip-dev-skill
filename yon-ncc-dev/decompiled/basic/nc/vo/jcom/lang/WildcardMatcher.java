/*
 * Decompiled with CFR 0.152.
 */
package nc.vo.jcom.lang;

class WildcardMatcher {
    WildcardMatcher() {
    }

    public static boolean match(String reg, String str) {
        return WildcardMatcher.match(reg, str, false);
    }

    public static boolean matchIgnoreCase(String reg, String str) {
        return WildcardMatcher.match(reg, str, true);
    }

    private static boolean match(String reg, String str, boolean ignoreCase) {
        if (str == null) {
            return false;
        }
        if (reg == null) {
            return false;
        }
        if (ignoreCase) {
            reg = reg.toLowerCase();
            str = str.toLowerCase();
        }
        boolean isMatch = WildcardMatcher.match0(reg, str);
        return isMatch;
    }

    private static void switchPosition(int begin, char[] chars) {
        int scale = 1;
        for (int i = begin; i < chars.length; ++i) {
            if (chars[i] == '?') {
                if (chars[i - 1] != '*') continue;
                chars[i - scale] = 63;
                chars[i] = 42;
                continue;
            }
            if (chars[i] != '*') break;
            ++scale;
        }
    }

    private static boolean match0(String reg, String str) {
        boolean isMatch = false;
        int regLen = reg.length();
        int strLen = str.length();
        int i = 0;
        int j = 0;
        char[] regChars = reg.toCharArray();
        while (i < regLen && j < strLen) {
            char rc = regChars[i];
            char sc = str.charAt(j);
            if (rc == '*') {
                if (i == regLen - 1) {
                    ++i;
                    j = strLen;
                    continue;
                }
                rc = reg.charAt(i + 1);
                if (rc == '?') {
                    WildcardMatcher.switchPosition(i + 1, regChars);
                    continue;
                }
                if (rc == '*') {
                    ++i;
                    continue;
                }
                ++i;
                int next = str.substring(j).lastIndexOf(rc);
                if (next == -1) break;
                j += next;
                continue;
            }
            if (rc != sc && rc != '?') break;
            ++i;
            ++j;
        }
        if (i < regLen && j >= strLen && WildcardMatcher.allIsStar(i, regChars)) {
            isMatch = true;
        } else if (i == regLen && j == strLen) {
            isMatch = true;
        }
        return isMatch;
    }

    private static boolean allIsStar(int begin, char[] chars) {
        boolean b = true;
        int count = chars.length;
        for (int i = begin; i < count; ++i) {
            if (chars[i] == '*') continue;
            b = false;
            break;
        }
        return b;
    }
}

