...
@Component
public class DmMerchantDeptParameterAuthRule
implements ICustomVersionAuthReferRpcZoneService {
    private static final Logger log = LoggerFactory.getLogger(DmMerchantDeptParameterAuthRule.class);
    ...
    public CustomAuthRuleResponse getValues(CustomAuthExtContent content) throws Exception {
        ...
        try {
            ...
            if (!CollectionUtils.isEmpty((Collection)deptIds)) {
                ...
                customAuthRuleResponse.setValues(this.getMerchantByDept(deptIds));
            } else {
                customAuthRuleResponse.setiSuccess(Boolean.valueOf(true));
                customAuthRuleResponse.setVersion(new Date());
            ...
            log.error("DmMerchantDeptParameterAuthRule" + e.getMessage(), (Throwable)e);
    ...
    public Set<String> getMerchantByDept(List<String> deptIds) throws Exception {
        QueryConditionGroup conditionGroup = QueryConditionGroup.and((ConditionExpression[])new ConditionExpression[]{QueryCondition.name((String)"specialManagementDep").in(deptIds)});
        QuerySchema schema = QuerySchema.create().addSelect("merchantId").addCondition((ConditionExpression)conditionGroup);
        List principalInDbList = MetaDaoHelper.queryObject((String)"aa.merchant.Principal", (QuerySchema)schema, null);
        if (!CollectionUtils.isEmpty((Collection)principalInDbList)) {
            return principalInDbList.stream().map(map -> String.valueOf(map.getMerchantId())).collect(Collectors.toSet());
        }
