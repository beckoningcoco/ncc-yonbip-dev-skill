...
@Component
public class StaffPtJobHigherAndLowerDeptRule
extends DPBaseRule {
    private static final Logger log = LoggerFactory.getLogger(StaffPtJobHigherAndLowerDeptRule.class);
    @Autowired
    ...
    protected Set<String> getCustomValues(CustomAuthContent content, String personId) throws Exception {
        Set<String> ptJobDeptIdList = this.dmPtDeptAuthRule.getCustomValues(content, personId);
        if (!CollectionUtils.isEmpty(ptJobDeptIdList)) {
            Set<String> childrenDeptList;
            HashSet<String> deptIdSet = new HashSet<String>();
            Set<String> parentDeptList = this.deptQueryService.getParentDeptList(ptJobDeptIdList);
