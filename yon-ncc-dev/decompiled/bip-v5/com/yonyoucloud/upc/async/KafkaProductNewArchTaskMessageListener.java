...
@DependsOn(value={"kafkaMQProductNewArchProducer"})
public class KafkaProductNewArchTaskMessageListener
implements InitializingBean {
    private static final Logger log = LoggerFactory.getLogger(KafkaProductNewArchTaskMessageListener.class);
    ...
    @KafkaListener(topics={"#{kafkaTopicService.getProductSynNewArchTopic()}"}, groupId="iuap_coredoc_group_productnewarch")
    public void onMessage(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        try {
            if (!CollectionUtils.isEmpty(records)) {
                this.mqSaveProductNewArchTaskService.dealWithAck(records, ack);
            }
        }
        catch (Throwable ex) {
            log.error(KafkaProductNewArchTaskMessageListener.class + "MQ Message Handle Error.", (Object)ex.getMessage(), (Object)ex);
        }
    }
    ...
    public void afterPropertiesSet() {
        ...
        this.retryTemplate.setListeners(new RetryListener[]{new RetryListenerSupport(){
            ...
            public <T, E extends Throwable> void onError(RetryContext context, RetryCallback<T, E> callback, Throwable t) {
                log.error(InternationalUtils.getMessageWithDefault((String)"UID:P_COREDOC-BE_179F628604080539", (String)"\u5904\u7406\u5f02\u5e38"), t);
            }
        }});
    }
}
