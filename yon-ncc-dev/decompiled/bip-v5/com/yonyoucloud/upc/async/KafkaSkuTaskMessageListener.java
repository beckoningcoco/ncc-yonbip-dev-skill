...
@Component(value="kafkaSkuTaskMessageListener")
@ConditionalOnProperty(name={"kafka.cluster.hosts"}, matchIfMissing=false)
@DependsOn(value={"kafkaMQSkuProducer"})
public class KafkaSkuTaskMessageListener
implements InitializingBean {
    private static final Logger log = LoggerFactory.getLogger(KafkaSkuTaskMessageListener.class);
    private static final Logger logger = LoggerFactory.getLogger(KafkaSkuTaskMessageListener.class);
    ...
    @KafkaListener(topics={"#{kafkaTopicService.getSkuSyncTopic()}"}, groupId="iuap_coredoc_group_coredocsync")
    public void onMessage(List<ConsumerRecord<?, ?>> records, Acknowledgment ack) {
        try {
            if (!CollectionUtils.isEmpty(records)) {
                this.mQSaveSkuTaskService.dealWithAck(records, ack);
            }
        }
        catch (Throwable e) {
            log.error(KafkaSkuTaskMessageListener.class + "MQ Message Handle Error." + e.getMessage(), e);
        }
    }
    ...
    public void afterPropertiesSet() {
        ...
        this.retryTemplate.setListeners(new RetryListener[]{new RetryListenerSupport(){
            ...
            public <T, E extends Throwable> void onError(RetryContext context, RetryCallback<T, E> callback, Throwable t) {
                log.error(InternationalUtils.getMessageWithDefault((String)"UID:P_COREDOC-BE_179F628604080222", (String)"\u5904\u7406\u5f02\u5e38"), (Object)t.getMessage(), (Object)t);
            }
        }});
    }
}
