...
@Component(value="merchantLockRule")
public class MerchantLockRule
extends AbstractCommonRule {
    private static final Logger log = LoggerFactory.getLogger(MerchantLockRule.class);
        ...
        for (BizObject bill : bills) {
            ...
            if (((Boolean)bill.get("frozenTogetherValue")).booleanValue()) {
                ...
                for (MerchantApplyRangeDetail mard : merchantApplyRangeDetails) {
                    mard.setLocker(AppContext.getCurrentUser().getName());
                    mard.setLockTime(new Date());
                    mard.setLockDate(new Date());
                    mard.setFrozenState(FrozenState.frozen);
                    List freezinglinks = (List)bill.get("freezingLink");
                    StringBuffer stringBuffer = new StringBuffer();
                    for (int i = 0; i < freezinglinks.size(); ++i) {
                        stringBuffer.append((String)freezinglinks.get(i));
                        if (i == freezinglinks.size() - 1) continue;
                        stringBuffer.append(",");
                    }
                    mard.setFreezingLink(stringBuffer.toString());
                ...
                merchantApplyRangeDetail.setLocker(AppContext.getCurrentUser().getName());
                ...
                merchantApplyRangeDetail.setFreezingLink(stringBuffer.toString());
