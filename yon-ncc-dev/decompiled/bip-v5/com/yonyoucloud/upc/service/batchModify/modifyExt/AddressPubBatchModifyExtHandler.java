...
@Service
public class AddressPubBatchModifyExtHandler
implements IPubBatchModifyEntityExtService {
    private static final Logger log = LoggerFactory.getLogger(AddressPubBatchModifyExtHandler.class);
    @Autowired
    ...
    public void batchModifyIsCheckField(BizObject bizObject, Map<String, Object> pubBatchModifyEntityFieldSaveIsCheckMap, PubBatchModifyEntitySaveDTO pubBatchModifyEntitySaveDTO) throws Exception {
        ...
        addressInfo.init((Map)bizObject);
        addressInfo.setEntityStatus(EntityStatus.Update);
        QuerySchema querySchema = QuerySchema.create();
        querySchema.addSelect("addressCode,isDefault,merchantId");
        QueryConditionGroup queryConditionGroup = QueryConditionGroup.and((ConditionExpression[])new ConditionExpression[]{QueryCondition.name((String)"id").eq(addressInfo.getId())});
        querySchema.addCondition((ConditionExpression)queryConditionGroup);
        List addressInDbList = MetaDaoHelper.queryObject((String)"aa.merchant.AddressInfo", (QuerySchema)querySchema, null);
        if (!CollectionUtils.isEmpty((Collection)addressInDbList)) {
            querySchema = QuerySchema.create();
            querySchema.addSelect("code,name,createOrg,creditCode");
            querySchema.addCondition((ConditionExpression)QueryConditionGroup.and((ConditionExpression[])new ConditionExpression[]{QueryCondition.name((String)"id").eq((Object)((AddressInfo)addressInDbList.get(0)).getMerchantId())}));
            ...
            if (!CollectionUtils.isEmpty((Collection)merchantInDbList)) {
                ...
                addressInfo.setIsDefault(((AddressInfo)addressInDbList.get(0)).getIsDefault());
                addressInfo.setAddressCode(((AddressInfo)addressInDbList.get(0)).getAddressCode());
                merchant.setId((Object)((AddressInfo)addressInDbList.get(0)).getMerchantId());
                ...
                merchant.setMerchantAddressInfos(addressInfoList);
    ...
    public void checkBatchModifyEntity(BizObject bizObject) throws Exception {
        if (AppContext.getCurrentUser().getUserType() == UserType.TenantAdmin || AppContext.getCurrentUser().getUserType() == UserType.TenantEmployee) {
            ...
            querySchema.addCondition((ConditionExpression)queryConditionGroup);
            List addressInDbList = MetaDaoHelper.queryObject((String)"aa.merchant.AddressInfo", (QuerySchema)querySchema, null);
            if (!CollectionUtils.isEmpty((Collection)addressInDbList) && this.orgExternalQryService.judgeExternalOrg((String)AppContext.getYTenantId(), (String)((AddressInfo)addressInDbList.get(0)).get("createOrg")).booleanValue()) {
                throw new CoreDocBusinessException(CoreExtrateBusinessErrorEnum.ERROR_ENUM_000_527_801436);
            }
        }
    }
    ...
    public void checkBatchModifyEntityFieldSaveMap(Map<String, Object> pubBatchModifyEntityFieldSaveMap) {
        ...
        if (pubBatchModifyEntityFieldSaveMap.containsKey("address") && pubBatchModifyEntityFieldSaveMap.get("address") == null) {
            throw new CoreDocBusinessException(CoreDocBusinessErrorEnum.MERCHANT_SUB_LIST_ADDRESS);
        }
    }
