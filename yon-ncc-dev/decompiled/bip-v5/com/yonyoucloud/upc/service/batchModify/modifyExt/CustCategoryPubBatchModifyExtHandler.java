...
@Service
public class CustCategoryPubBatchModifyExtHandler
implements IPubBatchModifyEntityExtService {
    private static final Logger log = LoggerFactory.getLogger(CustCategoryPubBatchModifyExtHandler.class);
    ...
    public void extendQuerySchema(QuerySchema querySchema, PubBatchModifyEntitySaveDTO pubBatchModifyEntitySaveDTO) throws Exception {
        ...
        if (!CollectionUtils.isEmpty((Collection)extIdList)) {
            querySchema.appendQueryCondition(new ConditionExpression[]{QueryCondition.name((String)"custCategoryApplyRanges.id").in((Collection)extIdList)});
            querySchema.addSelect("custCategoryApplyRanges.isCreator as isCreator,code");
        }
    }
