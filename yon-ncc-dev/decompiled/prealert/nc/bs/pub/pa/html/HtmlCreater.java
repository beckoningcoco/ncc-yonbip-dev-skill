...
import nc.bs.pub.pa.html.IAlertMessage1;
...
extends Thread {
    ...
    public static String getFormattedAlertMessageHTML(IAlertMessage alertMessage) {
        StringBuffer buffer = HtmlCreater.fillHeader(alertMessage.getTitle());
        HtmlCreater.fillContent(alertMessage, buffer);
        buffer.append("\n</body>");
        buffer.append("\n</html>");
    ...
    public static String getFormattedAlertMessageHTML(IAlertMessage alertMessage, AlertregistryVO info) {
        StringBuffer buffer = HtmlCreater.fillHeader(alertMessage.getTitle(), info.getMultiLangAlertname());
        HtmlCreater.fillContent(alertMessage, buffer, info.getMultiLangAlertname());
        buffer.append("\n</body>");
        buffer.append("\n</html>");
        return buffer.toString();
    }
    ...
    private static StringBuffer fillHeader(String msgTilte) {
        ...
        buffer.append("</title>\n<head>");
        buffer.append("<meta http-equiv=\"Content-Type\" content=\"text/html; charset=" + PaConstant.getCharset() + "\" >");
        buffer.append("</head>\n");
        buffer.append("<body>\n");
        return buffer;
    }
    ...
    public static String getFormattedalertMessageHTMs(IAlertMessage[] alertMessage) {
        Logger.debug((Object)"Entering HtmlCreater.getFormattedalertMessageHTML2()");
        StringBuffer buffer = HtmlCreater.fillHeader(alertMessage[0].getTitle());
        for (int k = 0; k < alertMessage.length; ++k) {
            Logger.debug((Object)("Formatted Messages start: " + k));
            if (alertMessage[k] == null) continue;
            HtmlCreater.fillContent(alertMessage[k], buffer);
        }
        buffer.append("\n</body>");
        buffer.append("\n</html>");
        Logger.debug((Object)"Leaving HtmlCreater.getFormattedalertMessageHTML2()");
        return buffer.toString();
    }
    ...
    public static String getStringAlertMessageHTML(String message) {
        StringBuffer buffer = HtmlCreater.fillHeader("alert message");
        if (message != null) {
    ...
    public static String getStringAlertMessageHTMLInBody(String message) {
        StringBuffer buffer = HtmlCreater.fillHeader("alert message");
        if (message != null) {
    ...
    public static String getTableAlertMessage(IAlertMessage alertMessage) {
        Logger.debug((Object)"Entering HtmlCreater.getTableAlertMessage()");
        String width = "100";
        if (alertMessage instanceof IAlertMessage1) {
            ...
            try {
                ...
                Logger.warn((Object)"Illegal character string representing the width.and use the defautl value 100");
                width = "100";
            }
        }
        float[] columnWidth = HtmlCreater.transvertColumnWidth(alertMessage);
        StringBuffer buffer = new StringBuffer();
        if (alertMessage.getBodyFields() != null) {
            ...
            buffer.append("<tr>");
            for (i = 0; i < alertMessage.getBodyFields().length; ++i) {
                buffer.append("<td width=\"" + columnWidth[i] * 100.0f + "%\" align=\"center\" >" + alertMessage.getBodyFields()[i] + "</td>");
            }
            for (i = alertMessage.getBodyFields().length; i < columnWidth.length; ++i) {
            ...
            if (alertMessage.getBodyValue() != null) {
                if (alertMessage instanceof IAlertMessage2) {
                    HtmlCreater.renderTableBody2((IAlertMessage2)alertMessage, columnWidth, buffer);
                } else {
                    HtmlCreater.renderTableBody(alertMessage, columnWidth, buffer);
                }
            }
            buffer.append("</table>");
        }
        Logger.debug((Object)"leaving HtmlCreater.getTableAlertMessage()");
        return buffer.toString();
    }
    ...
    private static void renderTableBody(IAlertMessage alertMessage, float[] columnWidth, StringBuffer buffer) {
        for (int i = 0; i < alertMessage.getBodyValue().length; ++i) {
            ...
            if (alertMessage.getBodyValue()[i] != null) {
                for (j = 0; j < alertMessage.getBodyValue()[i].length; ++j) {
                    Object value = alertMessage.getBodyValue()[i][j];
                    String columnAlignTag = HtmlCreater.getAlignByType(HtmlCreater.getValueType(value));
                    buffer.append("<td width=\"" + columnWidth[j] * 100.0f + "%\" " + columnAlignTag + " >" + (value != null ? value.toString() : "") + "</td>");
                }
            }
            for (j = alertMessage.getBodyValue()[i].length; j < columnWidth.length; ++j) {
        ...
        for (int i = 0; i < alertMessage.getBodyValue().length; ++i) {
            ...
            if (alertMessage.getBodyValue()[i] != null) {
                for (j = 0; j < alertMessage.getBodyValue()[i].length; ++j) {
                    ...
                    if (i == 0) {
                        columnProperty = HtmlCreater.initColumnProperty(alertMessage);
                    }
                    String columnAlignTag = null == columnProperty[j] ? " align=\"center\" " : columnProperty[j];
                    buffer.append("<td width=\"" + columnWidth[j] * 100.0f + "%\" " + columnAlignTag + " >" + (value == null ? (strNull != null ? strNull : "") : HtmlCreater.wrapValue(value, type)) + "</td>");
                }
            }
            for (j = alertMessage.getBodyValue()[i].length; j < columnWidth.length; ++j) {
    ...
    protected static String wrapValue(Object value, int type) {
        ...
        switch (type) {
            case 5: {
                res = HtmlCreater.getInstanceOfNumberFormat().format(value);
                break;
            }
            case 6: {
    ...
    private static String[] initColumnProperty(IAlertMessage2 alertMessage2) {
        ...
        if (alertMessage2 != null) {
            ...
            for (int i = 0; i < types.length; ++i) {
                columnprop[i] = HtmlCreater.getAlignByType(types[i]);
            }
        }
        return columnprop;
    }
    ...
    private static float[] transvertColumnWidth(IAlertMessage alertMessage) {
        Logger.debug((Object)"Entering HtmlCreater.transvertColumnWidth()");
        int columnCount = 0;
        if (alertMessage.getBodyFields() != null) {
        ...
        if (alertMessage.getBodyWidths() != null) {
            if (alertMessage.getBodyWidths().length < columnCount) {
                if (columnCount != 0) {
                    int i;
                    for (i = 0; i < alertMessage.getBodyWidths().length; ++i) {
                        result[i] = alertMessage.getBodyWidths()[i] * ((float)alertMessage.getBodyWidths().length / (float)columnCount);
                    }
                    for (i = alertMessage.getBodyWidths().length; i < columnCount; ++i) {
                        result[i] = alertMessage.getBodyWidths().length / columnCount;
                    }
                }
            } else if (alertMessage.getBodyWidths().length > columnCount) {
                for (int i = 0; i < columnCount; ++i) {
                    result[i] = alertMessage.getBodyWidths()[i] * ((float)columnCount / (float)alertMessage.getBodyWidths().length);
                }
            } else {
        ...
        Logger.debug((Object)"leaving HtmlCreater.transvertColumnWidth()");
        return result;
    }
}
