# |<<

**关联账簿 (fiarc_relatedbooks / nc.vo.archive.archiveinit.RelatedbooksVO)**

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/2214.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_relatedbooks | 关联账簿主键 | pk_relatedbooks | char(20) | √ | 主键 (UFID) |
| 2 | pk_accountingbook | 账簿主键 | pk_accountingbook | varchar(20) | √ | 账簿_财务核算账簿 (accountingbook) |
| 3 | pk_relorg | 财务组织 | pk_relorg | varchar(20) |  | 组织_业务单元_财务组织 (financeorg) |
| 4 | pk_group | 集团 | pk_group | varchar(20) |  | 组织_集团 (group) |
| 5 | pk_org | 组织 | pk_org | varchar(20) |  | 组织 (org) |
| 6 | creator | 创建人 | creator | varchar(20) |  | 用户 (user) |
| 7 | creationtime | 创建时间 | creationtime | char(19) |  | 日期时间 (UFDateTime) |
| 8 | modifier | 修改人 | modifier | varchar(20) |  | 用户 (user) |
| 9 | modifiedtime | 修改时间 | modifiedtime | char(19) |  | 日期时间 (UFDateTime) |