# |<<

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/7278.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_workcalendrule | pk_workcalendrule | pk_workcalendrule | char(20) | √ |
| 2 | code | code | code | varchar2(40) | √ |
| 3 | dataoriginflag | dataoriginflag | dataoriginflag | number(38, 0) |
| 4 | friday | friday | friday | char(1) |
| 5 | memo | memo | memo | varchar2(300) |
| 6 | monday | monday | monday | char(1) |
| 7 | name | name | name | varchar2(300) | √ |
| 8 | name2 | name2 | name2 | varchar2(300) |
| 9 | name3 | name3 | name3 | varchar2(300) |
| 10 | name4 | name4 | name4 | varchar2(300) |
| 11 | name5 | name5 | name5 | varchar2(300) |
| 12 | name6 | name6 | name6 | varchar2(300) |
| 13 | offdutytime | offdutytime | offdutytime | varchar2(8) |
| 14 | ondutytime | ondutytime | ondutytime | varchar2(8) |
| 15 | pk_group | pk_group | pk_group | varchar2(20) |  |  | '~' |
| 16 | pk_org | pk_org | pk_org | char(20) | √ |
| 17 | saturday | saturday | saturday | char(1) |
| 18 | sunday | sunday | sunday | char(1) |
| 19 | thursday | thursday | thursday | char(1) |
| 20 | tuesday | tuesday | tuesday | char(1) |
| 21 | wednesday | wednesday | wednesday | char(1) |
| 22 | creationtime | creationtime | creationtime | char(19) |
| 23 | creator | creator | creator | varchar2(20) |  |  | '~' |
| 24 | dr | dr | dr | number(10, 0) |  |  | 0 |
| 25 | modifiedtime | modifiedtime | modifiedtime | char(19) |
| 26 | modifier | modifier | modifier | varchar2(20) |  |  | '~' |
| 27 | ts | ts | ts | char(19) |  |  | to_char(sysdate,'yyyy-mm-dd hh24:mi:ss') |