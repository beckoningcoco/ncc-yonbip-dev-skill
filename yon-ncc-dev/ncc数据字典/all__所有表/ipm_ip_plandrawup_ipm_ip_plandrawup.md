# |<<

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/9462.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_plandrawup | pk_plandrawup | pk_plandrawup | char(20) | √ |
| 2 | adjuststate | adjuststate | adjuststate | varchar2(50) |
| 3 | approver | approver | approver | varchar2(20) |  |  | '~' |
| 4 | billmaker | billmaker | billmaker | varchar2(20) |  |  | '~' |
| 5 | bisgathered | bisgathered | bisgathered | char(1) |
| 6 | bislastver | bislastver | bislastver | char(1) |
| 7 | cbilltypecode | cbilltypecode | cbilltypecode | varchar2(20) |
| 8 | ccurrencyid | ccurrencyid | ccurrencyid | varchar2(20) |  |  | '~' |
| 9 | cfirstid | cfirstid | cfirstid | char(20) |
| 10 | cglobalcurrencyid | cglobalcurrencyid | cglobalcurrencyid | varchar2(20) |  |  | '~' |
| 11 | cgroupcurrencyid | cgroupcurrencyid | cgroupcurrencyid | varchar2(20) |  |  | '~' |
| 12 | corigcurrencyid | corigcurrencyid | corigcurrencyid | varchar2(20) |  |  | '~' |
| 13 | csrcid | csrcid | csrcid | char(20) |
| 14 | denddate | denddate | denddate | char(19) |
| 15 | dmakedate | dmakedate | dmakedate | char(19) |
| 16 | dstartdate | dstartdate | dstartdate | char(19) |
| 17 | fstatusflag | fstatusflag | fstatusflag | number(38, 0) |
| 18 | nallownsummny | nallownsummny | nallownsummny | number(28, 8) |
| 19 | nalltotalsummny | nalltotalsummny | nalltotalsummny | number(28, 8) |
| 20 | nbaseownsummny | nbaseownsummny | nbaseownsummny | number(28, 8) |
| 21 | nbasetotalsummny | nbasetotalsummny | nbasetotalsummny | number(28, 8) |
| 22 | nexchangerate | nexchangerate | nexchangerate | number(28, 8) |
| 23 | nglobalexchgrate | nglobalexchgrate | nglobalexchgrate | number(28, 8) |
| 24 | ngpownsummny | ngpownsummny | ngpownsummny | number(28, 8) |
| 25 | ngptotalsummny | ngptotalsummny | ngptotalsummny | number(28, 8) |
| 26 | ngroupexchgrate | ngroupexchgrate | ngroupexchgrate | number(28, 8) |
| 27 | norigownsummny | norigownsummny | norigownsummny | number(28, 8) |
| 28 | norigtotalsummny | norigtotalsummny | norigtotalsummny | number(28, 8) |
| 29 | pk_adjustdept | pk_adjustdept | pk_adjustdept | varchar2(20) |  |  | '~' |
| 30 | pk_adjustdept_v | pk_adjustdept_v | pk_adjustdept_v | varchar2(20) |  |  | '~' |
| 31 | pk_adjustpsn | pk_adjustpsn | pk_adjustpsn | varchar2(20) |  |  | '~' |
| 32 | pk_busitype | pk_busitype | pk_busitype | varchar2(20) |  |  | '~' |
| 33 | pk_dept | pk_dept | pk_dept | varchar2(20) |  |  | '~' |
| 34 | pk_dept_v | pk_dept_v | pk_dept_v | varchar2(20) |  |  | '~' |
| 35 | pk_group | pk_group | pk_group | char(20) | √ |
| 36 | pk_org | pk_org | pk_org | varchar2(20) |  |  | '~' |
| 37 | pk_org_v | pk_org_v | pk_org_v | varchar2(20) |  |  | '~' |
| 38 | pk_origdrawup | pk_origdrawup | pk_origdrawup | varchar2(20) |
| 39 | pk_psndoc | pk_psndoc | pk_psndoc | varchar2(20) |  |  | '~' |
| 40 | pk_transtype | pk_transtype | pk_transtype | varchar2(20) |  |  | '~' |
| 41 | submitstate | submitstate | submitstate | varchar2(50) |
| 42 | taudittime | taudittime | taudittime | char(19) |
| 43 | vadjustmemo | vadjustmemo | vadjustmemo | varchar2(3072) |
| 44 | vapprovenote | vapprovenote | vapprovenote | varchar2(1024) |
| 45 | vbillcode | vbillcode | vbillcode | varchar2(50) |
| 46 | vbillname | vbillname | vbillname | varchar2(1200) |
| 47 | version | version | version | number(28, 8) |
| 48 | vfirstcode | vfirstcode | vfirstcode | varchar2(50) |
| 49 | vfirsttrantype | vfirsttrantype | vfirsttrantype | varchar2(20) |
| 50 | vfirsttype | vfirsttype | vfirsttype | varchar2(20) |
| 51 | vmemo | vmemo | vmemo | varchar2(3072) |
| 52 | vmonth | vmonth | vmonth | varchar2(50) |
| 53 | vquarter | vquarter | vquarter | varchar2(50) |
| 54 | vreserve1 | vreserve1 | vreserve1 | varchar2(100) |
| 55 | vreserve2 | vreserve2 | vreserve2 | varchar2(100) |
| 56 | vreserve3 | vreserve3 | vreserve3 | varchar2(100) |
| 57 | vreserve4 | vreserve4 | vreserve4 | varchar2(100) |
| 58 | vreserve5 | vreserve5 | vreserve5 | varchar2(100) |
| 59 | vreserve6 | vreserve6 | vreserve6 | varchar2(100) |
| 60 | vreserve7 | vreserve7 | vreserve7 | varchar2(100) |
| 61 | vreserve8 | vreserve8 | vreserve8 | varchar2(100) |
| 62 | vreserve9 | vreserve9 | vreserve9 | varchar2(100) |
| 63 | vreserve10 | vreserve10 | vreserve10 | varchar2(100) |
| 64 | vsrccode | vsrccode | vsrccode | varchar2(50) |
| 65 | vsrctrantype | vsrctrantype | vsrctrantype | varchar2(20) |
| 66 | vsrctype | vsrctype | vsrctype | varchar2(20) |
| 67 | vtrantypecode | vtrantypecode | vtrantypecode | varchar2(20) |
| 68 | vyear | vyear | vyear | varchar2(20) |
| 69 | vdef1 | vdef1 | vdef1 | varchar2(101) |
| 70 | vdef2 | vdef2 | vdef2 | varchar2(101) |
| 71 | vdef3 | vdef3 | vdef3 | varchar2(101) |
| 72 | vdef4 | vdef4 | vdef4 | varchar2(101) |
| 73 | vdef5 | vdef5 | vdef5 | varchar2(101) |
| 74 | vdef6 | vdef6 | vdef6 | varchar2(101) |
| 75 | vdef7 | vdef7 | vdef7 | varchar2(101) |
| 76 | vdef8 | vdef8 | vdef8 | varchar2(101) |
| 77 | vdef9 | vdef9 | vdef9 | varchar2(101) |
| 78 | vdef10 | vdef10 | vdef10 | varchar2(101) |
| 79 | vdef11 | vdef11 | vdef11 | varchar2(101) |
| 80 | vdef12 | vdef12 | vdef12 | varchar2(101) |
| 81 | vdef13 | vdef13 | vdef13 | varchar2(101) |
| 82 | vdef14 | vdef14 | vdef14 | varchar2(101) |
| 83 | vdef15 | vdef15 | vdef15 | varchar2(101) |
| 84 | vdef16 | vdef16 | vdef16 | varchar2(101) |
| 85 | vdef17 | vdef17 | vdef17 | varchar2(101) |
| 86 | vdef18 | vdef18 | vdef18 | varchar2(101) |
| 87 | vdef19 | vdef19 | vdef19 | varchar2(101) |
| 88 | vdef20 | vdef20 | vdef20 | varchar2(101) |
| 89 | vdef21 | vdef21 | vdef21 | varchar2(101) |
| 90 | vdef22 | vdef22 | vdef22 | varchar2(101) |
| 91 | vdef23 | vdef23 | vdef23 | varchar2(101) |
| 92 | vdef24 | vdef24 | vdef24 | varchar2(101) |
| 93 | vdef25 | vdef25 | vdef25 | varchar2(101) |
| 94 | vdef26 | vdef26 | vdef26 | varchar2(101) |
| 95 | vdef27 | vdef27 | vdef27 | varchar2(101) |
| 96 | vdef28 | vdef28 | vdef28 | varchar2(101) |
| 97 | vdef29 | vdef29 | vdef29 | varchar2(101) |
| 98 | vdef30 | vdef30 | vdef30 | varchar2(101) |
| 99 | vdef31 | vdef31 | vdef31 | varchar2(101) |
| 100 | vdef32 | vdef32 | vdef32 | varchar2(101) |
| 101 | vdef33 | vdef33 | vdef33 | varchar2(101) |
| 102 | vdef34 | vdef34 | vdef34 | varchar2(101) |
| 103 | vdef35 | vdef35 | vdef35 | varchar2(101) |
| 104 | vdef36 | vdef36 | vdef36 | varchar2(101) |
| 105 | vdef37 | vdef37 | vdef37 | varchar2(101) |
| 106 | vdef38 | vdef38 | vdef38 | varchar2(101) |
| 107 | vdef39 | vdef39 | vdef39 | varchar2(101) |
| 108 | vdef40 | vdef40 | vdef40 | varchar2(101) |
| 109 | creationtime | creationtime | creationtime | char(19) |
| 110 | creator | creator | creator | varchar2(20) |  |  | '~' |
| 111 | dr | dr | dr | number(10, 0) |  |  | 0 |
| 112 | modifiedtime | modifiedtime | modifiedtime | char(19) |
| 113 | modifier | modifier | modifier | varchar2(20) |  |  | '~' |
| 114 | ts | ts | ts | char(19) |  |  | to_char(sysdate,'yyyy-mm-dd hh24:mi:ss') |