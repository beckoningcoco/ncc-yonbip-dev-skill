# |<<

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/11179.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_balancepay_discard | pk_balancepay_discard | pk_balancepay_discard | char(20) | √ |
| 2 | discardmoney | discardmoney | discardmoney | number(28, 8) |
| 3 | discard_num | discard_num | discard_num | number(28, 8) |
| 4 | discard_price | discard_price | discard_price | number(28, 8) |
| 5 | main_num | main_num | main_num | number(28, 8) |
| 6 | memo | memo | memo | varchar2(200) |
| 7 | pk_balancepay | pk_balancepay | pk_balancepay | char(20) | √ |
| 8 | pk_contract_base | pk_contract_base | pk_contract_base | varchar2(20) |  |  | '~' |
| 9 | pk_contract_v | pk_contract_v | pk_contract_v | varchar2(20) |  |  | '~' |
| 10 | pk_currency_origin | pk_currency_origin | pk_currency_origin | varchar2(20) |  |  | '~' |
| 11 | pk_group | pk_group | pk_group | varchar2(20) |  |  | '~' |
| 12 | pk_main_unit | pk_main_unit | pk_main_unit | varchar2(20) |  |  | '~' |
| 13 | pk_marbaseclass | pk_marbaseclass | pk_marbaseclass | varchar2(20) |  |  | '~' |
| 14 | pk_material | pk_material | pk_material | varchar2(20) |  |  | '~' |
| 15 | pk_material_v | pk_material_v | pk_material_v | varchar2(20) |  |  | '~' |
| 16 | pk_org | pk_org | pk_org | varchar2(20) |  |  | '~' |
| 17 | pk_org_v | pk_org_v | pk_org_v | varchar2(20) |  |  | '~' |
| 18 | pk_rent_unit | pk_rent_unit | pk_rent_unit | varchar2(20) |  |  | '~' |
| 19 | rent_changerate | rent_changerate | rent_changerate | varchar2(50) |
| 20 | rowno | rowno | rowno | varchar2(30) |
| 21 | src_bill_code | src_bill_code | src_bill_code | varchar2(40) |
| 22 | src_bill_type | src_bill_type | src_bill_type | varchar2(20) |
| 23 | src_body_rowno | src_body_rowno | src_body_rowno | varchar2(30) |
| 24 | src_pk_bill | src_pk_bill | src_pk_bill | varchar2(20) |
| 25 | src_pk_bill_b | src_pk_bill_b | src_pk_bill_b | varchar2(20) |
| 26 | src_transi_type | src_transi_type | src_transi_type | varchar2(20) |
| 27 | def1 | def1 | def1 | varchar2(101) |
| 28 | def2 | def2 | def2 | varchar2(101) |
| 29 | def3 | def3 | def3 | varchar2(101) |
| 30 | def4 | def4 | def4 | varchar2(101) |
| 31 | def5 | def5 | def5 | varchar2(101) |
| 32 | def6 | def6 | def6 | varchar2(101) |
| 33 | def7 | def7 | def7 | varchar2(101) |
| 34 | def8 | def8 | def8 | varchar2(101) |
| 35 | def9 | def9 | def9 | varchar2(101) |
| 36 | def10 | def10 | def10 | varchar2(101) |
| 37 | def11 | def11 | def11 | varchar2(101) |
| 38 | def12 | def12 | def12 | varchar2(101) |
| 39 | def13 | def13 | def13 | varchar2(101) |
| 40 | def14 | def14 | def14 | varchar2(101) |
| 41 | def15 | def15 | def15 | varchar2(101) |
| 42 | def16 | def16 | def16 | varchar2(101) |
| 43 | def17 | def17 | def17 | varchar2(101) |
| 44 | def18 | def18 | def18 | varchar2(101) |
| 45 | def19 | def19 | def19 | varchar2(101) |
| 46 | def20 | def20 | def20 | varchar2(101) |
| 47 | dr | dr | dr | number(10, 0) |  |  | 0 |
| 48 | ts | ts | ts | char(19) |  |  | to_char(sysdate,'yyyy-mm-dd hh24:mi:ss') |