# |<<

> https://community.yonyou.com/datadict/datadict-2105yiliao/ddc/11198.html

| 序号 | 属性编码 | 属性名称 | 字段编码 | 字段类型 | 是否必输 | 引用模型 | 默认值 | 取值范围/枚举 |
|------|------|------|------|------|------|------|------|------|
| 1 | pk_entrance_b | pk_entrance_b | pk_entrance_b | char(20) | √ |
| 2 | changerate | changerate | changerate | varchar2(50) |
| 3 | close_flag | close_flag | close_flag | char(1) |
| 4 | fact_main_num | fact_main_num | fact_main_num | number(28, 8) |
| 5 | fact_num | fact_num | fact_num | number(28, 8) |
| 6 | ismaterial_flag | ismaterial_flag | ismaterial_flag | char(1) |
| 7 | main_num | main_num | main_num | number(28, 8) |
| 8 | memo | memo | memo | varchar2(200) |
| 9 | nbatchcode | nbatchcode | nbatchcode | varchar2(40) |
| 10 | num | num | num | number(28, 8) |
| 11 | pk_contract_b_base_src | pk_contract_b_base_src | pk_contract_b_base_src | varchar2(20) |
| 12 | pk_contract_b_src | pk_contract_b_src | pk_contract_b_src | varchar2(20) |
| 13 | pk_entrance | pk_entrance | pk_entrance | char(20) | √ |
| 14 | pk_group | pk_group | pk_group | varchar2(20) |  |  | '~' |
| 15 | pk_jobmngfil | pk_jobmngfil | pk_jobmngfil | varchar2(20) |  |  | '~' |
| 16 | pk_main_unit | pk_main_unit | pk_main_unit | varchar2(20) |  |  | '~' |
| 17 | pk_marbaseclass | pk_marbaseclass | pk_marbaseclass | varchar2(20) |  |  | '~' |
| 18 | pk_material | pk_material | pk_material | varchar2(20) | √ |  | '~' |
| 19 | pk_material_v | pk_material_v | pk_material_v | varchar2(20) | √ |  | '~' |
| 20 | pk_org | pk_org | pk_org | varchar2(20) |  |  | '~' |
| 21 | pk_org_v | pk_org_v | pk_org_v | varchar2(20) |  |  | '~' |
| 22 | pk_producer | pk_producer | pk_producer | varchar2(20) |  |  | '~' |
| 23 | pk_rent_unit | pk_rent_unit | pk_rent_unit | varchar2(20) |  |  | '~' |
| 24 | pk_supplier | pk_supplier | pk_supplier | varchar2(20) |  |  | '~' |
| 25 | pk_unit | pk_unit | pk_unit | varchar2(20) |  |  | '~' |
| 26 | plan_end_date | plan_end_date | plan_end_date | char(19) |
| 27 | rent_changerate | rent_changerate | rent_changerate | varchar2(50) |
| 28 | rent_num | rent_num | rent_num | number(28, 8) |
| 29 | rent_price | rent_price | rent_price | number(28, 8) |
| 30 | rowno | rowno | rowno | varchar2(30) |
| 31 | rowno_src | rowno_src | rowno_src | varchar2(30) |
| 32 | start_date | start_date | start_date | char(19) |
| 33 | def1 | def1 | def1 | varchar2(101) |
| 34 | def2 | def2 | def2 | varchar2(101) |
| 35 | def3 | def3 | def3 | varchar2(101) |
| 36 | def4 | def4 | def4 | varchar2(101) |
| 37 | def5 | def5 | def5 | varchar2(101) |
| 38 | def6 | def6 | def6 | varchar2(101) |
| 39 | def7 | def7 | def7 | varchar2(101) |
| 40 | def8 | def8 | def8 | varchar2(101) |
| 41 | def9 | def9 | def9 | varchar2(101) |
| 42 | def10 | def10 | def10 | varchar2(101) |
| 43 | def11 | def11 | def11 | varchar2(101) |
| 44 | def12 | def12 | def12 | varchar2(101) |
| 45 | def13 | def13 | def13 | varchar2(101) |
| 46 | def14 | def14 | def14 | varchar2(101) |
| 47 | def15 | def15 | def15 | varchar2(101) |
| 48 | def16 | def16 | def16 | varchar2(101) |
| 49 | def17 | def17 | def17 | varchar2(101) |
| 50 | def18 | def18 | def18 | varchar2(101) |
| 51 | def19 | def19 | def19 | varchar2(101) |
| 52 | def20 | def20 | def20 | varchar2(101) |
| 53 | vfree1 | vfree1 | vfree1 | varchar2(101) |
| 54 | vfree2 | vfree2 | vfree2 | varchar2(101) |
| 55 | vfree3 | vfree3 | vfree3 | varchar2(101) |
| 56 | vfree4 | vfree4 | vfree4 | varchar2(101) |
| 57 | vfree5 | vfree5 | vfree5 | varchar2(101) |
| 58 | vfree6 | vfree6 | vfree6 | varchar2(101) |
| 59 | vfree7 | vfree7 | vfree7 | varchar2(101) |
| 60 | vfree8 | vfree8 | vfree8 | varchar2(101) |
| 61 | vfree9 | vfree9 | vfree9 | varchar2(101) |
| 62 | vfree10 | vfree10 | vfree10 | varchar2(101) |
| 63 | dr | dr | dr | number(10, 0) |  |  | 0 |
| 64 | ts | ts | ts | char(19) |  |  | to_char(sysdate,'yyyy-mm-dd hh24:mi:ss') |