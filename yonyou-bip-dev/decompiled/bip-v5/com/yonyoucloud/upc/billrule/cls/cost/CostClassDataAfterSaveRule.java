...
@Component(value="costClassDataAfterSaveRule")
public class CostClassDataAfterSaveRule
extends AbstractCommonRule {
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> map) throws Exception {
        ...
        for (BizObject cls : clss) {
            ...
            SqlHelper.update((String)"com.yonyoucloud.upc.bill.dao.productcls.UpdateCostClassFullPath", params);
            CostClass costClass = (CostClass)cls;
            this.checkParentStopstatus(costClass);
            if (costClass.getEntityStatus() == EntityStatus.Update) {
                TreeApplyRangeUtils.getInstance().updateChildFPathAndLevel("pc.cls.CostClass", cls.get("id"));
                if (costClass.getStopstatus().booleanValue()) {
                    HashMap<String, Object> paramMap = new HashMap<String, Object>();
                    paramMap.put("id", costClass.getId());
            ...
            if (costClass.getEntityStatus() != EntityStatus.Insert) continue;
            this.checksParentRefer(costClass);
            costClass.setIsEnd(Boolean.valueOf(true));
        }
        return new RuleExecuteResult();
    }

    private void updateChildFPathAndLevel(String fullname, Object id) throws Exception {
    ...
    private void checkParentStopstatus(CostClass costClass) throws Exception {
        boolean pStopstatus;
        QuerySchema schema;
        List clss;
        if (!costClass.getStopstatus().booleanValue() && costClass.get("parent") != null && (clss = MetaDaoHelper.query((String)"pc.cls.CostClass", (QuerySchema)(schema = QuerySchema.create().addSelect("*").addCondition((ConditionExpression)QueryConditionGroup.and((ConditionExpression[])new ConditionExpression[]{QueryCondition.name((String)"id").eq(costClass.get("parent"))}))))) != null && clss.size() > 0 && (pStopstatus = ((Boolean)((Map)clss.get(0)).get("stopstatus")).booleanValue())) {
            Object[] objectArray = new Object[1];
            objectArray[0] = MultilangUtil.getAttr((String)"pc.cls.CostClass", (String)"name", (BizObject)costClass, null);
            throw new CoreDocBusinessException(CoreExtrateBusinessErrorEnum.ERROR_ENUM_000_527_800155, objectArray);
        }
    }

    private void checksParentRefer(CostClass costClass) throws Exception {
        QuerySchema schema;
        List products;
        if (costClass.getParent() != null && (products = MetaDaoHelper.query((String)"pc.product.Product", (QuerySchema)(schema = QuerySchema.create().addSelect("id ").addCondition((ConditionExpression)QueryConditionGroup.and((ConditionExpression[])new ConditionExpression[]{QueryCondition.name((String)"costClass").eq((Object)costClass.getParent())}))))).size() > 0) {
