...
@Component(value="materialStatuslistQueryRule")
public class MaterialStatuslistQueryRule
extends AbstractCommonRule {
    ...
    private void addMaterialStatusDetailAction(Pager pager) throws Exception {
        ...
        recordList.stream().forEach(record -> {
            Map map = (Map)record;
            if (null != map.get("materialStatusDetail!id")) {
                ids.add(Long.valueOf(map.get("materialStatusDetail!id").toString()));
            }
        });
        if (ids.isEmpty()) {
            return;
        ...
        recordList.stream().forEach(record -> {
            Map map = (Map)record;
            List detailActionNames = materialStatusDetailActionNames.stream().filter(actionNames -> actionNames.get("materialStatusDetailId").equals(map.get("materialStatusDetail!id"))).collect(Collectors.toList());
            if (detailActionNames.size() > 0) {
                ...
                for (int i = 0; i < detailActionNames.size(); ++i) {
                    ...
                    actionNames2.append(InternationalUtils.getMessageWithDefault((String)"UID:P_COREDOC-BE_179F6286040802B5", (String)"\u3001"));
                }
                map.put("materialStatusDetail!action_name", new String(actionNames2));
            }
        });
    }
}
