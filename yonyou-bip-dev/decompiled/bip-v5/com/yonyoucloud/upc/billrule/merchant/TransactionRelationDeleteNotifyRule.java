...
@Component(value="transactionRelationDeleteNotifyRule")
public class TransactionRelationDeleteNotifyRule
extends AbstractCommonRule {
    private static final Logger log = LoggerFactory.getLogger(TransactionRelationDeleteNotifyRule.class);
    @Autowired
    ...
    public RuleExecuteResult execute(BillContext billContext, Map<String, Object> map) throws Exception {
        ...
        if (null != bills && bills.size() > 0) {
            for (BizObject bill : bills) {
                ...
                try {
                    String eventType;
                    HashMap<String, Object> businessObject;
                    if (isCreator.booleanValue()) {
                        if (user.getUserType() != UserType.ShopUser && user.getUserType() != UserType.ShopAdminUser && !"true".equals(bill.getString("isTransactionRelationList"))) continue;
                        businessObject = new HashMap<String, Object>();
                        businessObject.put("id", bill.getId());
                        businessObject.put("createOrg", bill.getString("createOrg"));
                        businessObject.put("belongOrg", bill.getString("belongOrg"));
                        eventType = Objects.requireNonNull(EventType.find("shop_user_merchant", "delete", null)).toString();
                        this.eventService2.sendEvent("YXYBASEDOC", eventType, businessObject);
                        continue;
                    }
                    if (!this.orgExternalQryService.judgeExternalOrg((String)AppContext.getYTenantId(), bill.getString("createOrg")).booleanValue() && !this.orgExternalQryService.judgeExternalOrg((String)AppContext.getYTenantId(), bill.getString("belongOrg")).booleanValue()) continue;
                    businessObject = new HashMap();
                    businessObject.put("id", bill.getId());
                    businessObject.put("createOrg", bill.getString("createOrg"));
                    businessObject.put("belongOrg", bill.getString("belongOrg"));
                    eventType = Objects.requireNonNull(EventType.find("transaction_relationship", "delete", null)).toString();
                    this.eventService2.sendEvent("YXYBASEDOC", eventType, businessObject);
                }
                catch (Exception e) {
