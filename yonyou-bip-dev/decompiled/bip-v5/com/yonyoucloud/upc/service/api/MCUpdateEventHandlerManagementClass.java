...
@Component(value="pc.mcUpdateEventHandlerManagementClass")
public class MCUpdateEventHandlerManagementClass
implements IMCUpdateEventHandler {
    private static final Logger log = LoggerFactory.getLogger(MCUpdateEventHandlerManagementClass.class);
    @Autowired
    ...
    public void Handle(List<String> orgIds) throws Exception {
        if (orgIds != null && orgIds.size() > 0) {
            for (String orgId : orgIds) {
                ...
                if (null == subOrgs || subOrgs.isEmpty() || (clsIds = (clsIdsMap = MetaDaoHelper.query((String)"pc.cls.ManagementClass", (QuerySchema)QuerySchema.create().addSelect("id").addCondition((ConditionExpression)QueryConditionGroup.and((ConditionExpression[])new ConditionExpression[]{QueryCondition.name((String)"orgId").eq((Object)orgId)})))).stream().map(map -> map.get("id").toString()).collect(Collectors.toList())).isEmpty()) continue;
    ...
    public void HandleShareRange(Map shareRangeData) throws Exception {
        for (final Object key : shareRangeData.keySet()) {
            ...
            List<String> clsIds = this.upcControlRuleService.getSelfCreateVoucherByCreateOrg("pc.cls.ManagementClass", (List<String>)new ArrayList<String>(){
                {
            ...
            List<Map> rangesInDb = this.findAllRangesInDb(clsIds);
            log.info("MCUpdateEventHandlerManagementClass HandleShareRange exists range data:{}", rangesInDb);
            for (String clsId : clsIds) {
                List<String> rangesOfCls = this.dealRangesOfCls(rangesInDb, clsId);
    ...
    private List<Map> findAllRangesInDb(List<String> clsIds) throws Exception {
        return MetaDaoHelper.query((String)"pc.cls.ManagementClassApplyRangeGroup", (QuerySchema)QuerySchema.create().addSelect("id as rangeId,orgGroupId.orgGroupDetailList.orgId as orgId, clsId as clsId").addCondition((ConditionExpression)QueryConditionGroup.and((ConditionExpression[])new ConditionExpression[]{QueryCondition.name((String)"clsId").in(clsIds)})));
    }

    private void dealFinalRangesOfCls(List<String> rangesOfCls, String clsId) throws Exception {
    ...
    private List<String> dealRangesOfCls(List<Map> rangesInDb, String clsId) {
        ArrayList<String> rangeList = new ArrayList<String>();
        for (Map range : rangesInDb) {
            if (null == range.get("clsId") || !range.get("clsId").toString().equals(clsId) || null == range.get("orgId")) continue;
