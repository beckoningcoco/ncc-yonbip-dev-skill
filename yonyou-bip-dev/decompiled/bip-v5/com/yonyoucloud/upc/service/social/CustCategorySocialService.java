...
@Component(value="distributehandleraa.custcategory.CustCategory")
public class CustCategorySocialService
extends AbstractDistributeHandler {
    Logger logger = LoggerFactory.getLogger(CustCategorySocialService.class);
    @Autowired
    ...
    public Map handler(SocialDistributeParam socialDistributeParam) throws Exception {
        ...
        Callable<Map> callable = () -> SocialUntil.queryData((SocialDistributeParam)socialDistributeParam, (Long)srcTenantId, (Long)desTenantId, (String)srcYTenantId, (String)desYTenantId);
        Map queryData = (Map)RobotExecutors.runAs((String)srcYTenantId, callable);
            ...
            if (!CollectionUtils.isEmpty((Collection)distributes)) {
                ...
                distributes.forEach(data -> {
                    ...
                    if (isSingleOrg != null && defaultOpenApp.equals(isSingleOrg.toString())) {
                        List result = this.iServiceIsolateService.findMainOrgPermission(AppContext.getCurrentUser().getYhtUserId(), "aa_custcategory", AppContext.getCurrentUser().getYhtTenantId());
                        if (CollectionUtils.isEmpty((Collection)result)) {
                            throw new CoreDocBusinessException(CoreExtrateBusinessErrorEnum.ERROR_ENUM_000_527_801018);
                        }
                        if (result.size() == 1 && "666666".equals(result.get(0))) {
                            throw new CoreDocBusinessException(CoreExtrateBusinessErrorEnum.ERROR_ENUM_000_527_801018);
                        }
                        List stringList = result.stream().filter(r -> !r.equals("666666")).collect(Collectors.toList());
                    ...
                    if (existData != null) {
                        ...
                        SocialUntil.setCustCategoryApplyRange((CustCategory)data);
                    }
                    ...
                    try {
                        if (data.containsKey((Object)"parent") && data.get("parent") != null) {
