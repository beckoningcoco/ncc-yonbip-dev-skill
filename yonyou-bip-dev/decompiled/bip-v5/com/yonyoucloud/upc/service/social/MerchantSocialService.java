...
@Component(value="distributehandleraa.merchant.Merchant")
public class MerchantSocialService
extends AbstractDistributeHandler {
    Logger logger = LoggerFactory.getLogger(MerchantSocialService.class);
    @Autowired
        ...
        if (!CollectionUtils.isEmpty((Collection)distributes)) {
            ...
            distributes.forEach(data -> {
                ...
                if (isSingleOrg != null && defaultOpenApp.equals(isSingleOrg.toString())) {
                    List result = (List)RobotExecutors.runAs((String)desYTenantId, () -> this.iServiceIsolateService.findMainOrgPermission(AppContext.getCurrentUser().getYhtUserId(), "aa_custcategory", AppContext.getCurrentUser().getYhtTenantId()));
                    ...
                    data.setCreateOrg((String)stringList.get(0));
                } else {
                    data.setCreateOrg("666666");
                }
                if (existData != null) {
